...
//    printf("%d: l %d\n", pkt_n, hdr.len);
        if (hdr.len == 0) {
            fprintf(stderr,
                    "at pkt %d skip zero byte - hmm? Was it 0a -> 0d 0a translation??\n",
                    pkt_n);
            pkt_n--;
            continue;
        }
...
#define DATA_BUF (iobuf + ETH_AND_UDP_HDR_OFFSETS + real_mode + \
                  custom_ns_header_offset)

        if ((r = fread(DATA_BUF, hdr.len, 1, in_f)) != 1) {
            fprintf(stderr,
                    "packet #%d: error reading %d bytes from the input file\n",
                    pkt_n, hdr.len);
            return -1;
        }
        if (real_mode && hdr.len == 1 && hdr.type == 1 && prev_len &&
            prev_len < 3) {
        ...
        if ((hdr.type & 0x7f) != ZB_MAC_TRANSPORT_TYPE_DUMP &&
            hdr.type != ZB_MAC_TRANSPORT_TYPE_DATA) {
            continue;
        }

        // create_eth_hdr(iobuf, hdr.len + real_mode, (hdr.type != 1));
        cap_hdr.caplen = cap_hdr.len = ETH_AND_UDP_HDR_OFFSETS + hdr.len +
                                       real_mode + custom_ns_header_offset;
