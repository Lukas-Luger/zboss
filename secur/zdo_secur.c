...
void zb_apsme_transport_key_indication(zb_uint8_t param) ZB_CALLBACK
{
    ...
    zb_apsme_transport_key_indication_t *ind =
        ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(param),
    ...
    switch (ind->key_type) {
        case ZB_STANDARD_NETWORK_KEY:
        {
            ...
            if (ZB_IEEE_ADDR_IS_ZERO(ZB_AIB().trust_center_address)) {
                ZB_IEEE_ADDR_COPY(
                    ZB_AIB().trust_center_address, ind->src_address);
                TRACE_MSG(TRACE_SECUR1, "TC is " TRACE_FORMAT_64,
                          (FMT__A,
                           TRACE_ARG_64(ZB_AIB().trust_center_address)));
...
#ifdef ZB_TC_AT_ZC
                {
                    ...
                    (void)zb_address_update(
                        ZB_AIB().trust_center_address, 0, ZB_FALSE, &addr_ref);
                }
#else
                /*
...
#error Dont know what to do here!
            ...
            if (secur_nwk_key_is_empty(ind->key.nwk.key) &&
                ...
                i = (ZG->nwk.nib.active_secur_material_i +
                     (zb_uint8_t)(ind->key.nwk.key_seq_number -
                                  ZG->nwk.nib.active_key_seq_number))
                    % ZB_SECUR_N_SECUR_MATERIAL;
                ZB_MEMCPY(ZG->nwk.nib.secur_material_set[i].key,
                          ind->key.nwk.key, ZB_CCM_KEY_SIZE);
                ZG->nwk.nib.secur_material_set[i].key_seq_number =
                    ind->key.nwk.key_seq_number;
            ...
            if (!ZG->aps.authenticated) {
                ...
                TRACE_MSG(TRACE_SECUR1, "authenticated; curr key #%hd",
                          (FMT__H, ind->key.nwk.key_seq_number));
        ...
        break;
        default:
            TRACE_MSG(TRACE_SECUR1, "unsupported key type %hd",
                      (FMT__H, ind->key_type));
            zb_free_buf(ZB_BUF_FROM_REF(param));
            break;
    }
...
static zb_ushort_t zb_secur_gen_upd_dev_status(zb_ushort_t rejoin_network,
                                               zb_ushort_t secure_rejoin);
...
void secur_authenticate_child(zb_uint8_t param) ZB_CALLBACK
{
    ...
    upd_dev_status = zb_secur_gen_upd_dev_status(ind.rejoin_network,
                                                 ind.secure_rejoin);
    TRACE_MSG(TRACE_SECUR3, "authenticate child %d, st %hd",
              (FMT__D_H, ind.network_address, upd_dev_status));
#ifdef ZB_COORDINATOR_ROLE
    if (ZB_NIB_DEVICE_TYPE() == ZB_NWK_DEVICE_TYPE_COORDINATOR
        ...
        zb_apsme_transport_key_req_t *req =
            ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                 param), zb_apsme_transport_key_req_t);
        ...
        if (upd_dev_status == ZB_STD_SEQ_SECURED_REJOIN) {
            ...
            TRACE_MSG(TRACE_SECUR3, "Send empty nwk key to " TRACE_FORMAT_64,
                      (FMT__A, TRACE_ARG_64(ind.extended_address)));
        }
        else {
            ZB_MEMCPY(req->key.nwk.key,
                      ZG->nwk.nib.secur_material_set[ZG->nwk.nib.
                                                     active_secur_material_i].key,
                      ZB_CCM_KEY_SIZE);
            req->key.nwk.key_seq_number = ZG->nwk.nib.active_key_seq_number;
            TRACE_MSG(TRACE_SECUR3, "Send nwk key #%hd to " TRACE_FORMAT_64,
                      (FMT__H_A,
                       req
                       ->key.nwk.key_seq_number,
...
#error Implement PRO!
...
#endif  /* ZB_COORDINATOR_ROLE */
    {
        ...
        zb_apsme_update_device_req_t *req =
            ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                 param), zb_apsme_update_device_req_t);
        ...
        TRACE_MSG(TRACE_SECUR3,
                  "update device %d status %hd; send cmd to " TRACE_FORMAT_64,
                  (FMT__D_H, ind.network_address, upd_dev_status,
                   TRACE_ARG_64(req->dest_address)));
...
{
    zb_uint16_t dest_br =
        *ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(param), zb_uint16_t);
    zb_apsme_transport_key_req_t *req = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                             param),
    ...
    req->key.nwk.key_seq_number = ZG->nwk.nib.active_key_seq_number + 1;
    {
        ...
        if (!key) {
            TRACE_MSG(TRACE_ERROR, "No nwk key # %hd",
                      (FMT__H, req->key.nwk.key_seq_number));
            return;
        }
...
void zb_secur_send_nwk_key_switch(zb_uint8_t param) ZB_CALLBACK
{
    zb_uint16_t dest_br =
        *ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(param), zb_uint16_t);
    zb_apsme_switch_key_req_t *req = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                          param),
                                                      zb_apsme_switch_key_req_t);

    TRACE_MSG(TRACE_SECUR1, "zb_secur_send_nwk_key_switch param %hd dest_br %d",
              (FMT__H_D, param, dest_br));
    if (dest_br == 0) {
        ...
        TRACE_MSG(TRACE_SECUR3, "broadcast key switch, remember key_sw %hd",
                  (FMT__H, ZG->zdo.handle.key_sw));
...
void secur_send_key_sw_next(zb_uint8_t param) ZB_CALLBACK
{
    ...
    zb_address_ieee_by_ref(req->dest_address.addr_long,
                           ZG->nwk.neighbor.base_neighbor[ZG->aps.tmp.
                                                          neighbor_table_iterator].addr_ref);
    ZB_SCHEDULE_CALLBACK(zb_apsme_switch_key_request, param);

    ZG->aps.tmp.neighbor_table_iterator = zb_nwk_neighbor_next_rx_on_i(
        ZG->aps.tmp.neighbor_table_iterator + 1);
    if (ZG->aps.tmp.neighbor_table_iterator != (zb_ushort_t) ~0) {
...
{
    zb_ushort_t i;
    zb_ushort_t shift = (ZG->nwk.nib.active_secur_material_i + 1) %
                        ZB_SECUR_N_SECUR_MATERIAL;

    for (i = 0;
         i < ZB_SECUR_N_SECUR_MATERIAL
         && (ZG->nwk.nib.secur_material_set[(i + shift) %
                                            ZB_SECUR_N_SECUR_MATERIAL].
             key_seq_number == key_number);
         ++i) {}
...
#endif
        {
            TRACE_MSG(TRACE_SECUR1, "Could not find nwk key #%hd !",
                      (FMT__H, key_number));
        }
    }
    ...
    if (i != ZB_SECUR_N_SECUR_MATERIAL) {
        ...
        ZG->nwk.nib.outgoing_frame_counter = 0;
        TRACE_MSG(TRACE_SECUR1, "switched to nwk key #%hd, i %hd",
                  (FMT__H_H, key_number, i));
    }
...
{
    zb_uint16_t *dest_br_p = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                  param), zb_uint16_t);
                                               ...
                                               zb_ushort_t secure_rejoin)
{
    ...
    switch (v) {
        ...
        default:
            TRACE_MSG(TRACE_SECUR1, "bad rejoin/secur combination %hd %hd",
                      (FMT__H_H, rejoin_network, secure_rejoin));
            return -1;
    }
}
...
void zb_apsme_update_device_indication(zb_uint8_t param) ZB_CALLBACK
{
    ...
    ind =
        *ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(param), zb_apsme_update_device_ind_t);
    if (ind.status == ZB_DEVICE_LEFT) {
        zb_address_ieee_ref_t addr_ref;
        /* remove device from the neighbor table and address translation table */
        TRACE_MSG(TRACE_SECUR3, "Device %d left - forget it",
                  (FMT__D, ind.device_short_address));

        if (zb_address_by_short(ind.device_short_address, ZB_FALSE, ZB_FALSE,
                                &addr_ref) == RET_OK
            || zb_address_by_ieee(ind.device_address, ZB_FALSE, ZB_FALSE,
                                  &addr_ref) == RET_OK) {
        ...
        if (ZDO_CTX().handle.allow_auth) {
            zb_apsme_transport_key_req_t *req;
            req = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                       param), zb_apsme_transport_key_req_t);
            ...
            if (ind.status == ZB_STD_SEQ_SECURED_REJOIN) {
                ...
                req->key.nwk.key_seq_number = 0;
                TRACE_MSG(TRACE_SECUR3,
                          "Indirect send empty nwk key to " TRACE_FORMAT_64,
                          (FMT__A, TRACE_ARG_64(ind.device_address)));
            }
            else if (ind.status == ZB_STD_SEQ_UNSECURED_JOIN ||
                     ind.status == ZB_STD_SEQ_UNSECURED_REJOIN) {
                /* send nwk key. */
                ZB_MEMCPY(req->key.nwk.key,
                          ZG->nwk.nib.secur_material_set[ZG->nwk.nib.
                                                         active_secur_material_i].key,
                ...
                req->key.nwk.key_seq_number = ZG->nwk.nib.active_key_seq_number;
                TRACE_MSG(TRACE_SECUR3,
                          "Indirect send nwk key #%hd to " TRACE_FORMAT_64,
                          (FMT__H_A, req->key.nwk.key_seq_number,
                           TRACE_ARG_64(ind.device_address)));
            }
            ZB_SCHEDULE_CALLBACK(zb_apsme_transport_key_request, param);
        }
        else {
            /* auth is disallowed, send remove device request */
            zb_apsme_remove_device_req_t *req = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                                     param),
...
void zb_apsme_remove_device_indication(zb_uint8_t param) ZB_CALLBACK
{
    ...
    ind =
        *ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(param), zb_apsme_remove_device_ind_t);
    lr = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(param), zb_nlme_leave_request_t);
    TRACE_MSG(TRACE_SECUR2,
              "zb_apsme_remove_device_ind from " TRACE_FORMAT_64 " child " TRACE_FORMAT_64,
              (FMT__A_A, TRACE_ARG_64(ind.src_address),
               TRACE_ARG_64(ind.child_address)));
    ...
    if (ZB_IEEE_ADDR_CMP(ind.src_address, ZB_AIB().trust_center_address)) {
        ...
        TRACE_MSG(TRACE_SECUR1,
                  "zb_apsme_remove_device_ind from non-TC - ignore it",
                  (FMT__0));
        zb_free_buf(ZB_BUF_FROM_REF(param));
    }
}
...
{
    zb_apsme_switch_key_ind_t *ind = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                          param),
                                                      zb_apsme_switch_key_ind_t);

    TRACE_MSG(TRACE_SECUR3,
              "zb_apsme_switch_key_ind from " TRACE_FORMAT_64 " key # %hd",
...
void zb_apsme_request_key_indication(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (ind.key_type == ZB_STANDARD_NETWORK_KEY) {
        ...
        if (!(zb_address_by_ieee(ind.src_address, ZB_FALSE, ZB_FALSE,
                                 &ref) == RET_OK
...
{
    return (zb_bool_t) !secur_nwk_key_is_empty(
        &ZG->nwk.nib.secur_material_set[0].key[0]);
}
...
void zb_secur_setup_preconfigured_key(zb_uint8_t *key, zb_uint8_t i)
{
    if (i >= ZB_SECUR_N_SECUR_MATERIAL) {
        ...
        ZB_MEMCPY(&ZG->nwk.nib.secur_material_set[i].key[0], key,
                  ZB_CCM_KEY_SIZE);
...
zb_uint8_t *secur_nwk_key_by_seq(zb_ushort_t key_seq_number)
{
    ...
    TRACE_MSG(TRACE_SECUR3, "cnt %hd seq %hd - i %hd, ret %p",
              (FMT__H_H_H_P, cnt, key_seq_number, i,
               ZG->nwk.nib.secur_material_set[i].key));
