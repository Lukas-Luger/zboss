...
static void zb_secur_nwk_status(zb_uint8_t param, zb_uint16_t addr_short,
                                zb_uint8_t status);
                             ...
                             zb_buf_t *dst)
{
    ...
    aux =
        (zb_nwk_aux_frame_hdr_t *)((zb_uint8_t *)nwhdr + hdr_size -
                                   sizeof(*aux));
    ...
    if (src->u.hdr.use_same_key
        ...
        TRACE_MSG(TRACE_SECUR3, "use same key number (%hd) and counter %hd",
                  (FMT__H_H, aux->key_seq_number,
                   ZG->nwk.nib.prev_outgoing_frame_counter));
        ZB_HTOLE32(&aux->frame_counter,
                   &ZG->nwk.nib.prev_outgoing_frame_counter);
        ZG->nwk.nib.prev_outgoing_frame_counter++;
    }
    else
#endif
    {
        ...
        aux->key_seq_number = ZG->nwk.nib.active_key_seq_number;
        TRACE_MSG(TRACE_SECUR3, "use active key number - %hd",
                  (FMT__H, aux->key_seq_number));
    }
    ...
    TRACE_MSG(TRACE_SECUR2,
              "secure nwk frm %p[%hd - %hd] hdr_size %hd fcnt %lx",
              (FMT__P_H_H_H_L, src, ZB_BUF_LEN(src), mac_hdr_size, hdr_size,
               aux->frame_counter));

    {
        zb_uint8_t *key = secur_nwk_key_by_seq(aux->key_seq_number);
        if (!key) {
            TRACE_MSG(TRACE_ERROR, "Can't get key by seq# %hd",
                      (FMT__H, aux->key_seq_number));
            ret = RET_ERROR;
        }
        else {
            ...
            ret = zb_ccm_encrypt_n_auth(key,
                                        ...
                                        (zb_uint8_t *)nwhdr + hdr_size, ZB_BUF_LEN(
                                            src) - hdr_size - mac_hdr_size,
                                        dst);
        }
    }
    if (ret == RET_OK) {
        ...
        aux =
            (zb_nwk_aux_frame_hdr_t *)(ZB_BUF_BEGIN(dst) +
                                       ((zb_uint8_t *)aux - ZB_BUF_BEGIN(src)));
        aux->secur_control = ZB_NWK_STD_SECUR_CONTROL_ZEROED_LEVEL;

        TRACE_MSG(TRACE_SECUR2, "secured frm %p[%hd] %d fcnt %ld",
                  (FMT__P_H_D_L,
                   dst,
                   ZB_BUF_LEN(dst), ret, ZG->nwk.nib.outgoing_frame_counter));
                               ...
                               zb_uint8_t mhr_len)
{
    ...
    if (!ZG->aps.authenticated) {
        TRACE_MSG(TRACE_SECUR3, "Not authenticated - drop this frame",
                  (FMT__0));
...

    {
        ...
        aux =
            (zb_nwk_aux_frame_hdr_t *)((zb_uint8_t *)nwk_hdr + hdr_size -
        ...
        if (ZB_FCF_GET_SRC_ADDRESSING_MODE(mhr->frame_control) ==
            ...
            ret = zb_address_update(aux->source_address,
                                    mhr->src_addr.addr_short, ZB_FALSE,
                                    &addr_ref);
        }
        if (ret == RET_OK) {
    ...
    if (ret == RET_OK) {
        ...
        if (nbe->key_seq_number != aux->key_seq_number) {
            ...
            TRACE_MSG(TRACE_SECUR3, "peer switched key to seq %hd",
                      (FMT__H, nbe->key_seq_number));
        }

        if (!key) {
            /* set 'frame security failed' */
            ret = RET_ERROR;
            TRACE_MSG(TRACE_SECUR3, "no key by seq %hd",
                      (FMT__H, aux->key_seq_number));
            zb_secur_nwk_status(param, mhr->src_addr.addr_short,
            ...
            if (nbe->incoming_frame_counter > frame_counter
                ...
                TRACE_MSG(TRACE_SECUR1,
                          "frm cnt %ld->%ld shift back - indicate status",
                          (FMT__L_L,
                           nbe
        ...
        TRACE_MSG(TRACE_SECUR1, "can't get neighbor - indicate nwk status",
                  (FMT__0));
        zb_secur_nwk_status(param, mhr->src_addr.addr_short,
    ...
    if (ret == RET_OK) {
        ...
        if (ret == RET_OK
            ...
            TRACE_MSG(TRACE_SECUR3,
                      "too short nsdu: %hd hdr %hd - indicate status",
                      (FMT__H_H, ZB_BUF_LEN(nsdu), hdr_size));
            zb_secur_nwk_status(param, mhr->src_addr.addr_short,
                                ZB_NWK_COMMAND_STATUS_FRAME_SECURITY_FAILED);
        }
        ...
        if (ret == RET_OK) {
            ...
            TRACE_MSG(TRACE_SECUR3, "unsecured frm %p[%hd] ok",
                      (FMT__P_H, nsdu, ZB_BUF_LEN(nsdu)));
        }
        else {
            TRACE_MSG(TRACE_SECUR3, "unsecure failed %d - indicate nwk status",
                      (FMT__D, ret));
            zb_secur_nwk_status(param, mhr->src_addr.addr_short,
                                ZB_NWK_COMMAND_STATUS_FRAME_SECURITY_FAILED);
        }
    ...
    if (ret == RET_OK
        ...
        /*
           ...
           sequence number."
         */
        TRACE_MSG(TRACE_SECUR1, "implicit key switch to %hd",
                  (FMT__H, aux->key_seq_number));
...
static void zb_secur_nwk_status(zb_uint8_t param, zb_uint16_t addr_short,
                                zb_uint8_t status)
{
    zb_nlme_status_indication_t *cmd = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                            param),
