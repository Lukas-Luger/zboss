    ...
    zb_uint8_t  *pT)
{
    ...
    msg_len +=
        (lenString_a !=
         0) ? ((lenString_a + 2) - ((lenString_a + 2) & 0x0f)) : 0;
...

{
    ...
    if (!pbuf) {
        return;                                   /* No free space available for operation */
                                                            ...
                                                            Output:  The first Mval bytes contain computed Authentication Tag T */
{
    ...
    if (!ccm_m) {
        return RET_ERROR;         /* Check if authentication is even requested. */
...

{
    ...
    ZB_BUF_INITIAL_ALLOC(crypted_text, string_a_len + string_m_len + ccm_m,
                         c_text);
