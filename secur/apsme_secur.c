    ...
    zb_uint16_t dest_short = 0;
    {
        ...
        if (req->key_type == ZB_STANDARD_NETWORK_KEY) {
            ...
            if (req->addr_mode == ZB_ADDR_16BIT_DEV_OR_BROADCAST) {
                ...
                ZB_IEEE_ADDR_COPY(dsc->dest_address,
                                  req->dest_address.addr_long);
            }
            ZB_IEEE_ADDR_COPY(dsc->source_address, ZB_PIB_EXTENDED_ADDRESS());
            if (req->key.nwk.use_parent) {
                dest_short = zb_address_short_by_ieee(
                    req->key.nwk.parent_address);
            ...
            TRACE_MSG(TRACE_SECUR3,
                      "send std nwk key #%hd from " TRACE_FORMAT_64,
                      (FMT__H_A, dsc->seq_number,
                       TRACE_ARG_64(dsc->source_address)));
...
void zb_apsme_update_device_request(zb_uint8_t param) ZB_CALLBACK
{
    ...
    ZB_BUF_INITIAL_ALLOC(ZB_BUF_FROM_REF(param),
                         sizeof(zb_apsme_update_device_pkt_t), p);

    ZB_IEEE_ADDR_COPY(p->device_address, req->device_address);
    ZB_HTOLE16(&p->device_short_address, &req->device_short_address);
    p->status = req->status;
    TRACE_MSG(TRACE_SECUR3,
              "send UPDATE-DEVICE " TRACE_FORMAT_64 " %d st %hd to " TRACE_FORMAT_64,
              (FMT__A_D_H, TRACE_ARG_64(p->device_address),
               p
               ->device_short_address, p->status,
...
void zb_apsme_switch_key_request(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (req->addr_mode == ZB_ADDR_16BIT_DEV_OR_BROADCAST) {
        TRACE_MSG(TRACE_SECUR1, "zb_apsme_switch_key_req dest %d # %d",
                  (FMT__D_D, req->dest_address.addr_short,
                   req->key_seq_number));
        ...
        TRACE_MSG(TRACE_SECUR1,
                  "zb_apsme_switch_key_req dest " TRACE_FORMAT_64 " %d # %d",
                  (FMT__A_D_D, TRACE_ARG_64(req->dest_address.addr_long),
                   req->dest_address.addr_short, req->key_seq_number));
    }
...
{
    zb_transport_key_nwk_key_dsc_pkt_t *dsc =
        (zb_transport_key_nwk_key_dsc_pkt_t *)ZB_BUF_BEGIN(ZB_BUF_FROM_REF(
                                                               param));
    ...
    switch (dsc->key_type) {
        case ZB_STANDARD_NETWORK_KEY:
            if (
                 ...
                 */
                zb_apsme_transport_key_indication_t *ind = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                                                param),
                                                                            zb_apsme_transport_key_indication_t);

                TRACE_MSG(TRACE_SECUR3, "in std nwk key #%d for me",
                          (FMT__D, dsc->seq_number));
...
/* This feature should be processed at request */
                ...
                if (ZB_IEEE_ADDR_IS_ZERO(dsc->dest_address)
                    ...
                    && ZB_IEEE_ADDR_CMP(dsc->source_address,
                     ...
                     */
                    ZG->aps.tmp.neighbor_table_iterator =
                        zb_nwk_neighbor_next_ze_children_rx_off_i(0);
                    ZG->aps.tmp.key_seq_number = ind->key.nwk.key_seq_number;
                    if (ZG->aps.tmp.neighbor_table_iterator !=
                        (zb_ushort_t) ~0) {
                        TRACE_MSG(TRACE_SECUR3, "send key #%hd to all ZE",
                                  (FMT__H, dsc->seq_number));
                ...
                if (zb_address_by_ieee(dsc->dest_address, ZB_FALSE, ZB_FALSE,
                                       &addr_ref) == RET_OK
                    ...
                    TRACE_MSG(TRACE_SECUR3, "send key #%hd to ZE %d, auth ok",
                              (FMT__H_D, dsc->seq_number, addr));
                    ...
                    TRACE_MSG(TRACE_SECUR1,
                              "child " TRACE_FORMAT_64 " not found",
                              (FMT__A, TRACE_ARG_64(dsc->dest_address)));
...
void zb_aps_in_update_device(zb_uint8_t param)
{
    ...
    zb_apsme_update_device_pkt_t *pkt =
        (zb_apsme_update_device_pkt_t *)ZB_BUF_BEGIN(ZB_BUF_FROM_REF(param));
    zb_apsme_update_device_ind_t *ind = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                             param),
    ...
    {
        zb_uint16_t src_short_addr = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                          param),
     ...
     * about. Remember it. */
    {
        ...
        (void)zb_address_update(ind->device_address, ind->device_short_address,
                                ZB_FALSE, &ref);
    }
...
void zb_aps_pass_nwk_key_to_children(zb_uint8_t param) ZB_CALLBACK
{
    ...
    ZG->nwk.neighbor.base_neighbor[ZG->aps.tmp.neighbor_table_iterator].
    relationship = ZB_NWK_RELATIONSHIP_CHILD;

    ZB_IEEE_ADDR_COPY(dsc->source_address, ZB_AIB().trust_center_address);

    TRACE_MSG(TRACE_SECUR3, "send nwk key #%hd to ch %d",
              (FMT__H_D, dsc->seq_number, dest_short));
    zb_aps_send_command(param,
                        dest_short,
    ...
    /* handle next child */
    ZG->aps.tmp.neighbor_table_iterator =
        zb_nwk_neighbor_next_ze_children_rx_off_i(
...
void zb_secur_apsme_remove_device(zb_uint8_t param) ZB_CALLBACK
{
    ...
    ZB_BUF_INITIAL_ALLOC(ZB_BUF_FROM_REF(param),
                         sizeof(zb_apsme_remove_device_pkt_t), p);

    ZB_IEEE_ADDR_COPY(p->child_address, req->child_address);
    TRACE_MSG(TRACE_SECUR2,
              "send REMOVE-DEVICE " TRACE_FORMAT_64 " to " TRACE_FORMAT_64,
              (FMT__A_A, TRACE_ARG_64(p->child_address),
               TRACE_ARG_64(
                   req->parent_address)));
...
{
    /* get source address from the nwk header and convert it to long address */
    zb_apsme_remove_device_pkt_t *pkt =
        (zb_apsme_remove_device_pkt_t *)ZB_BUF_BEGIN(ZB_BUF_FROM_REF(param));
    zb_apsme_remove_device_ind_t *ind = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                             param),
...
{
    /* get source address from the nwk header and convert it to long address */
    zb_apsme_request_nwk_key_pkt_t *pkt =
        (zb_apsme_request_nwk_key_pkt_t *)ZB_BUF_BEGIN(ZB_BUF_FROM_REF(param));
    zb_apsme_request_key_ind_t *ind = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                           param),
...
{
    /* get source address from the nwk header and convert it to long address */
    zb_apsme_switch_key_pkt_t *pkt = (zb_apsme_switch_key_pkt_t *)ZB_BUF_BEGIN(ZB_BUF_FROM_REF(
                                                                                   param));
...
{
    zb_apsme_request_key_req_t *req = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                           param),
    ...
    if (req->key_type == ZB_SECUR_NWK_KEY) {
        ...
        ZB_BUF_INITIAL_ALLOC(ZB_BUF_FROM_REF(param),
                             sizeof(zb_apsme_request_nwk_key_pkt_t), p);
