                             ...
                             zb_buf_t *dst)
{
    ...
    ret = zb_ccm_encrypt_n_auth(MAC_PIB().mac_key,
                                ...
                                ZB_BUF_BEGIN(src) + mac_hdr_size, ZB_BUF_LEN(
                                    src) - mac_hdr_size,            /* m */
                                dst);
...
zb_ret_t zb_mac_unsecure_frame(zb_uint8_t param)
{
...

    {
        ...
        for (i = 0; i < MAC_PIB().mac_device_table_entries; ++i) {
            TRACE_MSG(TRACE_SECUR2,
                      "i %hd is_long %hd panid 0x%x mhr_panid 0x%x",
                      (FMT__H_H_D_D, i, is_long_addr,
                       MAC_PIB().mac_device_table[i].pan_id, mhr.src_pan_id));
            ...
            if (MAC_PIB().mac_device_table[i].pan_id == mhr.src_pan_id) {
                if (is_long_addr) {
                    ...
                    TRACE_MSG(TRACE_SECUR2, "i %hd addr 0x%x mhr_addr 0x%x",
                              (FMT__H_D_D, i,
                               MAC_PIB().mac_device_table[i].short_address,
        ...
        if (i == MAC_PIB().mac_device_table_entries) {
            ...
            TRACE_MSG(TRACE_SECUR1,
                      "frm cnt %ld->%ld shift back - MAC unsecure failed",
                      (FMT__L_L,
                       MAC_PIB().mac_device_table[i].frame_counter >
                       mhr.frame_counter));
        }
    ...
    if (ret == RET_OK) {
        ...
        zb_uint8_t *p;
        ZB_MEMCPY(save_tail,
                  ZB_BUF_BEGIN(buf) + ZB_BUF_LEN(
                      buf) - ZB_TAIL_SIZE_FOR_RECEIVED_MAC_FRAME,
