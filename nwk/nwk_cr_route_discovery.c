...
void zb_nlme_route_discovery_request(zb_uint8_t param) ZB_CALLBACK
{
    ...
    zb_nlme_route_discovery_request_t *request = ZB_GET_BUF_PARAM(buf,
                                                                  zb_nlme_route_discovery_request_t);

    TRACE_MSG(TRACE_NWK1, ">>zb_nlme_route_discovery_request %hd",
              (FMT__H, param));
    ...
    if (ret == RET_OK
        ...
        NWK_ROUTE_DISCOVERY_CONFIRM(buf, ZB_NWK_STATUS_INVALID_REQUEST,
                                    ZB_NWK_COMMAND_STATUS_NO_ROUTE_AVAILABLE);
        ret = RET_ERROR;
        TRACE_MSG(TRACE_NWK1, "no routing capacity available, ret error",
                  (FMT__0));
        goto done;
    }
    ...
    if (ret == RET_OK
        ...
        TRACE_MSG(TRACE_NWK1,
                  "truing to discover our own address, send success", (FMT__0));
        goto done;
    }
    ...
    /* check it's not our neighbour */
    {
        ...
        if (ret == RET_OK
            && zb_nwk_neighbor_get_by_short(request->network_addr,
                                            &nbt) == RET_OK) {
            NWK_ROUTE_DISCOVERY_CONFIRM(buf, ZB_NWK_STATUS_SUCCESS, 0xff);
            TRACE_MSG(TRACE_NWK1,
                      "discovery device found in neighbour table, ret success",
                      (FMT__0));
            goto done;
        }
    }
    ...
    if (ret == RET_OK) {
        if (request->address_mode == ZB_ADDR_16BIT_DEV_OR_BROADCAST) {
            ...
            zb_nwk_mesh_route_discovery(buf, request->network_addr,
                                        request->radius);
        }
...
#else
    NWK_ROUTE_DISCOVERY_CONFIRM(buf, ZB_NWK_STATUS_ROUTE_ERROR,
                                ZB_NWK_COMMAND_STATUS_NO_ROUTING_CAPACITY);
#endif

done:
    TRACE_MSG(TRACE_NWK1, "<<zb_nlme_route_discovery_request %d",
              (FMT__D, ret));
}
#endif
/*! @} */
