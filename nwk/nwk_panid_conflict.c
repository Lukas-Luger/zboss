 ...
 */
void zb_panid_conflict_got_network_report(zb_uint8_t param, zb_uint16_t *panids,
                                          zb_uint8_t n_panids)
{
    if (ZG->nwk.handle.state == ZB_NLME_STATE_PANID_CONFLICT_RESOLUTION
        || ZG->nwk.handle.panid_conflict) {
        TRACE_MSG(TRACE_NWK3, "panid conflict resolve is already in progress",
                  (FMT__0));
        zb_free_buf(ZB_BUF_FROM_REF(param));
    }
    else {
        TRACE_MSG(TRACE_NWK3,
                  "got panid report, n_panids %hd, start conflict resolve",
                  (FMT__H, n_panids));
        ZB_BZERO(ZG->nwk.handle.known_panids,
                 sizeof(ZG->nwk.handle.known_panids));
        if (n_panids >
            sizeof(ZG->nwk.handle.known_panids) /
            sizeof(ZG->nwk.handle.known_panids[0])) {
            n_panids = sizeof(ZG->nwk.handle.known_panids) /
                       sizeof(ZG->nwk.handle.known_panids[0]);
        }
        ZB_MEMCPY(ZG->nwk.handle.known_panids, panids,
                  n_panids * sizeof(zb_uint16_t));
...
void zb_panid_conflict_remember_panid(zb_uint16_t panid) ZB_SDCC_REENTRANT
{
    ...
    for (i = 0;
        ...
        if (ZG->nwk.handle.known_panids[i] == panid) {
            ...
            TRACE_MSG(TRACE_NWK3, "i %hd remembered panid 0x%x",
                      (FMT__H_D, i, panid));
            return;
        }
    }
...
void zb_panid_conflict_scan_start(zb_uint8_t param) ZB_CALLBACK
{
    if (ZG->nwk.handle.state != (zb_uint8_t)ZB_NLME_STATE_IDLE) {
        TRACE_MSG(TRACE_NWK3, "NWK is busy, will resolve PANID conflict later",
                  (FMT__0));
        ...
        ZB_MLME_BUILD_SCAN_REQUEST(ZB_BUF_FROM_REF(
                                       param), ZB_DEFAULT_APS_CHANNEL_MASK, ACTIVE_SCAN,
...
void zb_panid_conflict_network_update(zb_uint8_t param) ZB_SDCC_REENTRANT
{
    ...
    TRACE_MSG(TRACE_NWK3,
              "zb_panid_conflict_network_update: chosen new_panid 0x%x",
              (FMT__D, ZG->nwk.handle.new_panid));
...
zb_uint8_t *zb_nwk_fill_out_command(zb_uint8_t param, zb_uint16_t dest,
                                    ...
                                    zb_uint8_t size) ZB_SDCC_REENTRANT
{
    ...
    TRACE_MSG(TRACE_NWK1,
              "zb_nwk_fill_out_command param %hd dest %d command_id %hd size %hd",
    ...
    hdr_size =
        have_long_dst ? ZB_NWK_FULL_HDR_SIZE(0) : ZB_NWK_HALF_HDR_SIZE(0);

    {
        ...
        ZB_NWK_FRAMECTL_SET_SRC_DEST_IEEE(nwhdr->frame_control, 1,
                                          have_long_dst);
...
/**
 */
void zb_panid_conflict_schedule_network_report(zb_uint8_t param,
                                               zb_uint16_t panid)
{
    if (ZG->nwk.handle.panid_conflict
        ...
        TRACE_MSG(TRACE_NWK1,
                  "panid conflict resolve is in progres, received new_panid 0x%x != conflicting panid 0x%x",
    ...
    if (!ZG->nwk.handle.panid_conflict) {
        TRACE_MSG(TRACE_NWK1,
                  "panid conflict detected, schedule panid conflict report",
                  (FMT__0));
        ZB_SCHEDULE_ALARM(zb_panid_conflict_send_network_report, param,
                          ZB_DEFAULT_SCAN_DURATION);
        ZG->nwk.handle.panid_conflict = 1;
        ZG->nwk.handle.new_panid = 0;
    }
...
void zb_panid_conflict_send_network_report(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (ZG->nwk.handle.panid_conflict
        && ZG->nwk.handle.new_panid != 0) {
        TRACE_MSG(TRACE_NWK1,
                  "Already got new panid - not need to send nwk report",
                  (FMT__0));
        zb_free_buf(ZB_BUF_FROM_REF(param));
        return;
    }
    ...
    rep =
        (zb_nwk_report_cmd_t *)zb_nwk_fill_out_command(
            param, ZG->nwk.nib.nwk_manager_addr, ZB_NWK_CMD_NETWORK_REPORT,
            sizeof(zb_nwk_report_cmd_t) + (i - 1) * sizeof(zb_uint16_t));
    p = &rep->panids[0];
    ...
    TRACE_MSG(TRACE_NWK2,
              "zb_panid_conflict_send_network_report param %hd %hd panids panid[0] = %hd",
              (FMT__H_H_D, param, i, p[0]));

    /* send request */
    ZB_SET_BUF_PARAM(ZB_BUF_FROM_REF(
                         param), ZB_NWK_INTERNAL_NSDU_HANDLE, zb_uint8_t);
...
void zb_panid_conflict_send_nwk_update(zb_uint8_t param) ZB_CALLBACK
{
    zb_nwk_update_cmd_t *upd =
        (zb_nwk_update_cmd_t *)zb_nwk_fill_out_command(
            param, ZB_NWK_BROADCAST_ALL_DEVICES, ZB_NWK_CMD_NETWORK_UPDATE,
            sizeof(zb_nwk_update_cmd_t));

    TRACE_MSG(TRACE_NWK1, ">> zb_panid_conflict_send_network_update param %hd",
              (FMT__H_D, param));
...
void zb_panid_conflict_send_status_ind(zb_uint8_t param) ZB_CALLBACK
{
    ...
    TRACE_MSG(TRACE_NWK1, ">> zb_panid_conflict_send_status_ind %hd",
              (FMT__H, param));
...
void zb_panid_conflict_network_update_recv(zb_nwk_update_cmd_t *upd)
{
    ...
    TRACE_MSG(TRACE_NWK1,
              "zb_panid_conflict_network_update_recv update_id %hd panid %d",
...
{
    zb_mlme_start_req_t *req = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                    param),
                                                zb_mlme_start_req_t);

    TRACE_MSG(TRACE_NWK1, "Change PANID to %d",
              (FMT__D, ZG->nwk.handle.new_panid));
