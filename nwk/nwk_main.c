...
static void zb_nwk_call_leave_ind(zb_uint8_t param, zb_uint8_t rejoin,
                                  zb_address_ieee_ref_t addr_ref);
static void zb_nwk_leave_handler(zb_uint8_t param, zb_nwk_hdr_t *nwk_hdr,
                                 zb_uint8_t lp) ZB_SDCC_REENTRANT;
#endif
...
zb_nwk_hdr_t *nwk_alloc_and_fill_hdr(zb_buf_t *buf,
                                     ...
                                     zb_uint8_t *src_ieee_addr,
                                     zb_uint8_t *dst_ieee_addr,
                                     ...
                                     zb_bool_t is_cmd_frame) ZB_SDCC_REENTRANT
{
    ...
    zb_ushort_t hdr_size =
        (src_ieee_addr && dst_ieee_addr) ? ZB_NWK_FULL_HDR_SIZE(is_multicast)
                                         : ((src_ieee_addr ||
                                             dst_ieee_addr) ?
                                            ZB_NWK_HALF_HDR_SIZE(is_multicast) :
                                            ZB_NWK_SHORT_HDR_SIZE(
                                                is_multicast));
...
zb_uint8_t *nwk_alloc_and_fill_cmd(zb_buf_t *buf, zb_nwk_cmd_t cmd,
                                   zb_uint8_t cmd_size)
{
    zb_uint8_t *ptr = NULL;
 ...
 */
static zb_ret_t nwk_calc_destination(zb_buf_t *nsdu, zb_uint8_t handle,
                                     zb_uint16_t *mac_dst, zb_short_t *indirect,
                                     zb_nwk_command_status_t *cmd_status)
ZB_SDCC_REENTRANT
{
    zb_ret_t ret = RET_OK;
    ...
    ZVUNUSED(cmd_status);
    TRACE_MSG(TRACE_NWK1,
              ">>calc_dest nsdu %p handle %hd mac_dst %p indir %p cmd_status %p",
              (FMT__P_H_P_P_P, nsdu, handle, mac_dst, indirect, cmd_status));
    ...
    *indirect = ZB_FALSE;
...
#endif
    {
        ...
        TRACE_MSG(TRACE_NWK1, "dest addr %d dev type %hd",
                  (FMT__D_H, nwhdr->dst_addr, ZG->nwk.nib.device_type));
        ...
        else if (((ZB_NWK_FRAMECTL_GET_DESTINATION_IEEE(nwhdr->frame_control) ==
                   0
                   &&  zb_address_by_short(nwhdr->dst_addr, ZB_FALSE, ZB_FALSE,
                                           &addr_ref) == RET_OK)
                  || ((ZB_NWK_FRAMECTL_GET_DESTINATION_IEEE(nwhdr->frame_control)
                       != 0
                       &&  zb_address_by_ieee(nwhdr->dst_ieee_addr, ZB_FALSE,
                                              ZB_FALSE, &addr_ref) == RET_OK)))
        ...
        /* is the destination in our routing table */
            ...
            TRACE_MSG(TRACE_NWK1, "found route for %hd, next hop %hd",
                      (FMT__H_H, nwhdr->dst_addr, route->next_hop_addr));
        ...
        /* discovery is in progress */
            ...
            if (ret == RET_OK) {
                ...
                zb_nwk_route_discovery_t *disc_entry =
                    zb_nwk_mesh_find_route_discovery_entry(nwhdr->dst_addr);
                ...
                if (!disc_entry
                    ...
                    TRACE_MSG(TRACE_NWK3, "already init r disc to %d",
                              (FMT__D, nwhdr->dst_addr));
                }
        ...
        /* are we allowed to discover the route? */
                 ...
                 && !route) {
            TRACE_MSG(TRACE_NWK2, "init r disc to %d",
                      (FMT__D, nwhdr->dst_addr));
 ...
 * is 0, but we have no such frames */
            ...
            TRACE_MSG(TRACE_NWK2, "route using tree r, to the neighb %p",
                      (FMT__P, nbh));
            if (nbh) {
                ...
                TRACE_MSG(TRACE_ERROR, "tree route fail, n.t. empty?",
                          (FMT__0));
                ret = RET_ERROR;
            ...
            TRACE_MSG(TRACE_ERROR, "Unable to route!, get_discovery_route %hd",
                      (FMT__H,
                       ZB_NWK_FRAMECTL_GET_DISCOVER_ROUTE(nwhdr->frame_control)));
            ret = RET_ERROR;
...
//   printf("nwk ent: 0x%lx\n", (uint32_t)ent);
    if (ent) {
        ...
        if (nent) {
            ...
            TRACE_MSG(TRACE_NWK2, "mark neighbor %hd as got ack from",
                      (FMT__H, ent->neighbor_table_iterator));
            ZB_SET_BIT_IN_BIT_VECTOR(ent->passive_ack,
                                     ent->neighbor_table_iterator);

            /* find next child */
            ent->neighbor_table_iterator = zb_nwk_neighbor_next_ze_children_i(
                ent->dst_addr, ent->neighbor_table_iterator + 1);
        }
        else {
        ...
        ZB_CALC_NONE_ZERO_BITS_IN_BIT_VECTOR(ent->passive_ack,
                                             ZB_NWK_BRCST_PASSIVE_ACK_ARRAY_SIZE,
                                             i);
        TRACE_MSG(TRACE_NWK2, "number of got passive acks %hd total %hd",
                  (FMT__H_H, i, passive_ack_device_num));
        ...
        if (ent->neighbor_table_iterator == (zb_ushort_t) ~0
            && (ent->retries >= ZB_NWK_MAX_BROADCAST_RETRIES
                ...
                || (passive_ack_device_num
                    ...
                    && ent->src_addr != ZB_PIB_SHORT_ADDRESS()))) {
            /* clear entrie */
            TRACE_MSG(TRACE_NWK2, "last retransmition for the entrie",
                      (FMT__0));
            ...
            param = ent->buf;
            NWK_ARRAY_PUT_ENT(ZG->nwk.handle.brrt, ent,
...
//       od_hex_dump(ZB_BUF_BEGIN(ZB_BUF_FROM_REF(ent->buf)), 32, 16);
             ...
             */
            {
                zb_mcps_data_req_params_t *data_req = ZB_GET_BUF_PARAM(buf,
                                                                       zb_mcps_data_req_params_t);
    ...
    if (ZG->nwk.handle.brrt_cnt) {
        if (ent) {
            ...
            zb_time_t next_execution = ZB_TIMER_GET() +
                                       ZB_NWK_MAX_BROADCAST_JITTER_INTERVAL;

            /* calc next execution time */
            for (i = 0; i < ZB_NWK_BRR_TABLE_SIZE; i++) {
                if (ZG->nwk.handle.brrt[i].used
                    && ZG->nwk.handle.brrt[i].retries <
                    ZB_NWK_MAX_BROADCAST_RETRIES) {
                    if (ZG->nwk.handle.brrt[i].next_retransmit <
                        next_execution) {
                        next_execution = ZG->nwk.handle.brrt[i].next_retransmit;
                    }
                }
            }
            TRACE_MSG(TRACE_NWK1, "schedule next retransmition at %d",
                      (FMT__D, next_execution));
            ZB_SCHEDULE_ALARM_CANCEL(nwk_broadcast_transmition, 0);
            ZB_SCHEDULE_ALARM(nwk_broadcast_transmition, 0,
                              next_execution - ZB_TIMER_GET());
        }
    }
...
//   od_hex_dump(packet, 32, 16);
    ...
    if (ret == RET_OK) {
        ...
        if (ZB_NWK_IS_ADDRESS_BROADCAST(nwhdr->dst_addr)) {
            ...
            NWK_ARRAY_FIND_ENT(ZG->nwk.handle.btt, ent,
                               (ent->source_addr == nwhdr->src_addr) &&
                               (ent->sequence_number == nwhdr->seq_num));
            if (!ent) {
                NWK_ARRAY_GET_ENT(ZG->nwk.handle.btt, ent,
                                  ZG->nwk.handle.btt_cnt);
                if (ent) {
                    ent->source_addr = nwhdr->src_addr;
                    ent->sequence_number = nwhdr->seq_num;
                    ent->expiration_time = ZB_NWK_BROADCAST_DELIVERY_TIME();
                    TRACE_MSG(TRACE_NWK3,
                              "btt add new ent addr %d seq %hd expire t %hd",
                              (FMT__D_H_H, ent->source_addr,
                               ent->sequence_number, ent->expiration_time));
        ...
        ZB_MCPS_BUILD_DATA_REQUEST(packet, ZB_PIB_SHORT_ADDRESS(), mac_dst,
                                   ((indirect ?
                                     MAC_TX_OPTION_INDIRECT_TRANSMISSION_BIT : 0)
                                    |
                                    /* if not broadcast, wants ack */
                                    ((mac_dst !=
                                      0xffff) ? MAC_TX_OPTION_ACKNOWLEDGED_BIT :
                                     0x0)),
                                   handle);
        ...
        if (ZB_NWK_IS_ADDRESS_BROADCAST(nwhdr->dst_addr)
            ...
            if (ZG->nwk.handle.brrt_cnt < ZB_NWK_BRR_TABLE_SIZE) {
                ...
                NWK_ARRAY_GET_ENT(ZG->nwk.handle.brrt, ent,
                                  ZG->nwk.handle.brrt_cnt);
                ZB_ASSERT(ent);
                ...
                ent->neighbor_table_iterator =
                    zb_nwk_neighbor_next_ze_children_i(ent->dst_addr, 0);
                TRACE_MSG(TRACE_NWK3, "neighbor_table_iterator %hd",
                          (FMT__H, ent->neighbor_table_iterator));
    ...
    if (ret != RET_OK
        ...
        TRACE_MSG(TRACE_NWK1, "cant deliver packet %p err %d",
                  (FMT__P_D, packet, ret));
        ...
        if (zb_nwk_neighbor_get_by_short(mac_dst, &nbt) == RET_OK
            ...
            zb_nlme_status_indication_t *status =  ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                                        param),
...
void call_status_indication(zb_uint8_t param) ZB_CALLBACK
{
    zb_nlme_status_indication_t *status =
        ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(param), zb_nlme_status_indication_t);
    ...
    TRACE_MSG(TRACE_NWK1, "status %hd dest_addr %d",
...
void zb_mcps_data_confirm(zb_uint8_t param) ZB_CALLBACK
{
    ...
    zb_mcps_data_confirm_params_t *confirm = ZB_GET_BUF_PARAM(buf,
    ...
    else if (confirm->msdu_handle ==
             ZB_NWK_INTERNAL_LEAVE_IND_AT_DATA_CONFIRM_HANDLE) {
        TRACE_MSG(TRACE_NWK3, "schedule leave.indication", (FMT__0));
        zb_nwk_call_leave_ind(param, ZG->nwk.leave_context.rejoin_after_leave,
                              (zb_uint8_t)-1);
    }
    else
#endif
    {
        ...
        if (confirm->msdu_handle == ZB_NWK_INTERNAL_NSDU_HANDLE) {
            if (buf->u.hdr.status != MAC_SUCCESS) {
                ...
                TRACE_MSG(TRACE_NWK3,
                          "internal nwk transmission - send nwk status ind",
                          (FMT__0));
                call_status_indication(param);
            }
            else {
                TRACE_MSG(TRACE_NWK3,
                          "free confirm to internal nwk transmission",
                          (FMT__0));
                zb_free_buf(buf);
            }
        }
...
void new_buffer_allocated(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (param != 0
        ...
        zb_nwk_buf_alloc_wait_t *ent =
            &ZG->nwk.handle.wait_alloc[ZG->nwk.handle.wait_alloc_cnt - 1];
        ...
        switch ((zb_nwk_wait_reason_t)ent->wait) {
            case ZB_NWK_WAIT_REASON_IND_AND_RETRANSMIT:
            {
                 ...
                 */
#ifdef ZB_SECURITY
                TRACE_MSG(TRACE_SECUR3, "will use same key for secure",
                          (FMT__0));
                ...
                ZB_SET_BUF_PARAM(ZB_BUF_FROM_REF(
                                     ent->buf), ZB_NWK_INTERNAL_NSDU_HANDLE,
                                 zb_uint8_t);
        ...
        NWK_ARRAY_PUT_ENT(ZG->nwk.handle.wait_alloc, ent,
                          ZG->nwk.handle.wait_alloc_cnt);
...
void zb_mcps_data_indication(zb_uint8_t param) ZB_CALLBACK
{
    ...
    TRACE_MSG(TRACE_NWK2, ">>zb_mcps_data_indication param %hd",
              (FMT__H, param));
...
//   od_hex_dump(mac_hdr.src_addr.addr_long, sizeof(zb_ieee_addr_t), 16);
    ...
    if (ZB_NWK_IS_ADDRESS_BROADCAST(dst_addr)) {
        ...
        if (!(dst_addr == ZB_NWK_BROADCAST_ALL_DEVICES
            ...
            TRACE_MSG(TRACE_NWK1, "unsupported broadc pkt 0x%x - drop",
                      (FMT__D, dst_addr));
            zb_free_buf(buf);
            goto done;
        }

        {
            ...
            if (!ZG->nwk.handle.joined_pro) {
                ...
                NWK_ARRAY_FIND_ENT(ZG->nwk.handle.brrt, brrt_ent,
                                   (brrt_ent->src_addr == src_addr) &&
                                   (brrt_ent->seq_num == nwk_hdr->seq_num));
                if (brrt_ent
                    && ZB_FCF_GET_SRC_ADDRESSING_MODE(mac_hdr.frame_control) ==
                    ZB_ADDR_16BIT_DEV_OR_BROADCAST
                    && zb_nwk_neighbor_get_by_short(mac_hdr.src_addr.addr_short,
                                                    &nbt) == RET_OK) {
                    ...
                    TRACE_MSG(TRACE_NWK3,
                              ...
                              (FMT__D_D_H, mac_hdr.src_addr.addr_short,
                               src_addr,
                               nwk_hdr->seq_num));
                    ZB_SET_BIT_IN_BIT_VECTOR(brrt_ent->passive_ack, ZB_NWK_NEIGHBOR_GET_INDEX_BY_ADDRESS(
                                                 nbt));

                    ZB_NWK_GET_PASSIVE_ACK_DEV_NUM(passive_ack_dev_num);

                    ZB_CALC_NONE_ZERO_BITS_IN_BIT_VECTOR(brrt_ent->passive_ack,
                                                         ZB_NWK_BRCST_PASSIVE_ACK_ARRAY_SIZE,
                                                         n);
                    TRACE_MSG(TRACE_NWK2,
                              "number of got passive acks %hd total %hd",
                              (FMT__H_H, n, passive_ack_dev_num));
                    if (
                        /* if this packet is from us, we should have exactly neighbor size
                         * number acks */
                        ...
                        /* clear entrie */
                        TRACE_MSG(TRACE_NWK2,
                                  "got all passive acks, drop retransmition entrie, call confirm",
                                  (FMT__0));
                        /* Not need to transmit broadcast more. Must call confirm now. */
                        ZB_SCHEDULE_CALLBACK(zb_nwk_call_br_confirm,
                                             brrt_ent->buf);
                        NWK_ARRAY_PUT_ENT(ZG->nwk.handle.brrt, brrt_ent,
                                          ZG->nwk.handle.brrt_cnt);
                    }
                }
            }
            ...
            if (ent) {
                TRACE_MSG(TRACE_NWK1,
                          "brc from %d to 0x%x seq %hd already proceed - drop",
                          (FMT__D_H, src_addr, dst_addr, nwk_hdr->seq_num));
                zb_free_buf(buf);
                goto done;
            }
            else {
                ...
                if (ent) {
                    ...
                    ent->expiration_time = ZB_NWK_BROADCAST_DELIVERY_TIME();
                    TRACE_MSG(TRACE_NWK3,
                              "btt add ent addr %d seq %hd exp t %hd",
                              (FMT__D_H_H,
                               ent
                               ->source_addr, ent->sequence_number,
                               ent->expiration_time));
    ...
    TRACE_MSG(TRACE_NWK3, "new radius %hd, frame type %hd",
              (FMT__H_H, nwk_hdr->radius,
               ZB_NWK_FRAMECTL_GET_FRAME_TYPE(nwk_hdr->frame_control)));
...
#endif
            nbt->lqi = lqi;
            TRACE_MSG(TRACE_NWK3, "update lqi for %d to %hd",
                      (FMT__D_H, nwk_hdr->src_addr, lqi));
        }
    }
...
//     printf("nwk indicate %i retransmit %i\n", indicate, retransmit);
    if (retransmit
        ...
        NWK_ARRAY_GET_ENT(ZG->nwk.handle.wait_alloc, alloc_wait,
                          ZG->nwk.handle.wait_alloc_cnt);
        if (alloc_wait) {
...
void zb_nwk_call_br_confirm(zb_uint8_t param) ZB_CALLBACK
{
    ...
    ZB_FCF_SET_DST_ADDRESSING_MODE(mhr.frame_control,
                                   ZB_ADDR_16BIT_DEV_OR_BROADCAST);
    ZB_FCF_SET_SRC_ADDRESSING_MODE(mhr.frame_control,
                                   ZB_ADDR_16BIT_DEV_OR_BROADCAST);
...
void nwk_frame_indication(zb_uint8_t param) ZB_CALLBACK
{
    ...
    zb_uint8_t frame_type = ZB_NWK_FRAMECTL_GET_FRAME_TYPE(
        nwk_hdr->frame_control);
    ...
    if (frame_type == ZB_NWK_FRAME_TYPE_DATA ||
        ...
        if (command_id == ZB_NWK_CMD_LEAVE) {
            ...
            zb_uint8_t *leave_pl =
                (zb_uint8_t *)ZB_NWK_CMD_FRAME_GET_CMD_PAYLOAD(buf, hdr_size);
        ...
        if (0) {}
...
#ifdef ZB_ROUTER_ROLE
#ifdef ZB_NWK_MESH_ROUTING
        else if ((command_id == ZB_NWK_CMD_ROUTE_REQUEST) &&
                 (!ZG->nwk.handle.joined_pro)) {
            zb_nwk_cmd_rreq_t *rreq =
                (zb_nwk_cmd_rreq_t *)ZB_NWK_CMD_FRAME_GET_CMD_PAYLOAD(buf,
                                                                      hdr_size);
            ...
            zb_nwk_cmd_rrep_t *rrep =
                (zb_nwk_cmd_rrep_t *)ZB_NWK_CMD_FRAME_GET_CMD_PAYLOAD(buf,
                                                                      hdr_size);
            ...
            zb_nwk_report_cmd_t *report =
                (zb_nwk_report_cmd_t *)ZB_NWK_CMD_FRAME_GET_CMD_PAYLOAD(buf,
                                                                        hdr_size);
            ...
            if (!ZB_NWK_REPORT_IS_PANID_CONFLICT(report->command_options)
                ...
                TRACE_MSG(TRACE_ERROR, "drop report cmd %hd",
                          (FMT__H,
                           ZB_NWK_REPORT_COMMAND_ID(report->command_options)));
                zb_free_buf(buf);
            }
            else {
                zb_panid_conflict_got_network_report(param, report->panids, ZB_NWK_REPORT_INFO_COUNT(
                                                         report->command_options));
            }
        }
        else if (command_id == ZB_NWK_CMD_NETWORK_STATUS) {
            zb_nlme_status_indication_t *cmd =
                (zb_nlme_status_indication_t *)ZB_NWK_CMD_FRAME_GET_CMD_PAYLOAD(
                    buf,
                    hdr_size);
            ...
            zb_nwk_update_cmd_t *upd =
                (zb_nwk_update_cmd_t *)ZB_NWK_CMD_FRAME_GET_CMD_PAYLOAD(buf,
                                                                        hdr_size);
            if (!ZB_NWK_REPORT_IS_PANID_CONFLICT(upd->command_options)
                || !ZB_EXTPANID_CMP(upd->epid, ZB_NIB_EXT_PAN_ID())) {
                TRACE_MSG(TRACE_ERROR, "drop nwk update cmd %hd",
                          (FMT__H,
                           ZB_NWK_REPORT_COMMAND_ID(upd->command_options)));
                zb_free_buf(buf);
            }
            else {
        ...
        for (i = 0; i < ZB_NWK_BTR_TABLE_SIZE; i++) {
            if (ZG->nwk.handle.btt[i].used) {
                if (ZG->nwk.handle.btt[i].expiration_time) {
                    ...
                    NWK_ARRAY_PUT_ENT(ZG->nwk.handle.btt,
                                      &ZG->nwk.handle.btt[i],
                                      ZG->nwk.handle.btt_cnt);
                }
            }
                                 ...
                                 zb_uint8_t lp) ZB_SDCC_REENTRANT
{
    ...
    TRACE_MSG(TRACE_NWK3,
              "+zb_nwk_leave_handler from %d request %hd remove_ch %hd rejoin %hd",
              (FMT__D_H_H_H, nwk_hdr->src_addr,
               ZB_LEAVE_PL_GET_REQUEST(lp), ZB_LEAVE_PL_GET_REMOVE_CHILDREN(lp),
               ZB_LEAVE_PL_GET_REJOIN(lp)));

    status = zb_address_by_short(nwk_hdr->src_addr, ZB_FALSE, ZB_FALSE,
                                 &addr_ref);
    if (status == RET_OK) {
        if (zb_nwk_neighbor_get(addr_ref, ZB_FALSE, &nbt) != RET_OK) {
#ifdef ZB_ROUTER_ROLE
            TRACE_MSG(TRACE_NWK2, "incoming LEAVE not from the neighbor",
                      (FMT__0));
    ...
    if (status == RET_OK
        ...
        /* drop */
        TRACE_MSG(TRACE_NWK1, "REQUEST not from the parent or to ED - drop",
                  (FMT__0));
        status = RET_ERROR;
    }
    if (status == RET_OK) {
        if (nbt->relationship == ZB_NWK_RELATIONSHIP_PARENT) {
            ...
            if (!ZB_LEAVE_PL_GET_REQUEST(lp)) {
                ...
                ZG->nwk.leave_context.leave_ind_prnt.rejoin =
                    ZB_LEAVE_PL_GET_REJOIN(lp);
...
#ifdef ZB_ROUTER_ROLE
            if ((ZB_LEAVE_PL_GET_REQUEST(lp) ||
                 ZB_LEAVE_PL_GET_REMOVE_CHILDREN(lp))
                && ZB_NIB_DEVICE_TYPE() != (zb_uint8_t)ZB_NWK_DEVICE_TYPE_ED) {
                /* must remove ourself */
...
#endif
                new_lp = (zb_uint8_t *)nwk_alloc_and_fill_cmd(packet,
                                                              ZB_NWK_CMD_LEAVE,
                                                              sizeof(zb_uint8_t));
                *new_lp = 0;
                ZB_LEAVE_PL_SET_REJOIN(*new_lp, ZB_LEAVE_PL_GET_REJOIN(lp));
                ZB_LEAVE_PL_SET_REMOVE_CHILDREN(*new_lp, ZB_LEAVE_PL_GET_REMOVE_CHILDREN(
                                                    lp));
                /* When will got data.confirm at this request, do actual leave */
                ZG->nwk.leave_context.rejoin_after_leave =
                    ZB_LEAVE_PL_GET_REJOIN(lp);
                ZB_SET_BUF_PARAM(packet,
                                 ZB_NWK_INTERNAL_LEAVE_IND_AT_DATA_CONFIRM_HANDLE,
                                 zb_uint8_t);
                TRACE_MSG(TRACE_NWK2,
                          "send LEAVE with request 0, rejoin %hd, remove_children %hd",
                          (FMT__H_H, ZB_LEAVE_PL_GET_REJOIN(lp),
                           ZB_LEAVE_PL_GET_REMOVE_CHILDREN(lp)));
...
#endif
            {
                ...
                zb_nwk_call_leave_ind(param, ZB_LEAVE_PL_GET_REJOIN(lp),
                                      (zb_address_ieee_ref_t)-1);
            }
#ifdef ZB_ROUTER_ROLE
            else {
                /* forget our parent (hmm?) */
                TRACE_MSG(TRACE_NWK2, "our parent has left - hmm?", (FMT__0));
                zb_nwk_call_leave_ind(param, ZB_LEAVE_PL_GET_REJOIN(
                                          lp), addr_ref);
            }
#endif
        }
...
static void zb_nwk_call_leave_ind(zb_uint8_t param, zb_uint8_t rejoin,
                                  zb_address_ieee_ref_t addr_ref)
{
    zb_nlme_leave_indication_t *request = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                               param),
...
void zb_nwk_do_leave(zb_uint8_t param, zb_uint8_t rejoin) ZB_SDCC_REENTRANT
{
    ...
    ZB_BUF_REUSE(buf);
    if (rejoin) {
        zb_nlme_join_request_t *req = ZB_GET_BUF_PARAM(buf,
        ...
        zb_address_ieee_ref_t addr_ref;
        if (zb_address_by_short(ZB_NIB_NETWORK_ADDRESS(), ZB_FALSE, ZB_FALSE,
                                &addr_ref) == RET_OK) {
...
#ifdef ZB_ROUTER_ROLE
    if (device_type == ZB_NWK_DEVICE_TYPE_ROUTER ||
        device_type == ZB_NWK_DEVICE_TYPE_COORDINATOR) {
...
zb_ushort_t zb_nwk_hdr_size(zb_uint8_t *fctl)
{
    ...
    return (
        ...
        +
        (ZB_NWK_FRAMECTL_GET_DESTINATION_IEEE(fctl) +
         ZB_NWK_FRAMECTL_GET_SOURCE_IEEE(fctl)) * sizeof(zb_ieee_addr_t) +
