 ...
 */
static zb_ret_t zb_nwk_mesh_send_rreq(zb_buf_t *cbuf,
                                      zb_nwk_cmd_rreq_t *nwk_cmd_rreq,
                                      zb_uint16_t src_addr, zb_uint8_t seq_num,
                                      ...
                                      zb_uint8_t radius) ZB_SDCC_REENTRANT
{
    ...
    TRACE_MSG(TRACE_NWK1,
              ">> send_rreq cbuf %p rreq %p s_addr %d path_cost %hd radius %hd",
              (FMT__P_P_D_H_H,
               cbuf,
               nwk_cmd_rreq, src_addr, path_cost, radius));
...
#endif
    {
        ...
        nwhdr = nwk_alloc_and_fill_hdr(cbuf,
                                       ...
                                       NULL, NULL, ZB_FALSE, secure, ZB_TRUE);
        rreq_cmd = (zb_nwk_cmd_rreq_t *)nwk_alloc_and_fill_cmd(cbuf,
                                                               ZB_NWK_CMD_ROUTE_REQUEST,
                                                               sizeof(
                                                                   zb_nwk_cmd_rreq_t));
        rreq_cmd->opt = 0;
        ...
        if (rreq) {
            ...
            if (ZG->nwk.nib.rreq_cnt == 1) {
                ZB_SCHEDULE_ALARM_CANCEL(zb_nwk_mesh_expiry_rreq,
                                         ZB_ALARM_ANY_PARAM);
                ZB_SCHEDULE_ALARM(zb_nwk_mesh_expiry_rreq, -1,
                                  ZB_NWK_RREQ_RETRY_INTERVAL);
            }
        }
#endif
 ...
 */
static zb_ret_t zb_nwk_mesh_send_rrep(zb_buf_t *cbuf, zb_uint8_t rreq_id,
                                      zb_uint16_t originator,
                                      zb_uint16_t responder,
                                      zb_uint8_t path_cost,
                                      zb_uint16_t sender_addr) ZB_SDCC_REENTRANT
{
    zb_ret_t ret = RET_OK;
    zb_nwk_hdr_t *nwhdr;
...
#endif
    TRACE_MSG(TRACE_NWK1,
              ">> send_rrep cbuf %p rreq_id %d orig %d resp %d p_cost %hd s_addr %d",
              (FMT__P_D_D_D_H_D,
               cbuf,
               rreq_id, originator, responder, path_cost, sender_addr));
    nwhdr = nwk_alloc_and_fill_hdr(cbuf, sender_addr, NULL, NULL, ZB_FALSE,
                                   secure, ZB_TRUE);
    rrep = (zb_nwk_cmd_rrep_t *)nwk_alloc_and_fill_cmd(cbuf,
                                                       ZB_NWK_CMD_ROUTE_REPLY,
                                                       sizeof(zb_nwk_cmd_rrep_t));
    rrep->opt = 0;
                                 ...
                                 zb_uint8_t radius) ZB_SDCC_REENTRANT
{
    ...
    TRACE_MSG(TRACE_NWK1, ">> route_discovery cbuf %p dest_addr %d radius %hd",
              (FMT__P_D_H, cbuf, dest_addr, radius));
    ...
    if (cbuf) {
        ...
        nwk_hdr->radius =
            radius ? radius : (zb_uint8_t)(ZG->nwk.nib.max_depth << 1);
                              ...
                              zb_nwk_cmd_rreq_t *nwk_cmd_rreq) ZB_SDCC_REENTRANT
{
    ...
    TRACE_MSG(TRACE_NWK1, ">> rreq_handler buf %p hdr %p cmd %p",
              (FMT__P_P_P, buf, nwk_hdr, nwk_cmd_rreq));
    ...
    if (routing_ent) {
        TRACE_MSG(TRACE_NWK1, "r ent fnd, state %d",
                  (FMT__D, routing_ent->status));
        ...
        NWK_ARRAY_GET_ENT(ZG->nwk.nib.routing_table, routing_ent,
                          ZG->nwk.nib.routing_table_cnt);
        if (routing_ent) {
            routing_ent->dest_addr = nwk_cmd_rreq->dest_addr;
            routing_ent->status = ZB_NWK_ROUTE_STATE_DISCOVERY_UNDERWAY;
            TRACE_MSG(TRACE_NWK1, "new routing ent : dest_addr %d status %hd",
                      (FMT__D_H,
                       routing_ent
                       ->dest_addr, routing_ent->status));
        }
        else {
            ret = RET_NO_MEMORY;
    ...
    TRACE_MSG(TRACE_NWK1, "rreq_id %hd src_addr %d",
              (FMT__H_D, nwk_cmd_rreq->rreq_id, nwk_hdr->src_addr));
    ...
    if (disc_ent) {
        TRACE_MSG(TRACE_NWK1, "dsc entry found, path_cost %hd forward_cost %hd",
                  (FMT__H_H, path_cost, disc_ent->forward_cost));
        ...
        if (path_cost < disc_ent->forward_cost) {
            ...
            TRACE_MSG(TRACE_NWK1, "new forward cost %hd node %d old %d",
                      (FMT__H_D_D, path_cost, src_addr, disc_ent->sender_addr));
        ...
        NWK_ARRAY_GET_ENT(ZG->nwk.nib.route_disc_table, disc_ent,
        ...
        if (disc_ent) {
            ...
            TRACE_MSG(TRACE_NWK1,
                      "disc ent added: request_id %hd src_addr %d sndr_addr %d d_addr %d fwd_cost %hd res_cost %hd exp_time %d",
                      (FMT__H_D_D_D_H_H_D,
                       disc_ent
                       ->request_id, disc_ent->source_addr,
                       disc_ent->sender_addr,
                       disc_ent->dest_addr,
                       disc_ent
                       ->forward_cost, disc_ent->residual_cost,
                       disc_ent->expiration_time));
        }
        else {
            ret = RET_NO_MEMORY;
     ...
     * case, forward route request otherwise */
    {
        ...
        if (nwk_cmd_rreq->dest_addr == ZB_NIB_NETWORK_ADDRESS()
            || zb_nwk_neighbor_get_by_short(nwk_cmd_rreq->dest_addr,
                                            &nbt) == RET_OK) {
            zb_uint16_t src_addr;
            nwk_get_mac_source_addr(buf, &src_addr);

            TRACE_MSG(TRACE_ERROR, "this rreq f/us or neighbor, snd rrep",
                      (FMT__0));
            zb_nwk_mesh_send_rrep(buf, nwk_cmd_rreq->rreq_id, nwk_hdr->src_addr,
                                  nwk_cmd_rreq->dest_addr, path_cost, src_addr);
            ...
            ZB_MEMCPY(&nwk_cmd_rreq_save, nwk_cmd_rreq,
                      sizeof(nwk_cmd_rreq_save));

            TRACE_MSG(TRACE_ERROR, "don't know rreq dest, prop it", (FMT__0));
            zb_nwk_mesh_send_rreq(buf, &nwk_cmd_rreq_save, nwk_hdr->src_addr,
                                  nwk_hdr->seq_num, path_cost, nwk_hdr->radius);
    ...
    if (ret != RET_OK) {
        TRACE_MSG(TRACE_NWK1, "error %d above, rm disc and r ent if",
                  (FMT__D, ret));
        if (disc_ent) {
            NWK_ARRAY_PUT_ENT(ZG->nwk.nib.route_disc_table, disc_ent,
                              ZG->nwk.nib.route_disc_table_cnt);
        }
        if (routing_ent) {
            NWK_ARRAY_PUT_ENT(ZG->nwk.nib.routing_table, routing_ent,
                              ZG->nwk.nib.routing_table_cnt);
        }
                              ...
                              zb_nwk_cmd_rrep_t *nwk_cmd_rrep) ZB_SDCC_REENTRANT
{
    ...
    TRACE_MSG(TRACE_NWK1, ">> rrep_handler buf %p nwk_hdr %p nwk_cmd_rrep %p",
              (FMT__P_P_P, buf, nwk_hdr, nwk_cmd_rrep));
    ...
    NWK_ARRAY_FIND_ENT( ZG->nwk.nib.route_disc_table, disc_ent,
                        (disc_ent->request_id == nwk_cmd_rrep->rreq_id) &&
                        (disc_ent->source_addr == nwk_cmd_rrep->originator));
    NWK_ARRAY_FIND_ENT( ZG->nwk.nib.routing_table, routing_ent,
                        (routing_ent->dest_addr == nwk_cmd_rrep->responder));
    TRACE_MSG(TRACE_NWK1, "p_cost %d disc_ent %p r_ent %p residual_cost %hd",
              (FMT__D_P_P_H, \
               path_cost,
               disc_ent, routing_ent, disc_ent ? disc_ent->residual_cost : 0));

    /* check response is meaningful for us */
    if (!disc_ent
        || !routing_ent
        || path_cost > disc_ent->residual_cost) {
        TRACE_MSG(TRACE_NWK1, "drop rresp, no purpose ent fnd or path too long",
                  (FMT__0));
        if (!disc_ent || !routing_ent) {
            TRACE_MSG(TRACE_NWK1, "one of disc or r ent is absent, free other",
                      (FMT__0));
            if (disc_ent) {
                NWK_ARRAY_PUT_ENT(ZG->nwk.nib.route_disc_table, disc_ent,
                                  ZG->nwk.nib.route_disc_table_cnt);
    ...
    if (nwk_cmd_rrep->originator == ZB_NIB_NETWORK_ADDRESS()) {
        zb_ushort_t i;
        TRACE_MSG(TRACE_NWK1, "got rrepl for our rreq, snd wait pckts",
                  (FMT__0));
        ...
        for (i = 0; i < ZB_NWK_PENDING_TABLE_SIZE; i++) {
            if (ZG->nwk.nib.pending_table[i].used
                && ZG->nwk.nib.pending_table[i].dest_addr ==
                routing_ent->dest_addr) {
                TRACE_MSG(TRACE_NWK1, "fnd pkt to %d addr, send it",
                          (FMT__D, routing_ent->dest_addr));
                ZB_SET_BUF_PARAM(ZG->nwk.nib.pending_table[i].buf,
                                 ZG->nwk.nib.pending_table[i].handle,
                                 zb_uint8_t);
                ZB_SCHEDULE_CALLBACK(zb_nwk_forward,
                                     ZB_REF_FROM_BUF(ZG->nwk.nib.pending_table[i
                                                     ].buf));
                NWK_ARRAY_PUT_ENT(ZG->nwk.nib.pending_table,
                                  &ZG->nwk.nib.pending_table[i],
                                  ZG->nwk.nib.pending_table_cnt);
            }
        }

        /* if the request was initiated by APS, confirm that route discovery failed */
        TRACE_MSG(TRACE_NWK1, "aps_rreq_addr %d  dst_addr %d",
                  (FMT__D_D, ZG->nwk.nib.aps_rreq_addr,
                   routing_ent->dest_addr));
        ...
        /* forward */
        TRACE_MSG(TRACE_NWK1, "frwd rrep to the %d address",
                  (FMT__D, disc_ent->sender_addr));
        zb_nwk_mesh_send_rrep(buf, nwk_cmd_rrep->rreq_id,
                              nwk_cmd_rrep->originator, nwk_cmd_rrep->responder,
                              path_cost, disc_ent->sender_addr);
...
static void zb_nwk_mesh_resend_rreq(zb_buf_t *cbuf, zb_nwk_rreq_t *rreq)
{
    ...
    TRACE_MSG(TRACE_NWK1, ">> resend_rreq cbuf %p rreq %p",
              (FMT__P_P, cbuf, rreq));
    ...
    ZB_NWK_ALLOC_COMMAND_GET_PAYLOAD_PTR(cbuf, ZB_NWK_CMD_ROUTE_REQUEST,
                                         zb_nwk_cmd_rreq_t, rreq_cmd);
    ...
    total =
        (rreq->originator ==
         ZB_NIB_NETWORK_ADDRESS()) ?  ZB_MWK_INITIAL_RREQ_RETRIES :
        ZB_MWK_RREQ_RETRIES;
    TRACE_MSG(TRACE_NWK1, "sent %d times of %d total",
              (FMT__D_D, rreq->retries, total + 1));
    ...
    if (ZG->nwk.nib.rreq_cnt) {
        ...
        for (i = ZG->nwk.nib.rreq_num; i < ZB_NWK_RREQ_TABLE_SIZE; i++) {
            if (ZG->nwk.nib.rreq[i].used) {
                ...
                zb_nwk_mesh_resend_rreq(ZB_BUF_FROM_REF(
                                            param), &ZG->nwk.nib.rreq[i]);
                param = -1;
            }
        }
...
void nwk_route_disc_failed(zb_uint8_t param) ZB_CALLBACK
{
    ...
    ZB_SCHEDULE_ALARM(zb_nwk_mesh_expiry_route_disc, 0,
                      ZB_NWK_EXPIRY_ROUTE_DISCOVERY);
...
void zb_nwk_mesh_expiry_route_disc(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (ZG->nwk.nib.route_disc_table_cnt) {
        ...
        TRACE_MSG(TRACE_NWK1, "disc tbl ent cnt %d",
                  (FMT__D, ZG->nwk.nib.route_disc_table_cnt));
        for (i = 0; i < ZB_NWK_ROUTE_DISCOVERY_TABLE_SIZE; i++) {
            if (ZG->nwk.nib.route_disc_table[i].used) {
                TRACE_MSG(TRACE_NWK1, "ent %d exp_time  %d",
                          (FMT__D_D, i,
                ...
                if (ZG->nwk.nib.route_disc_table[i].expiration_time) {
                    ...
                    NWK_ARRAY_FIND_ENT(ZG->nwk.nib.routing_table, route_ent,
                                       (route_ent->dest_addr ==
                                        ZG->nwk.nib.route_disc_table[i].
                                        dest_addr)
                                       && (route_ent->status !=
                                           ZB_NWK_ROUTE_STATE_ACTIVE));

                    /* free expired route discovery entry */
                    TRACE_MSG(TRACE_NWK1, "free disc ent %d dst_addr %d",
                              (FMT__D_D, i,
                               ZG->nwk.nib.route_disc_table[i].dest_addr));
                    NWK_ARRAY_PUT_ENT(ZG->nwk.nib.route_disc_table,
                                      &ZG->nwk.nib.route_disc_table[i],
                                      ZG->nwk.nib.route_disc_table_cnt);
                    ...
                    if (route_ent) {
                        ...
                        TRACE_MSG(TRACE_NWK1, "free routing entrie, addr %d",
                                  (FMT__D, route_ent->dest_addr));
                        NWK_ARRAY_PUT_ENT(ZG->nwk.nib.routing_table, route_ent,
                                          ZG->nwk.nib.routing_table_cnt);
                        goto done;
                    }
                }
...
void zb_nwk_mesh_expiry_pending(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (ZG->nwk.nib.pending_table_cnt) {
        ...
        for (i = 0; i < ZB_NWK_PENDING_TABLE_SIZE; i++) {
            if (ZG->nwk.nib.pending_table[i].used) {
                TRACE_MSG(TRACE_NWK1, "pend ent %d dst_addr %d exp %d",
                          (FMT__D_D_D,
                           i,
                           ZG->nwk.nib.pending_table[i].dest_addr,
                           ZG->nwk.nib.pending_table[i].expiry));
                if (ZG->nwk.nib.pending_table[i].expiry == 0) {
                    TRACE_MSG(TRACE_NWK1, "free buf %p",
                              (FMT__P, ZG->nwk.nib.pending_table[i].buf));
...
ZB_SDCC_REENTRANT
{
    ...
    NWK_ARRAY_FIND_ENT(ZG->nwk.nib.routing_table, route,
                                        ...
                                        zb_uint8_t handle) ZB_SDCC_REENTRANT
{
    ...
    NWK_ARRAY_GET_ENT(ZG->nwk.nib.pending_table, pend,
                      ZG->nwk.nib.pending_table_cnt);
    if (pend) {
