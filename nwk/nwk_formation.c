...
void zb_mlme_start_confirm(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (buf->u.hdr.status != 0) {
        TRACE_MSG(TRACE_ERROR, "mlme_start FAILED %hd",
        ...
        switch (ZG->nwk.handle.state) {
            case ZB_NLME_STATE_ROUTER:
                ...
                TRACE_MSG(TRACE_NWK1,
                          "max_children %hd, zr# %hd, ze# %hd, mac_association_permit %hd",
                          (FMT__H_H,
                           ZG
                           ->nwk.nib.max_children, ZG->nwk.nib.router_child_num,
            ...
            default:
                TRACE_MSG(TRACE_NWK1, "wrong nwk ste %hd",
