...
static void nwk_join_failure_confirm(zb_uint8_t param,
                                     zb_uint8_t s) ZB_SDCC_REENTRANT
{
    zb_buf_t ZB_SDCC_XDATA *buf = (zb_buf_t *)ZB_BUF_FROM_REF(param);
    zb_nlme_join_confirm_t *join_confirm = ZB_GET_BUF_PARAM(buf,
                                                            zb_nlme_join_confirm_t);

    join_confirm->status = (zb_mac_status_t)(s);
    buf->u.hdr.status = s;
    join_confirm->network_address =
        (s == MAC_SUCCESS) ? ZB_NIB_NETWORK_ADDRESS() : 0xffff;
    ZB_EXTPANID_COPY(join_confirm->extended_pan_id, ZB_NIB_EXT_PAN_ID());
    ZB_SCHEDULE_CALLBACK(zb_nlme_join_confirm, param);
}

static zb_ext_neighbor_tbl_ent_t *nwk_choose_parent(
    zb_address_pan_id_ref_t panid_ref,
    zb_mac_capability_info_t capability_information) ZB_SDCC_REENTRANT
{
    ...
    TRACE_MSG(TRACE_NWK1,
              ">>nwk_choose_parent panid_ref %hd cap %hx nib_upd_id %hd",
              (FMT__H_H_H, panid_ref, capability_information,
               ZB_NIB_UPDATE_ID()));
    ...
    for (i = 0; i < ZG->nwk.neighbor.ext_neighbor_used; ++i) {
        ...
        TRACE_MSG(TRACE_NWK2,
                  ...
                  (FMT__P_H_H_H_H_H_H_H, ne, panid_ref,
                   ...
                   (zb_uint8_t)ne->router_capacity,
                   (zb_uint8_t)ne->end_device_capacity, (zb_uint8_t)ne->lqi,
        ...
        if (ne->panid_ref == panid_ref
            ...
            && ((ZB_MAC_CAP_GET_DEVICE_TYPE(capability_information) &&
                 (ne->stack_profile == 1)) ?
            ...
            if (ret
                ...
                TRACE_MSG(TRACE_NWK2,
                          "best_par %d %p ne %p dep %d / %d, lqi %hd / %hd - skip",
                          (FMT__D_P_P_D_D_H_H, ret, ne, ne->depth, ret->depth,
                           ret->lqi,
                           ne->lqi));
                continue;
            }
            ...
            TRACE_MSG(TRACE_NWK2,
                      "ne %p is not potential parent: panid_ok %hd potent_prnt_ok %hd permit_j_ok %hd cap_ok %hd lqi_ok %hd upd_id_ok %hd",
                      (FMT__P_H_H_H_H_H_H,
                       ne,
                       (zb_uint8_t)(ne->panid_ref == panid_ref),
                       (zb_uint8_t)ne->potential_parent,
                       (zb_uint8_t)ne->permit_joining,
                       (zb_uint8_t)(ZB_MAC_CAP_GET_DEVICE_TYPE(
                                        capability_information) ?
                                    (zb_uint8_t)ne->router_capacity : (
                                        zb_uint8_t)ne->end_device_capacity),
                       (zb_uint8_t)ZB_LINK_QUALITY_IS_OK_FOR_JOIN(ne->lqi),
                       (zb_uint8_t)ZB_NWK_UPDATE_ID1_GE_ID2(ne->update_id,
                                                            ZB_NIB_UPDATE_ID())));
        }
    } /* for */
 ...
 */
static zb_ret_t nwk_association_join(zb_buf_t *buf,
                                     zb_nlme_join_request_t *request)
ZB_SDCC_REENTRANT
{
    ...
    TRACE_MSG(TRACE_NWK1, ">>assoc_join buf %p req %p",
              (FMT__P_P, buf, request));
    ...
    else if (zb_address_get_pan_id_ref(request->extended_pan_id,
                                       &panid_ref) != RET_OK) {
    ...
    if (ret == ZB_NWK_STATUS_SUCCESS) {
        best_parent = nwk_choose_parent(panid_ref,
    ...
    if (best_parent) {
        ...
        if (!ZB_ADDRESS_COMPRESED_IS_ZERO(best_parent->long_addr)) {
            ...
            TRACE_MSG(TRACE_NWK2,
                      "Will assoc to pan " TRACE_FORMAT_64 "/%d, dev " TRACE_FORMAT_64 " %p",
                      (FMT__A_D_A_P,
                       TRACE_ARG_64(
                           request->extended_pan_id),
                       panid,
                       TRACE_ARG_64(long_addr), (void *)best_parent));

            TRACE_MSG(TRACE_NWK2, "ch %hd",
                      (FMT__H, best_parent->logical_channel));
            ...
            TRACE_MSG(TRACE_NWK2,
                      "Will assoc to pan " TRACE_FORMAT_64 "/%d, dev %d %p ch %hd",
                      (FMT__A_D_D_P_H,
                       TRACE_ARG_64(
                           request->extended_pan_id), panid,
                       best_parent->short_addr,
                       best_parent,
                       best_parent->logical_channel));
...
{
    zb_ret_t ret = 0;
    zb_mlme_associate_confirm_t *request = ZB_GET_BUF_PARAM((zb_buf_t *)ZB_BUF_FROM_REF(
                                                                param),
    ...
    if (request->status == MAC_SUCCESS) {
        zb_nwk_set_device_type(
            ZB_MAC_CAP_GET_DEVICE_TYPE(ZG->nwk.handle.saved_req.join.
                                       capability_information) ?
        ...
        ZB_EXTPANID_COPY(
            ZB_NIB_EXT_PAN_ID(), ZG->nwk.handle.saved_req.join.extended_pan_id);
        ...
        TRACE_MSG(TRACE_NWK3, "new update_id %hd",
                  (FMT__H, ZB_NIB_UPDATE_ID()));

        /* remember this device long addr + short addr in the addr table */
        zb_address_update(
            ZB_PIB_EXTENDED_ADDRESS(), request->assoc_short_address, ZB_TRUE,
            &addr_ref);
        ...
        /* remember potential parent address to find it in the neighbor table */
        {
            ...
            if (short_addr == (zb_uint16_t) ~0) {
                ZB_ADDRESS_DECOMPRESS(long_addr,
                                      ZG->nwk.handle.tmp.join.parent->long_addr);
            }
            else {
            ...
            if (short_addr == (zb_uint16_t) ~0) {
                /* use long address */
                ret = zb_address_by_ieee(long_addr, ZB_FALSE, ZB_FALSE,
                                         &addr_ref);
            }
            else {
            ...
            if (ret == RET_OK) {
                ...
                TRACE_MSG(TRACE_NWK3, "nb ent %p type %hd rel %hd",
                          (FMT__P_H_H, nent, (zb_uint8_t)nent->device_type,
                           (zb_uint8_t)nent->relationship));
        ...
        /* Notify upper layer */
        {
            ...
            zb_nlme_join_confirm_t *join_confirm = ZB_GET_BUF_PARAM((zb_buf_t *)ZB_BUF_FROM_REF(
                                                                        param),
                                                                    zb_nlme_join_confirm_t);

            join_confirm->status = (zb_mac_status_t)status;
            join_confirm->network_address = ZB_NIB_NETWORK_ADDRESS();
            ZB_EXTPANID_COPY(join_confirm->extended_pan_id,
                             ZG->nwk.handle.saved_req.join.extended_pan_id);
            join_confirm->active_channel =
                ZG->nwk.handle.tmp.join.parent->logical_channel;
            ZB_SCHEDULE_CALLBACK(zb_nlme_join_confirm, param);
        }
    }
    else {
        ...
        ret = nwk_association_join((zb_buf_t *)ZB_BUF_FROM_REF(
                                       param), &ZG->nwk.handle.saved_req.join);
        ...
        if (ret == ZB_NWK_STATUS_NOT_PERMITTED
             ...
             */
            ZB_MAC_CAP_SET_DEVICE_TYPE(
                ZG->nwk.handle.saved_req.join.capability_information, 0);
            ...
            ret = nwk_association_join((zb_buf_t *)ZB_BUF_FROM_REF(
                                           param),
                                       &ZG->nwk.handle.saved_req.join);
            TRACE_MSG(TRACE_NWK1, "Assoc as ZE retr ret %d", (FMT__D, ret));
        }
        if (ret != 0) {
            TRACE_MSG(TRACE_NWK1, "Assoc fail - pass MAC st %d up",
                      (FMT__D, status));
...
static void zb_nlme_rejoin(zb_uint8_t param) ZB_SDCC_REENTRANT
{
    ...
    zb_nlme_join_request_t *request = ZB_GET_BUF_PARAM((zb_buf_t *)ZB_BUF_FROM_REF(
                                                           param),
    ...
    if (ZG->nwk.handle.state == ZB_NLME_STATE_IDLE) {
        TRACE_MSG(TRACE_NWK1, "xpanid " TRACE_FORMAT_64,
                  (FMT__A, TRACE_ARG_64(request->extended_pan_id)));
        /* Save request params */
        ZB_EXTPANID_COPY(ZG->nwk.handle.tmp.rejoin.extended_pan_id,
                         request->extended_pan_id);
        ZG->nwk.handle.tmp.rejoin.capability_information =
            request->capability_information;
        ...
        /* start active scan */
        ZB_MLME_BUILD_SCAN_REQUEST(buf, scan_channels, ACTIVE_SCAN,
                                   scan_duration);
        ...
        TRACE_MSG(TRACE_NWK1, "nwk is busy, state %hd",
                  (FMT__H, ZG->nwk.handle.state));
...
void zb_nlme_rejoin_scan_confirm(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (zb_address_get_pan_id_ref(ZG->nwk.handle.tmp.rejoin.extended_pan_id,
                                  &panid_ref) != RET_OK) {
        TRACE_MSG(TRACE_NWK1, "No dev with xpanid " TRACE_FORMAT_64,
                  (FMT__A,
                   TRACE_ARG_64(ZG->nwk.handle.tmp.rejoin.extended_pan_id)));
        ret = ZB_NWK_STATUS_NOT_PERMITTED;
    }
    else {
        ...
        if (!best_parent) {
...
#ifdef ZB_SECURITY
            zb_bool_t secure =
                (zb_bool_t)(ZG->nwk.nib.secure_all_frames &&
            ...
            if (ZB_IEEE_ADDR_IS_ZERO(ieee_addr)) {
                 ...
                 * translation table?
                 */
                if (zb_address_ieee_by_short(best_parent->short_addr,
                                             ieee_addr) == RET_OK) {
                    TRACE_MSG(TRACE_NWK1, "short %d, long " TRACE_FORMAT_64,
                              (FMT__D_A, best_parent->short_addr,
                               TRACE_ARG_64(
                                   ieee_addr)));
...
#endif
            rejoin_request = (zb_nwk_rejoin_request_t *)nwk_alloc_and_fill_cmd(
                buf, ZB_NWK_CMD_REJOIN_REQUEST,
                sizeof(zb_nwk_rejoin_request_t));
            rejoin_request->capability_information =
                ZG->nwk.handle.tmp.rejoin.capability_information;

#if 0
            {
                zb_ushort_t hdr_size =
                    ZB_IEEE_ADDR_IS_ZERO(ieee_addr) ? ZB_NWK_HALF_HDR_SIZE(0) :
                    ZB_NWK_FULL_HDR_SIZE(0);
#ifdef ZB_SECURITY
                if (secure) {
            ...
            ZB_NWK_FRAMECTL_SET_SRC_DEST_IEEE(nwhdr->frame_control, 1, ZB_IEEE_ADDR_IS_ZERO(
                                                  ieee_addr) ? 0 : 1);
            ...
            ZB_NWK_ALLOC_COMMAND_GET_PAYLOAD_PTR(buf, ZB_NWK_CMD_REJOIN_REQUEST,
            ...
            (void)zb_mac_setup_for_associate(best_parent->logical_channel,
                                             ZB_NIB_PAN_ID(),
                                             best_parent->short_addr,
                                             ieee_addr);

            /* send request */
            ZB_SET_BUF_PARAM(buf, ZB_NWK_INTERNAL_REJOIN_CMD_HANDLE,
                             zb_uint8_t);
            ...
            zb_address_update(ieee_addr, best_parent->short_addr, ZB_TRUE,
                              &addr_ref);
...
void remove_parent_from_potential_parents(zb_ext_neighbor_tbl_ent_t *parent)
{
    if (parent) {
        ...
        if (zb_address_update(ieee_addr, parent->short_addr, ZB_FALSE,
                              &addr_ref) == RET_OK) {
...
void zb_nlme_rejoin_response_timeout(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (ZG->nwk.handle.tmp.rejoin.parent
        ...
        /* call mlme-poll */
        zb_mlme_poll_request_t *req = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                           param),
...
void zb_nlme_rejoin_response(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (ZG->nwk.handle.state == ZB_NLME_STATE_REJOIN) {
        ...
        ZB_ADDRESS_DECOMPRESS(parent_addr,
                              ZG->nwk.handle.tmp.rejoin.parent->long_addr);

        TRACE_MSG(TRACE_NWK1, "status %hd nwk_addr %d",
                  (FMT__H_D, rejoin_response->rejoin_status,
        ...
        TRACE_MSG(TRACE_NWK1, "MAC_PIB().mac_extended_address " TRACE_FORMAT_64,
                  (FMT__A,
                   TRACE_ARG_64(
                       MAC_PIB().mac_extended_address)));
        TRACE_MSG(TRACE_NWK1, "parent_addr " TRACE_FORMAT_64,
                  (FMT__A, TRACE_ARG_64(parent_addr)));
        ...
        if (rejoin_response->rejoin_status == MAC_SUCCESS
            ...
            && (!ZB_NWK_FRAMECTL_GET_DESTINATION_IEEE(nwhdr->frame_control)
                || ZB_IEEE_ADDR_CMP(nwhdr->dst_ieee_addr,
                                    MAC_PIB().mac_extended_address))
            && (ZB_IEEE_ADDR_IS_ZERO(parent_addr)
                || ZB_IEEE_ADDR_CMP(nwhdr->src_ieee_addr, parent_addr))) {
            /* remember this device long addr + short addr in the addr table */
            zb_address_update(
                ZB_PIB_EXTENDED_ADDRESS(), rejoin_response->network_addr, ZB_TRUE,
                &addr_ref);
            /* save parent long address if necessary */
            /* dirty hack here!!! if dst address is absent, dst pointer will be src */
            zb_address_update(ZB_NWK_FRAMECTL_GET_DESTINATION_IEEE(nwhdr->
                                                                   frame_control) ? nwhdr->src_ieee_addr : nwhdr->dst_ieee_addr,
                              ZG->nwk.handle.tmp.rejoin.parent->short_addr,
                              ZB_TRUE, /* lock! */
             ...
             */
#if defined ZB_NWK_DISTRIBUTED_ADDRESS_ASSIGN && defined ZB_ROUTER_ROLE
            ZB_NIB_DEPTH() = ZG->nwk.handle.tmp.rejoin.parent->depth +
                             (ZB_NIB_DEPTH() != 0xf);
            /* Calculate CScip value for tree routing */
            ZG->nwk.nib.cskip = zb_nwk_daa_calc_cskip(ZB_NIB_DEPTH());
#endif
            ...
            ZB_EXTPANID_COPY(
                ZB_NIB_EXT_PAN_ID(), ZG->nwk.handle.tmp.rejoin.extended_pan_id);
...

            {
                ...
                if (zb_nwk_neighbor_get(ZG->nwk.handle.parent, ZB_FALSE,
                                        &nent) == RET_OK) {
                    nent->relationship = ZB_NWK_RELATIONSHIP_PARENT;
                }
                else {
                    TRACE_MSG(TRACE_ERROR,
                              "Oops: no our parent in the neighbor table!",
                              (FMT__0));
                }
            }
...
static void zb_nlme_orphan_scan(zb_uint8_t param) ZB_SDCC_REENTRANT
{
    ...
    zb_nlme_join_request_t *request = ZB_GET_BUF_PARAM(buf,
                                                       zb_nlme_join_request_t);
    ...
    if (ZG->nwk.handle.state == ZB_NLME_STATE_IDLE) {
        /* perform orphan scan */
        ZB_MLME_BUILD_SCAN_REQUEST(buf, request->scan_channels, ORPHAN_SCAN,
                                   request->scan_duration);
        ZB_SCHEDULE_CALLBACK(zb_mlme_scan_request, param);
        ZG->nwk.handle.state = ZB_NLME_STATE_ORPHAN_SCAN;
    }
    else {
        TRACE_MSG(TRACE_NWK1, "nwk is busy, state %d",
                  (FMT__D, ZG->nwk.handle.state));
        nwk_join_failure_confirm(param, ZB_NWK_STATUS_NOT_PERMITTED);
    }
...
void zb_nlme_orphan_scan_confirm(zb_uint8_t param) ZB_SDCC_REENTRANT
{
    ...
    zb_mac_scan_confirm_t *confirm =
        ZB_GET_BUF_PARAM(buf, zb_mac_scan_confirm_t);
    zb_uint8_t status = confirm->status;

    TRACE_MSG(TRACE_NWK1, ">> orphan_scan_cnfrm prm %hd status %hd",
              (FMT__H_H, param, buf->u.hdr.status));

    nwk_join_failure_confirm(param,
                             (status ==
                              MAC_SUCCESS) ? ZB_NWK_STATUS_SUCCESS : ZB_NWK_STATUS_NO_NETWORKS);
...
void zb_mlme_orphan_indication(zb_uint8_t param)
{
    ...
    if (zb_address_by_ieee(ind->orphan_addr, ZB_FALSE, ZB_FALSE, &ref) == RET_OK
        ...
        zb_mac_orphan_response_t *resp = ZB_GET_BUF_PARAM(buf,
...
void zb_mlme_comm_status_indication(zb_uint8_t param) ZB_CALLBACK
{
    ...
    zb_mlme_comm_status_indication_t *request = ZB_GET_BUF_PARAM((zb_buf_t *)ZB_BUF_FROM_REF(
                                                                     param),
    ...
    TRACE_MSG(TRACE_NWK1, ">>mlme_comm_status_ind", (FMT__0));
    {
        zb_uint8_t i;
        for (i = 0; i < 8; i++) {
            TRACE_MSG(TRACE_NWK1, "addr[] %hd %hx",
                      (FMT__H_H, i, request->dst_addr.addr_long[i]));
        }
    }
    if (request->dst_addr_mode == ZB_ADDR_64BIT_DEV) {
        ret = zb_address_by_ieee(request->dst_addr.addr_long, ZB_FALSE,
                                 ZB_FALSE, &addr_ref);
    }
    else {
        ret = zb_address_by_short(request->dst_addr.addr_short, ZB_FALSE,
                                  ZB_FALSE, &addr_ref);
    }
    ...
    if (ret == RET_OK) {
        if (request->status == MAC_SUCCESS) {
            ...
            zb_nlme_join_indication_t *resp = ZB_GET_BUF_PARAM((zb_buf_t *)ZB_BUF_FROM_REF(
                                                                   param),
                                                               zb_nlme_join_indication_t);

            zb_address_by_ref(resp->extended_address, &resp->network_address,
                              addr_ref);
            resp->capability_information = 0;
            /* Only that 2 fields of capability information are meaningful */
            ZB_MAC_CAP_SET_DEVICE_TYPE(resp->capability_information,
                                       (nent->device_type ==
                                        ZB_NWK_DEVICE_TYPE_ROUTER));
            ZB_MAC_CAP_SET_RX_ON_WHEN_IDLE(resp->capability_information,
                                           nent->rx_on_when_idle);
