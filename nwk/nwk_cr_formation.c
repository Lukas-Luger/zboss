...
static void call_mlme_start(zb_buf_t *buf, zb_uint16_t pan_id,
                            zb_uint16_t channel) ZB_SDCC_REENTRANT;
#endif
...
void zb_nlme_network_formation_request(zb_uint8_t param) ZB_CALLBACK
{
    ...
    TRACE_MSG(TRACE_NWK1, "nwk state %hd joined %hd",
              (FMT__H_H, ZG->nwk.handle.state, ZG->nwk.handle.joined));

    if (!(ZG->nwk.handle.state == ZB_NLME_STATE_IDLE
          && !ZG->nwk.handle.joined)) {
        TRACE_MSG(TRACE_NWK1, "nwk busy or joined",
                  (FMT__0, ZG->nwk.handle.state));
        NWK_FORMATION_FAILURE_CONFIRM((zb_buf_t *)ZB_BUF_FROM_REF(
                                          param),
        ...
        zb_nlme_network_formation_request_t *request =
            ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                 param), zb_nlme_network_formation_request_t);

        /* Save request values */
        ZB_MEMCPY(&ZG->nwk.handle.saved_req.formation, request,
                  sizeof(*request));
        ...
        ZB_MLME_BUILD_SCAN_REQUEST((zb_buf_t *)ZB_BUF_FROM_REF(
                                    ...
                                    * channel specified */
                                   MAGIC_IS_POWER_OF_TWO(ZG->nwk.handle.
                                                         saved_req.formation.
                                                         scan_channels) ? ACTIVE_SCAN : ED_SCAN,
        ...
        ZG->nwk.handle.state = MAGIC_IS_POWER_OF_TWO(
            ZG->nwk.handle.saved_req.formation.scan_channels) ?
...
{
    zb_mac_scan_confirm_t *scan_confirm = ZB_GET_BUF_PARAM(buf,
    ...
    ZB_MEMCPY(ZG->nwk.handle.tmp.formation.energy_detect,
              scan_confirm->list.energy_detect,
    ...
    for (i = 0, channel_mask <<= ZB_MAC_START_CHANNEL_NUMBER;
         ...
         i++, channel_mask <<= 1) {
        TRACE_MSG(TRACE_NWK1, "ch %hd e_level %hd ",
    ...
    if (ZG->nwk.handle.saved_req.formation.scan_channels) {
        ...
        ZB_MLME_BUILD_SCAN_REQUEST(buf,
                                   ZG->nwk.handle.saved_req.formation.scan_channels,
                                   ACTIVE_SCAN,
                                   ZG->nwk.handle.saved_req.formation.scan_duration);
...
void nwk_formation_select_channel(zb_buf_t *buf) ZB_SDCC_REENTRANT
{
    ...
    zb_uint8_t *channel_pan_count =
        &ZG->nwk.handle.tmp.formation.channel_pan_count[-
                                                        ZB_MAC_START_CHANNEL_NUMBER
        ];
    zb_uint8_t *panid_handled_bm =
        &ZG->nwk.handle.tmp.formation.panid_handled_bm[0];
    zb_uint_t i = 0;


    TRACE_MSG(TRACE_NWK1, ">>sel_ch buf %p", (FMT__P, buf));

    ZB_BZERO(panid_handled_bm,
             sizeof(ZG->nwk.handle.tmp.formation.panid_handled_bm));
    ...
    for (i = 0; i < ZG->nwk.neighbor.ext_neighbor_used; ++i) {
        if (ZB_CHECK_BIT_IN_BIT_VECTOR(panid_handled_bm,
                                       ZG->nwk.neighbor.ext_neighbor[i].
                                       panid_ref)) {
            continue;
        }
        ZB_SET_BIT_IN_BIT_VECTOR(panid_handled_bm,
                                 ZG->nwk.neighbor.ext_neighbor[i].panid_ref);
    ...
    if (channel != ZB_MAC_START_CHANNEL_NUMBER) {
        ...
        while (!unique_pan_id) {
            ...
            for (i = 0, unique_pan_id = 1;
                 i < ZG->nwk.neighbor.ext_neighbor_used; i++) {
                if (ZG->nwk.neighbor.ext_neighbor[i].logical_channel ==
                    channel) {
                    zb_uint16_t nt_panid;
                    zb_address_get_short_pan_id(
                        ZG->nwk.neighbor.ext_neighbor[i].panid_ref, &nt_panid);
                    if (nt_panid == pan_id) {
                        TRACE_MSG(TRACE_NWK1, "pan_id %d is on ch %hd",
                                  (FMT__D_H, pan_id, channel));
                        unique_pan_id = 0;
                        break;
                    }
                }
...
void zb_nlme_start_router_request(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (ZB_NIB_DEVICE_TYPE() == ZB_NWK_DEVICE_TYPE_ROUTER) {
        ...
        zb_nlme_start_router_request_t *request = ZB_GET_BUF_PARAM((zb_buf_t *)ZB_BUF_FROM_REF(
                                                                       param),
                                                                   zb_nlme_start_router_request_t);


        TRACE_MSG(TRACE_NWK1, "b_ord %hd sf_ord %hd bat_life_ext %hd",
                  (FMT__H_H_H,
                   request
                   ->beacon_order, request->superframe_order,
        ...
        ZB_MEMCPY(ZB_GET_BUF_PARAM((zb_buf_t *)ZB_BUF_FROM_REF(param),
                                   zb_mlme_start_req_t), &req, sizeof(req));
...
//   ZB_PIB_BEACON_PAYLOAD().device_depth = ZB_NIB_DEPTH();
    ...
    ZB_MEMSET(&ZB_PIB_BEACON_PAYLOAD().txoffset, 0xFF,
              sizeof(ZB_PIB_BEACON_PAYLOAD().txoffset));
    /*
...
/* TODO: check if parameter type is correct - zb_mlme_reset_request_t*
   is put on zb_nlme_network_formation_request(),zb_mlme_reset_request() call */
        ...
        /* request was saved in our internal buffer in zb_nlme_network_formation_request() */
        zb_nlme_network_formation_request_t *request =
            ZB_GET_BUF_PARAM(&ZG->nwk.handle.buf,
        ...
        ZB_MLME_BUILD_SCAN_REQUEST((zb_buf_t *)ZB_BUF_FROM_REF(param),
                                   ...
                                   MAGIC_IS_POWER_OF_TWO(
                                       request->scan_channels) ? ACTIVE_SCAN : ED_SCAN,
...
void zb_nlme_network_formation_confirm(zb_uint8_t param) ZB_CALLBACK
{
    ...
    TRACE_MSG(TRACE_NWK1, ">>zb_nlme_network_formation vbuf %hd", (FMT__H,
                                                                   param));

    if (buf) {
        zb_nlme_network_formation_confirm_t *confirm =
            (zb_nlme_network_formation_confirm_t *)ZB_BUF_BEGIN(buf);
        ZVUNUSED(confirm);
    }
