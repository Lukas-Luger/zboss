...
static zb_mac_status_t zb_nwk_accept_child(zb_ieee_addr_t device_address,
...
void zb_nlme_rejoin_request(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (ZG->nwk.handle.rejoin_req_table_cnt < ZB_NWK_REJOIN_REQUEST_TABLE_SIZE
        ...
        status = zb_nwk_accept_child(*src_ieee_addr,
                                     rejoin_request->capability_information, ZB_MAC_GET_LQI(
                                         buf), &address);
        TRACE_MSG(TRACE_NWK3, "address assigned %d", (FMT__D));
        dst_addr = nwhdr->src_addr;

        nwhdr = nwk_alloc_and_fill_hdr(buf, dst_addr,
                                       ZB_PIB_EXTENDED_ADDRESS(), dst_ieee_addr, ZB_FALSE, secure,
                                       ZB_TRUE);
#ifdef ZB_SECURITY
        if (secure) {
#ifdef ZB_SECURITY
            buf->u.hdr.encrypt_type = ZB_SECUR_NWK_ENCR;
            TRACE_MSG(TRACE_NWK1, "rejoin req was secured, reply secured also",
                      (FMT__0));
#endif
        }
#endif
        rejoin_response = (zb_nwk_rejoin_response_t *)nwk_alloc_and_fill_cmd(
            buf, ZB_NWK_CMD_REJOIN_RESPONSE, sizeof(zb_nwk_rejoin_response_t));
        ...
        ZB_SET_BUF_PARAM(buf,
                         (status == MAC_SUCCESS) ? ZB_NWK_INTERNAL_REJOIN_CMD_RESPONSE : ZB_NWK_INTERNAL_NSDU_HANDLE,
                         zb_uint8_t);
        ...
        if (status == MAC_SUCCESS) {
            ...
            ZG->nwk.handle.rejoin_req_table[ZG->nwk.handle.rejoin_req_table_cnt]
                = address;
            TRACE_MSG(TRACE_NWK1, "rejoin_req_table[%hd] = %d",
                      (FMT__H_D, ZG->nwk.handle.rejoin_req_table_cnt, address));
            ZG->nwk.handle.rejoin_req_table_cnt++;
        }
    }
    else {
        TRACE_MSG(TRACE_NWK1,
                  "rejoin req tbl is full or bad request - drop req", (FMT__0));
...
void zb_nlme_rejoin_resp_sent(zb_uint8_t param) ZB_CALLBACK
{
    ...
    TRACE_MSG(TRACE_NWK1, ">>rejoin_resp_sent param %hd status %hd",
              (FMT__H_H, param, ZB_BUF_FROM_REF(param)->u.hdr.status));

    if (ZG->nwk.handle.rejoin_req_table_cnt
        && zb_address_by_short(ZG->nwk.handle.rejoin_req_table[ZG->nwk.handle.
                                                               rejoin_req_table_cnt
                                                               - 1], ZB_FALSE,
...
#ifdef ZB_SECURITY
        {
            ...
            resp->secure_rejoin = !ZB_NWK_FRAMECTL_GET_SECURITY(
                nwhdr->frame_control);
            TRACE_MSG(TRACE_NWK3, "secure_rejoin %hd (1 means insecure!)",
                      (FMT__H, resp->secure_rejoin));
        }
#else
...
ZB_SDCC_REENTRANT
{
    ...
    TRACE_MSG(TRACE_NWK1,
              ">>zb_nwk_accept_child device_address " TRACE_FORMAT_64 " cap 0x%hx lqi %hd address %p",
              (FMT__P_H_H, TRACE_ARG_64(device_address), capability, lqi,
               address));
    ...
    if (zb_nwk_neighbor_get_by_ieee(device_address, &ent) == RET_OK) {
        if (
            ...
            ZB_MAC_CAP_GET_DEVICE_TYPE(capability)
            /* idle match */
            && ZB_MAC_CAP_GET_RX_ON_WHEN_IDLE(capability) ==
            ent->rx_on_when_idle
            ) {
            zb_address_short_by_ref(&addr, ent->addr_ref);
            TRACE_MSG(TRACE_NWK3, "found dev %hd short %d in neighb tbl",
                      (FMT__H_D, ent->addr_ref, addr));
        }
        else {
            TRACE_MSG(TRACE_NWK3, "rm dev %hd from neighb tbl: diff caps",
                      (FMT__H, ent->addr_ref));
    ...
    TRACE_MSG(TRACE_NWK1,
              "cap_inf 0x%x ed_ch_num %hd router_ch_num %hd max_ch %hd max_routers %hd",
              (FMT__D_H_H_H_H,
               (zb_uint16_t)capability,
               (zb_uint8_t)ZG->nwk.nib.ed_child_num,
               (zb_uint8_t)ZG->nwk.nib.router_child_num,
               (zb_uint8_t)ZG->nwk.nib.max_children,
               (zb_uint8_t)ZG->nwk.nib.max_routers));
    if (!ent) {
        status = MAC_OUT_OF_CAP;
        if ((ZG->nwk.nib.max_children >
             ZG->nwk.nib.ed_child_num + ZG->nwk.nib.router_child_num)
            ...
            if (ZB_MAC_CAP_GET_DEVICE_TYPE(capability)) {
                if (ZG->nwk.nib.router_child_num < ZG->nwk.nib.max_routers) {
                    ...
                    TRACE_MSG(TRACE_NWK3, "assigned router addr 0x%x",
                              (FMT__D, addr));
                ...
                TRACE_MSG(TRACE_NWK3, "assigned ed addr 0x%x (%u %hd %u %hd)",
                          (FMT__D_D_H_D_H, addr, ZG->mac.pib.mac_short_address,
                           ZG->nwk.nib.max_routers, ZG->nwk.nib.cskip,
        ...
        if (status == MAC_SUCCESS) {
            ...
            status = MAC_OUT_OF_CAP;
            if (zb_address_update(device_address, addr, ZB_TRUE,
                                  &ieee_ref) == RET_OK) {
    ...
    if (MAC_PIB().mac_association_permit
        && ZG->nwk.nib.router_child_num + ZG->nwk.nib.ed_child_num >=
        ZG->nwk.nib.max_children) {
        TRACE_MSG(TRACE_NWK1,
                  "status %hd, max_children %hd, routers %hd, eds %hd - disable association permit",
                  (FMT__H_H_H_H, status, ZG->nwk.nib.max_children,
                   ZG->nwk.nib.router_child_num, ZG->nwk.nib.ed_child_num));
    ...
    if (status == MAC_SUCCESS) {
        ...
        ent->device_type = ZB_NWK_DEVICE_TYPE_ED + ZB_MAC_CAP_GET_DEVICE_TYPE(
            capability);
        ent->rx_on_when_idle = ZB_MAC_CAP_GET_RX_ON_WHEN_IDLE(capability);
#ifdef ZB_SECURITY
        ent->relationship =
            ZB_NIB_SECURITY_LEVEL() ? ZB_NWK_RELATIONSHIP_UNAUTHENTICATED_CHILD
                                    :
            ZB_NWK_RELATIONSHIP_CHILD;
#else
        ent->relationship = ZB_NWK_RELATIONSHIP_CHILD;
#endif
        ent->depth = ZG->nwk.nib.depth +
                     (ZG->nwk.nib.depth < ZG->nwk.nib.max_depth);
        ...
        TRACE_MSG(TRACE_NWK2, "neighbor ent %p rel %hd dev_t %hd rx.on %hd",
                  (FMT__P_H_H_H, ent, (zb_uint8_t)ent->relationship,
                   (zb_uint8_t)ent->device_type,
                   (zb_uint8_t)ent->rx_on_when_idle));

        *address = addr;
    }

    TRACE_MSG(TRACE_NWK1, "<<zb_nwk_accept_child status %hd, address %d",
              (FMT__H_D, status, addr));
    return status;
}
...
void zb_mlme_associate_indication(zb_uint8_t param) ZB_CALLBACK
{
    ...
    zb_mlme_associate_indication_t *request = ZB_GET_BUF_PARAM((zb_buf_t *)ZB_BUF_FROM_REF(
                                                                   param),
    ...
    status = zb_nwk_accept_child(request->device_address, request->capability,
                                 request->lqi, &address);
#endif
    ...
    ZB_MLME_BUILD_ASSOCIATE_RESPONSE((zb_buf_t *)ZB_BUF_FROM_REF(
                                         param), device_address, address,
                                     status);
