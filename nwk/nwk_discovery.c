...
{
    zb_nlme_network_discovery_request_t *request = ZB_GET_BUF_PARAM((zb_buf_t *)ZB_BUF_FROM_REF(
                                                                        param),
                                                                    zb_nlme_network_discovery_request_t);

    TRACE_MSG(TRACE_NWK1, ">>zb_nlme_network_discovery_request %p",
              (FMT__P, request));
    ...
    if (ZG->nwk.handle.state == ZB_NLME_STATE_IDLE) {
...

        {
            ...
            TRACE_MSG(TRACE_NWK1, "scan channels 0x%x scan_duration %hd",
                      (FMT__D_H, rq.scan_channels, rq.scan_duration));
            ZB_MLME_BUILD_SCAN_REQUEST((zb_buf_t *)ZB_BUF_FROM_REF(
                                           param), rq.scan_channels, ACTIVE_SCAN,
                                       rq.scan_duration);
        }
...
{
    zb_nlme_ed_scan_request_t *request = ZB_GET_BUF_PARAM((zb_buf_t *)ZB_BUF_FROM_REF(
                                                              param),
    ...
    if (ZB_NIB_DEVICE_TYPE() != ZB_NWK_DEVICE_TYPE_NONE) {
        {
            ...
            ZB_MEMCPY(&rq, request, sizeof(rq));
            ZB_MLME_BUILD_SCAN_REQUEST((zb_buf_t *)ZB_BUF_FROM_REF(
                                           param), rq.scan_channels, ED_SCAN,
                                       rq.scan_duration);
        }
        ZB_SCHEDULE_CALLBACK(zb_mlme_scan_request, param);
        ...
        NWK_CONFIRM_STATUS((zb_buf_t *)ZB_BUF_FROM_REF(
                               param), ZB_NWK_STATUS_INVALID_REQUEST,
                           zb_nlme_ed_scan_confirm);
    }
...
{
    zb_uint8_t *mac_hdr =
        ZB_MAC_GET_FCF_PTR(ZB_BUF_BEGIN((zb_buf_t *)ZB_BUF_FROM_REF(param)));
    zb_mac_mhr_t mhr;
    zb_uint8_t mhr_len;
    zb_mac_beacon_payload_t *beacon_payload;


    TRACE_MSG(TRACE_NWK1, ">>zb_mlme_beacon_notify_indication %hd",
              (FMT__H, param));
...
#ifdef ZB_MAC_TESTING_MODE
    {
        ...
        payload_len = ZB_BUF_LEN(ZB_BUF_FROM_REF(param)) -
                      ZB_BEACON_PAYLOAD_OFFSET(mhr_len) -
                      ZB_BEACON_PAYLOAD_TAIL;
    ...
    if (ZG->nwk.handle.state == ZB_NLME_STATE_DISC
        ...
        TRACE_MSG(TRACE_NWK2,
                  "beacon payload %p: protocol id %d, version %d, stack profile %d",
                  (FMT__P_D_D_D,
                   beacon_payload,
                   (int)beacon_payload->protocol_id,
                   (
                       int)beacon_payload->protocol_version,
                   (int)beacon_payload->stack_profile));
...
/*        && beacon_payload->stack_profile == ZB_STACK_PROFILE */
            ...
            if ((ZG->nwk.handle.state != ZB_NLME_STATE_REJOIN)
                || ZB_64BIT_ADDR_CMP(beacon_payload->extended_panid,
                                     ZG->nwk.handle.tmp.rejoin.extended_pan_id))
            {
                ...
                ret = zb_address_set_pan_id(mhr.src_pan_id,
                                            beacon_payload->extended_panid,
                                            &panid_ref);
                ...
                if (ret == RET_OK) {
                    ...
                    if (ZB_FCF_GET_SRC_ADDRESSING_MODE(&mhr.frame_control) ==
                        ...
                        /* 16 bit address. */
                        ret = zb_nwk_exneighbor_by_short(panid_ref,
                                                         mhr.src_addr.addr_short,
                                                         &enbt);
                    }
#ifndef ZB_LIMITED_FEATURES
                    else {
                        ret = zb_nwk_exneighbor_by_ieee(panid_ref,
                ...
                if (ret == RET_OK) {
                    ...
                    ZB_MAC_GET_SUPERFRAME_SPEC(mac_hdr, mhr_len,
                                               superframe_spec);
                    ...
                    enbt->lqi = ZB_MAC_GET_LQI((zb_buf_t *)ZB_BUF_FROM_REF(
                                                   param));
#ifdef ZB_NS_BUILD
                    ...
                    enbt->logical_channel =
                        ZB_MAC_GET_CURRENT_LOGICAL_CHANNEL();
                    TRACE_MSG(TRACE_NWK2, "ch %hd",
                              (FMT__H, enbt->logical_channel));
                    enbt->depth = beacon_payload->device_depth;
                    enbt->beacon_order =
                        ZB_MAC_GET_BEACON_ORDER(superframe_spec);
                    enbt->permit_joining = ZB_MAC_GET_ASSOCIATION_PERMIT(
                        superframe_spec);
                    ...
                    enbt->superframe_order = ZB_MAC_GET_SUPERFRAME_ORDER(
                        superframe_spec);
                    enbt->router_capacity = beacon_payload->router_capacity;
                    enbt->end_device_capacity =
                        beacon_payload->end_device_capacity;

                    /* Some complex stuff */
#ifdef ZB_ROUTER_ROLE
                    if ((ZB_NIB_DEVICE_TYPE() !=
                         (zb_uint8_t)ZB_NWK_DEVICE_TYPE_ED) &&
                        (enbt->stack_profile != 2)) {
                        enbt->potential_parent = enbt->permit_joining &&
                                                 enbt->router_capacity;
                    }
                    else
#endif
                    {
                    ...
                    else if (enbt->permit_joining || enbt->router_capacity ||
                             enbt->end_device_capacity) {
                ...
                TRACE_MSG(TRACE_NWK2,
                          "Skip beacon not for us: its protocol id %hd, version %hd, stack profile %hd",
                          (FMT__H_H_H,
                           beacon_payload
                           ->protocol_id,
                           beacon_payload
...
/* during PAN_ID conflict resolution										  */
        ...
        if (mhr.src_pan_id == ZB_NIB_PAN_ID()
            && beacon_payload
            && !ZB_EXTPANID_CMP(beacon_payload->extended_panid,
                                ZB_NIB_EXT_PAN_ID())) {
            if (ZG->nwk.nib.nwk_manager_addr == ZB_PIB_SHORT_ADDRESS()) {
                TRACE_MSG(TRACE_NWK1,
                          "Got PANID conflict, I am a nwk manager - start panid conflict resolve",
                          (FMT__0));
                ZB_MEMSET(ZG->nwk.handle.known_panids, -1,
                          sizeof(ZG->nwk.handle.known_panids));
                ZG->nwk.handle.state = ZB_NLME_STATE_IDLE;
                TRACE_MSG(TRACE_NWK3,
                          "sending Pan ID conflict resolution network update",
                          (FMT__0));
        ...
        TRACE_MSG(TRACE_NWK1,
                  "NWK state is %d - not ZB_NLME_STATE_DISC - ignore beacon",
                  (FMT__D, ZG->nwk.handle.state));
    }
...
//#if defined ZB_ROUTER_ROLE && !defined ZB_NWK_ONE_SCAN_ATTEMPT
        ...
        TRACE_MSG(TRACE_NWK3, "disc_count %hd",
                  (FMT__H, ZDO_CTX().zdo_ctx.discovery_ctx.disc_count));
        ...
        if (ZDO_CTX().zdo_ctx.discovery_ctx.disc_count) {
            ...
            TRACE_MSG(TRACE_NWK3, "call discovery_request channels 0x%x",
                      (FMT__D, req->scan_channels));
            ZG->nwk.handle.state = ZB_NLME_STATE_IDLE;
            ZB_SCHEDULE_ALARM(zb_nlme_network_discovery_request, param,
                              ZDO_CTX().conf_attr.nwk_time_btwn_scans);
...
//#endif /* */
            ...
            scan_confirm = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                param), zb_mac_scan_confirm_t);
            TRACE_MSG(TRACE_NWK3, "scan type %hd, status %hd, auto_req %hd",
                      (FMT__H_H_H, scan_confirm->scan_type,
                       scan_confirm->status, MAC_PIB().mac_auto_request));
            if (scan_confirm->scan_type == ACTIVE_SCAN &&
                scan_confirm->status == MAC_SUCCESS &&
                MAC_PIB().mac_auto_request) {
                zb_pan_descriptor_t *pan_desc;
                pan_desc =
                    (zb_pan_descriptor_t *)ZB_BUF_BEGIN(ZB_BUF_FROM_REF(param));
                TRACE_MSG(TRACE_NWK3, "ative scan res count %hd",
                          (FMT__H, scan_confirm->result_list_size));
                for (i = 0; i < scan_confirm->result_list_size; i++) {
                    TRACE_MSG(TRACE_NWK3,
                              "pan desc: coord addr mode %hd, coord addr %x, pan id %x, channel %hd, superfame %x, lqi %hx",
                              (FMT__H_D_D_H_D_H, pan_desc->coord_addr_mode,
                               pan_desc->coord_address.addr_short,
                               pan_desc->coord_pan_id,
                               pan_desc->logical_channel,
                               pan_desc->super_frame_spec,
                               pan_desc->link_quality));

                    if (pan_desc->coord_addr_mode == ZB_ADDR_64BIT_DEV) {
                        TRACE_MSG(TRACE_MAC3,
                                  "Extended coord addr " TRACE_FORMAT_64,
                                  (FMT__A,
                                   TRACE_ARG_64(pan_desc->coord_address.
                                                addr_long)));
                    }
            ...
            ZB_BUF_INITIAL_ALLOC((zb_buf_t *)ZB_BUF_FROM_REF(
                                     param),
                                 sizeof(*discovery_confirm) +
                                 sizeof(*network_descriptor) * ZB_PANID_TABLE_SIZE,
                                 discovery_confirm);
            network_descriptor =
                (zb_nlme_network_descriptor_t *)(discovery_confirm + 1);
            ...
            while (dev_type_cmp) {
                ...
                for (i = 0; i < ZG->nwk.neighbor.ext_neighbor_used; ++i) {
                    if (!dev_type_cmp ||
                        ZG->nwk.neighbor.ext_neighbor[i].device_type ==
                        ZB_NWK_DEVICE_TYPE_COORDINATOR) {
                        for (j = 0;
                             j < n_nwk_dsc &&
                             !zb_address_cmp_pan_id_by_ref(ZG->nwk.neighbor.
                                                           ext_neighbor[i].
                                                           panid_ref,
                                                           network_descriptor[j]
                                                           .extended_pan_id);
                             ++j) {}
                        if (j == n_nwk_dsc) {
                            /* This ext pan id not found - add this PAN */
                            zb_address_get_pan_id(ZG->nwk.neighbor.ext_neighbor[
                                                      i].panid_ref,
                                                  network_descriptor[j].extended_pan_id);
                            network_descriptor[j].logical_channel =
                                ZG->nwk.neighbor.ext_neighbor[i].logical_channel;
                            network_descriptor[j].stack_profile =
                                ZG->nwk.neighbor.ext_neighbor[i].stack_profile;
                            network_descriptor[j].zigbee_version =
                                ZB_PROTOCOL_VERSION;
                            network_descriptor[j].beacon_order =
                                ZG->nwk.neighbor.ext_neighbor[i].beacon_order;
                            network_descriptor[j].superframe_order =
                                ZG->nwk.neighbor.ext_neighbor[i].
                                superframe_order;
                            network_descriptor[j].permit_joining =
                                ZG->nwk.neighbor.ext_neighbor[i].permit_joining;
                            network_descriptor[j].router_capacity =
                                ZG->nwk.neighbor.ext_neighbor[i].router_capacity;
                            network_descriptor[j].end_device_capacity =
                                ZG->nwk.neighbor.ext_neighbor[i].
                                end_device_capacity;
                            n_nwk_dsc++;
                        }
            ...
            discovery_confirm->status =
                (zb_mac_status_t)((zb_buf_t *)ZB_BUF_FROM_REF(param))->u.hdr.
                status;
            ZB_SCHEDULE_CALLBACK(zb_nlme_network_discovery_confirm, param);
            ZG->nwk.handle.state = ZB_NLME_STATE_IDLE;
        }
        ...
        TRACE_MSG(TRACE_ERROR, "wrong nwk state %d",
                  (FMT__D, (int)ZG->nwk.handle.state));
        ZB_ASSERT(0);
    }
