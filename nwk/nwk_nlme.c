...
void zb_nlme_leave_request(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (!ZG->nwk.handle.joined) {
        TRACE_MSG(TRACE_ERROR, "got leave.request when not joined",
                  (FMT__H, param));
        ret = RET_ERROR;
        ...
        if (zb_nwk_neighbor_get_by_ieee(r.device_address, &nbt) != RET_OK) {
            ...
            TRACE_MSG(TRACE_NWK2,
                      "device is not authenticated - do't send LEAVE to it (status 0)",
                      (FMT__0));
            ret = RET_ERROR;
        }
    ...
    if (ret == RET_OK) {
        ...
        if (!nbt) {
            ...
            nwhdr = nwk_alloc_and_fill_hdr(nsdu,
                                           child_addr,
                                           ZB_PIB_EXTENDED_ADDRESS(), ieee_addr, ZB_FALSE, secure,
                                           ZB_TRUE);
        }
#endif  /* router */
        ...
        lp = (zb_uint8_t *)nwk_alloc_and_fill_cmd(nsdu, ZB_NWK_CMD_LEAVE,
                                                  sizeof(zb_uint8_t));
        *lp = 0;
        ...
        if (nbt) {
            ...
            TRACE_MSG(TRACE_NWK3,
                      "send leave.request request 1 rejoin %hd remove_children %hd",
        ...
        {
            TRACE_MSG(TRACE_NWK3,
                      "send leave.request request 0 (I am leaving) rejoin %hd",
                      (FMT__H, r.rejoin));
        }
    }
...
void zb_nlme_reset_request(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (ZG->nwk.handle.state == ZB_NLME_STATE_IDLE
        ...
        if (request->warm_start) {
            ...
            NWK_ARRAY_CLEAR(ZG->nwk.nib.routing_table, zb_nwk_routing_t,
                            ZG->nwk.nib.routing_table_cnt);
            NWK_ARRAY_CLEAR(ZG->nwk.nib.route_disc_table,
                            zb_nwk_route_discovery_t,
                            ZG->nwk.nib.route_disc_table_cnt);
#endif

            NWK_CONFIRM_STATUS(buf, ZB_NWK_STATUS_SUCCESS,
                               zb_nlme_reset_confirm);
        }
        else {
            zb_mlme_reset_request_t *req = ZB_GET_BUF_PARAM(buf,
        ...
        NWK_CONFIRM_STATUS(buf, ZB_NWK_STATUS_INVALID_REQUEST,
                           zb_nlme_reset_confirm);
    }
...
{
    zb_buf_t *buf = (zb_buf_t *)ZB_BUF_FROM_REF(param);
    zb_nlme_sync_request_t *request = ZB_GET_BUF_PARAM(buf,
    ...
    if (request->track) {
        NWK_CONFIRM_STATUS(buf, ZB_NWK_STATUS_INVALID_PARAMETER,
                           zb_nlme_sync_confirm);
    }
    else {
        /* call mlme-poll */
...
{
    zb_mlme_sync_loss_ind_t *ind = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                        param),
...
void zb_nlme_get_request(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (req.nib_attribute == ZB_NIB_ATTRIBUTE_SEQUENCE_NUMBER) {
        ZB_BUF_INITIAL_ALLOC(buf,
                             sizeof(zb_nlme_get_confirm_t) + sizeof(zb_uint8_t),
                             conf);
        conf->attribute_length = sizeof(zb_uint8_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_nlme_get_confirm_t)) = ZB_NIB_SEQUENCE_NUMBER();
    }
    else if (req.nib_attribute == ZB_NIB_ATTRIBUTE_PASSIVE_ASK_TIMEOUT) {
        ZB_BUF_INITIAL_ALLOC(buf,
                             sizeof(zb_nlme_get_confirm_t) + sizeof(zb_uint16_t),
                             conf);
        conf->attribute_length = sizeof(zb_uint16_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_nlme_get_confirm_t)) = ZG->nwk.nib.passive_ack_timeout;
    }
        ...
        conf->attribute_length = sizeof(zb_uint8_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_nlme_get_confirm_t)) = ZG->nwk.nib.max_broadcast_retries;
    }
        ...
        conf->attribute_length = sizeof(zb_uint8_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_nlme_get_confirm_t)) = ZG->nwk.nib.max_children;
    }
        ...
        conf->attribute_length = sizeof(zb_uint8_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_nlme_get_confirm_t)) = ZB_NIB_MAX_DEPTH();
    }
        ...
        conf->attribute_length = sizeof(zb_uint8_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_nlme_get_confirm_t)) = ZG->nwk.nib.max_routers;
    }
#endif
        ...
        conf->attribute_length = sizeof(zb_uint8_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_nlme_get_confirm_t)) = ZG->nwk.nib.addr_alloc;
    }
        ...
        conf->attribute_length = sizeof(zb_uint8_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_nlme_get_confirm_t)) = ZG->nwk.nib.use_tree_routing;
    }
        ...
        conf->attribute_length = sizeof(zb_uint16_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_nlme_get_confirm_t)) = ZG->nwk.nib.nwk_manager_addr;
    }
        ...
        *(((zb_uint8_t *)conf) +
        ...
        conf->attribute_length = sizeof(zb_uint8_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_nlme_get_confirm_t)) = ZG->nwk.nib.security_level;
    }
        ...
        conf->attribute_length = sizeof(zb_uint8_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_nlme_get_confirm_t)) = ZG->nwk.nib.active_secur_material_i;
    }
        ...
        conf->attribute_length = sizeof(zb_uint8_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_nlme_get_confirm_t)) = ZG->nwk.nib.active_key_seq_number;
    }
#endif
#ifdef ZB_SECURITY
        ...
        conf->attribute_length = sizeof(zb_uint8_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_nlme_get_confirm_t)) = ZG->nwk.nib.secure_all_frames;
    }
#endif
    else {
...
void zb_nlme_get_confirm(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if ((conf->nib_attribute == ZB_NIB_ATTRIBUTE_SEQUENCE_NUMBER)
        ...
        TRACE_MSG(TRACE_NWK2, "attr %hd get param %hd",
                  (FMT__H_H, conf->nib_attribute,
                   *(((zb_uint8_t *)conf) + sizeof(zb_nlme_get_confirm_t))));
    }
    TRACE_MSG(TRACE_NWK1, "<<zb_nlme_get_confirm status %hd",
              (FMT__H, conf->status));
    zb_free_buf(buf);
}
...
void zb_nlme_set_confirm(zb_uint8_t param) ZB_CALLBACK
{
    ...
    {
        zb_nlme_set_confirm_t *conf =
            ( zb_nlme_set_confirm_t *)ZB_BUF_BEGIN(buf);
        TRACE_MSG(TRACE_NWK1, "<<zb_nlme_set_confirm status %hd",
                  (FMT__H, conf->status));
    }
#endif
    zb_free_buf(buf);
...
void zb_nlme_send_status(zb_uint8_t param) ZB_CALLBACK
{
    ...
    zb_nlme_send_status_t *request =
        ZB_GET_BUF_PARAM(buf, zb_nlme_send_status_t);
    zb_nlme_status_indication_t *status_cmd;

    TRACE_MSG(TRACE_NWK1, ">> zb_nlme_send_status param %hd", (FMT__H, param));

    nwk_alloc_and_fill_hdr(buf, request->dest_addr, NULL, NULL, ZB_FALSE,
                           ZB_FALSE, ZB_TRUE);
    status_cmd = (zb_nlme_status_indication_t *)nwk_alloc_and_fill_cmd(buf,
