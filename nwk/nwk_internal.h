...
#define NWK_FORMATION_FAILURE_CONFIRM(buf, s)                           \
    ...
    {                                                                       \
        (buf)->u.hdr.status = (s);                                            \
        ZB_SCHEDULE_CALLBACK(zb_nlme_network_formation_confirm, \
                             ZB_REF_FROM_BUF((buf))); \
    }                                                                       \
    while (0)
...
#define NWK_ROUTE_DISCOVERY_CONFIRM(buf, s, ns)                         \
    ...
    {                                                                       \
        ...
        ZB_BUF_INITIAL_ALLOC(buf, sizeof(zb_nlme_route_discovery_confirm_t), \
                             ptr); \
        ZB_ASSERT(ptr);                                                       \
        (buf)->u.hdr.status = (zb_ret_t)(s);                                  \
        ptr->status = ns;                                                     \
        ZB_SCHEDULE_CALLBACK((zb_nlme_route_discovery_confirm), \
                             ZB_REF_FROM_BUF((buf))); \
    }                                                                       \
    while (0)
...
#define NWK_ARRAY_FIND_ENT(array, ent, condition) do                    \
    {                                                                       \
        zb_ushort_t i;                                                           \
        for (i = 0, ent = &(array)[0]; i < sizeof(array) / sizeof((array)[0]); \
             i++, ent = &(array)[i]) \
        {                                                                     \
            if (ent->used                                                      \
                && (condition))                                               \
 ...
 */
#define ZB_NWK_DISTRIBUTED_ADDRESS_ASSIGN_ROUTER_ADDRESS() (ZG->mac.pib. \
                                                            mac_short_address + \
                                                            ZG->nwk.nib. \
                                                            router_child_num * \
                                                            ZG->nwk.nib.cskip + \
 ...
 */
#define ZB_NWK_DISTRIBUTED_ADDRESS_ASSIGN_ED_ADDRESS() (ZG->mac.pib. \
                                                        mac_short_address + \
                                                        ZG->nwk.nib.max_routers \
                                                        * ZG->nwk.nib.cskip + \
...
#define ZB_NWK_ROUTER_ADDRESS_ASSIGN \
    ZB_NWK_DISTRIBUTED_ADDRESS_ASSIGN_ROUTER_ADDRESS
...
/**
   ...
   @return nothing
 */
void zb_nwk_mesh_route_discovery(zb_buf_t *cbuf, zb_uint16_t dest_addr,
                                 zb_uint8_t radius) ZB_SDCC_REENTRANT;
...
/**
   ...
   @return nothing
 */
void zb_nwk_mesh_rreq_handler(zb_buf_t *buf, zb_nwk_hdr_t *nwk_hdr,
                              zb_nwk_cmd_rreq_t *nwk_cmd_rreq) ZB_SDCC_REENTRANT;
...
/**
   ...
   @return nothing
 */
void zb_nwk_mesh_rrep_handler(zb_buf_t *buf, zb_nwk_hdr_t *nwk_hdr,
                              zb_nwk_cmd_rrep_t *nwk_cmd_rrep) ZB_SDCC_REENTRANT;
 ...
 */
zb_nwk_route_discovery_t *zb_nwk_mesh_find_route_discovery_entry(
    zb_uint16_t dest_addr) ZB_SDCC_REENTRANT;
 ...
 */
zb_ret_t zb_nwk_mesh_add_buf_to_pending(zb_buf_t *buf,
                                        zb_uint8_t handle) ZB_SDCC_REENTRANT;
