...
void zb_nlme_permit_joining_request(zb_uint8_t param) ZB_CALLBACK
{
    ...
    zb_nlme_permit_joining_request_t *request = ZB_GET_BUF_PARAM(buf,
                                                                 zb_nlme_permit_joining_request_t);

    TRACE_MSG(TRACE_NWK1, ">>zb_nlme_permit_joining_request %hd", (FMT__H,
                                                                   param));
    CHECK_PARAM_RET_ON_ERROR(request);

    TRACE_MSG(TRACE_NWK1, "permit join %d device type %d",
              (FMT__D_D, (int)ZG->nwk.handle.permit_join,
               (int)ZB_NIB_DEVICE_TYPE()));
    ...
    if (ZB_NIB_DEVICE_TYPE() == ZB_NWK_DEVICE_TYPE_COORDINATOR
        ...
        TRACE_MSG(TRACE_NWK1, "permit duration %d",
                  (FMT__D, (int)request->permit_duration));
...
//                 break;
            ...
            default:
                ...
                MAC_PIB().mac_association_permit = 1;
                ZB_SCHEDULE_ALARM(nwk_permit_timeout, 0,
                                  request->permit_duration *
                                  ZB_TIME_ONE_SECOND);
                break;
        }
