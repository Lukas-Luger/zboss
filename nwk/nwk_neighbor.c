...
static zb_ret_t alloc_new_extneiboard(zb_address_pan_id_ref_t panid_ref,
...
void zb_nwk_exneighbor_start() ZB_SDCC_REENTRANT
{
        ...
        for (i = 0;
            ...
            if (ZG->nwk.neighbor.base_neighbor[i].used) {
                ...
                if (hole) {
                    ...
                    ZB_MEMCPY(&ZG->nwk.neighbor.base_neighbor[i - hole],
                              &ZG->nwk.neighbor.base_neighbor[i],
                              sizeof(zb_neighbor_tbl_ent_t));
                    ZG->nwk.neighbor.addr_to_neighbor[ZG->nwk.neighbor.
                                                      base_neighbor[i].addr_ref]
                        = i - hole;
                    ZG->nwk.neighbor.base_neighbor[i].used = 0;
                    hole--;
                    TRACE_MSG(TRACE_NWK3, "i %d: new used %d, hole %d",
                              (FMT__D_D_D, i, used, hole));
                }
                else {
                    TRACE_MSG(TRACE_NWK3, "i %d: new used %d",
                              (FMT__D_D, i, used));
                }
            }
            else {
    ...
    if (ZG->nwk.neighbor.base_neighbor_used >
        ZG->nwk.neighbor.base_neighbor_size - cut) {
        cut = ZG->nwk.neighbor.base_neighbor_size -
              ZG->nwk.neighbor.base_neighbor_used;
        TRACE_MSG(TRACE_NWK3, "cut %d", (FMT__D, cut));
    }
    ZG->nwk.neighbor.base_neighbor_size -= cut;
    TRACE_MSG(TRACE_NWK3, "new base n size %d",
              (FMT__D, (int)ZG->nwk.neighbor.base_neighbor_size));

    {
        /* let's align to 4 */
        zb_uint8_t off = MAGIC_ROUND_TO_4(
            ZG->nwk.neighbor.base_neighbor_size *
            sizeof(zb_neighbor_tbl_ent_t));
        cut = cut * sizeof(zb_neighbor_tbl_ent_t) -
              (off - ZG->nwk.neighbor.base_neighbor_size *
               sizeof(zb_neighbor_tbl_ent_t));
        ZG->nwk.neighbor.ext_neighbor =
            (zb_ext_neighbor_tbl_ent_t *)(((char *)(&ZG->nwk.neighbor.
                                                    base_neighbor[
                                                        0])) + off);
        ZG->nwk.neighbor.ext_neighbor_size = cut /
                                             sizeof(zb_ext_neighbor_tbl_ent_t);
        TRACE_MSG(TRACE_NWK3, "cut %d, off %d, ext n size %d ext n %p",
                  (FMT__D_D_D_P,
                   (int)
                   cut, (int)off, (int)ZG->nwk.neighbor.ext_neighbor_size,
                   ZG->nwk.neighbor.ext_neighbor));
    }
...
void zb_nwk_exneighbor_stop(zb_uint16_t parent_short_addr) ZB_SDCC_REENTRANT
{
    ...
    if (ZG->nwk.handle.joined) {
        ...
        if (zb_address_get_pan_id_ref(ZB_NIB_EXT_PAN_ID(),
                                      &my_panid_ref) != RET_OK) {
            /* Must extsts already!! */
            TRACE_MSG(TRACE_NWK1,
                      "PAN id " TRACE_FORMAT_64 " not in panids arr - ?",
                      (FMT__A,
                       TRACE_ARG_64(
                           ZB_NIB_EXT_PAN_ID())));
        }

        TRACE_MSG(TRACE_NWK3, "ext n used %d",
                  (FMT__D, ZG->nwk.neighbor.ext_neighbor_used));
        for (i = 0; i < ZG->nwk.neighbor.ext_neighbor_used; ++i) {
            ...
            if (ZG->nwk.neighbor.ext_neighbor[i].panid_ref == my_panid_ref) {
                ...
                if (ZG->nwk.neighbor.ext_neighbor[i].short_addr !=
                    (zb_uint16_t) ~0) {
                    if (!ZB_ADDRESS_COMPRESED_IS_ZERO(ZG->nwk.neighbor.
                                                      ext_neighbor[i].long_addr))
                    {
                        /* both addresses are known */
                        zb_ieee_addr_decompress(long_address,
                                                &ZG->nwk.neighbor.ext_neighbor[i].long_addr);
                        ret = zb_address_update(long_address,
                                                ZG->nwk.neighbor.ext_neighbor[i].short_addr,
                                                ZB_FALSE, &addr_ref); /* don't lock address: keep locked */
                        TRACE_MSG(TRACE_NWK3, "both addr, ret %d, ref %d",
                                  (FMT__D_D, ret, addr_ref));
                    }
                    else {
                        /* have only short */
                        ret =
                        ...
                        TRACE_MSG(TRACE_NWK3, "short addr, ret %d, ref %d",
                                  (FMT__D_D, ret, addr_ref));
                    }
                }
                else {
                    ...
                    zb_ieee_addr_decompress(long_address,
                                            &ZG->nwk.neighbor.ext_neighbor[i].long_addr);
                    ret = zb_address_by_ieee(long_address, ZB_TRUE, ZB_FALSE,
                                             &addr_ref);
                    TRACE_MSG(TRACE_NWK3, "long addr, ret %d, ref %d",
                              (FMT__D_D, ret, addr_ref));
                }

                if (ret == RET_OK) {
                    if (ZG->nwk.neighbor.addr_to_neighbor[addr_ref] !=
                        (zb_uint8_t) ~0) {
                        /* entry exists - update */
                        ent =
                            &ZG->nwk.neighbor.base_neighbor[ZG->nwk.neighbor.
                                                            addr_to_neighbor
                                                            [addr_ref]];
                        TRACE_MSG(TRACE_NWK3, "upd ent %p for a_ref %d",
                                  (FMT__P_D, ent, addr_ref));
                        ZB_ASSERT(ent->used);
                        if (ent->device_type == ZB_NWK_DEVICE_TYPE_NONE) {
                            ent->device_type =
                                ZG->nwk.neighbor.ext_neighbor[i].device_type;
                            TRACE_MSG(TRACE_NWK3, "set dev_type %d",
                                      (FMT__D, ent->device_type));
                        }
                    }
                    else {
                        if (ZG->nwk.neighbor.base_neighbor_used ==
                            ...
                            base_neighbor_cleanup();
                            TRACE_MSG(TRACE_NWK3, "after clean sz %d, used %d",
                                      (FMT__D_D,
                                       ZG
                                       ->nwk.neighbor.base_neighbor_size,
                                       ZG->nwk.neighbor.base_neighbor_used));
                        }
                        if (ZG->nwk.neighbor.base_neighbor_used <
                            ZG->nwk.neighbor.base_neighbor_size) {
                            ...
                            ZG->nwk.neighbor.addr_to_neighbor[addr_ref] =
                                ZG->nwk.neighbor.base_neighbor_used;
                            ZG->nwk.neighbor.base_neighbor_used++;
                            ...
                            ent->device_type =
                                ZG->nwk.neighbor.ext_neighbor[i].device_type;
                            ent->rx_on_when_idle =
                                (ent->device_type != ZB_NWK_DEVICE_TYPE_ED);
                            ent->depth = ZG->nwk.neighbor.ext_neighbor[i].depth;
                            ent->permit_joining =
                                ZG->nwk.neighbor.ext_neighbor[i].permit_joining;
                            TRACE_MSG(TRACE_NWK3,
                                      "new ent %p, used %d, r.w.i %d, dev_t %d",
                                      (FMT__P_D_D_D,
                                       ent,
                                       ZG->nwk.neighbor.base_neighbor_used,
                                       ent->rx_on_when_idle, ent->device_type));
                            /* relationship is unknown. */
                            ent->relationship =
                                ZB_NWK_RELATIONSHIP_NONE_OF_THE_ABOVE;
                        }
                        else {
                            /* no space available! */
                            TRACE_MSG(TRACE_ERROR, "No spc for base ent!",
                                      (FMT__0));
                        }
                    }
                    if (ent) {
            ...
            ZG->nwk.neighbor.base_neighbor_size += free_size /
                                                   sizeof(zb_neighbor_tbl_ent_t);
            free_size -= free_size / sizeof(zb_neighbor_tbl_ent_t) *
                         sizeof(zb_neighbor_tbl_ent_t);
    ...
    TRACE_MSG(TRACE_NWK3, "restore base n size %d",
              (FMT__D, (int)ZG->nwk.neighbor.base_neighbor_size));
    TRACE_MSG(TRACE_NWK3, "base_neighbor_used %d",
              (FMT__D, (int)ZG->nwk.neighbor.base_neighbor_used));
...
void zb_nwk_exneighbor_stop(zb_uint16_t parent_short_addr) ZB_SDCC_REENTRANT
{
    ...
    if (ZG->nwk.handle.joined
        ...
        && zb_address_by_short(parent_short_addr, ZB_TRUE, ZB_FALSE,
                               &addr_ref) == RET_OK) {
        ...
        for (i = 0; i < ZG->nwk.neighbor.ext_neighbor_used; ++i) {
            ...
            if (eent->short_addr == parent_short_addr
                ...
                TRACE_MSG(TRACE_NWK3, "new ent %p, used %d, r.w.i %d, dev_t %d",
                          (FMT__P_D_D_D,
                           ent,
                           ZG->nwk.neighbor.base_neighbor_used,
                           ent->rx_on_when_idle,
...
zb_ret_t zb_nwk_exneighbor_by_short(zb_address_pan_id_ref_t panid_ref,
                                    zb_uint16_t short_addr,
                                    zb_ext_neighbor_tbl_ent_t **enbt)
ZB_SDCC_REENTRANT
{
    ...
    TRACE_MSG(TRACE_NWK1, ">>exnb_by_short pan %d, addr %d, enbt %p",
              (FMT__D_D_P,
               (int)
...
zb_ret_t zb_nwk_exneighbor_by_ieee(zb_address_pan_id_ref_t panid_ref,
...
ZB_SDCC_REENTRANT
{
    ...
    for (i = 0; i < ZG->nwk.neighbor.ext_neighbor_used; ++i) {
        if (ZG->nwk.neighbor.ext_neighbor[i].panid_ref == panid_ref
            && ZB_ADDRESS_COMPRESSED_CMP(ZG->nwk.neighbor.ext_neighbor[i].
                                         long_addr, compressed_addr)) {
            TRACE_MSG(TRACE_NWK3, "found ent # %d", (FMT__D, i));
...
ZB_SDCC_REENTRANT
{
    ...
    TRACE_MSG(TRACE_NWK1, ">>alloc_new_extnb pan %d addr %d enbt %p",
              (FMT__D_D_P,
               panid_ref,
               short_addr, enbt));

    if (ZG->nwk.neighbor.ext_neighbor_used ==
        ...
        TRACE_MSG(TRACE_NWK3, "allocated ext ent %p new used %hd",
                  (FMT__P_D, en, ZG->nwk.neighbor.ext_neighbor_used));
...
zb_ret_t zb_nwk_neighbor_get(zb_address_ieee_ref_t addr,
                             zb_bool_t create_if_absent,
                             zb_neighbor_tbl_ent_t **nbt) ZB_SDCC_REENTRANT
{
    zb_ret_t ret = RET_OK;
    zb_uint8_t n = ZG->nwk.neighbor.addr_to_neighbor[addr];

    TRACE_MSG(TRACE_NWK1, ">>zb_nwk_neighbor_get addr_ref %hd cr %hd nbt %p",
              (FMT__H_H_P,
               addr,
               create_if_absent, nbt));
    ...
    if (n == (zb_uint8_t)-1) {
        if (create_if_absent) {
            if (ZG->nwk.neighbor.base_neighbor_used ==
                ...
                ZG->nwk.neighbor.base_neighbor_used++;
                {
                    zb_neighbor_tbl_ent_t *bn =
                        &ZG->nwk.neighbor.base_neighbor[n];
...
ZB_SDCC_REENTRANT
{
...
#endif
    {
        ...
        zb_neighbor_tbl_ent_t *ent =
            &ZG->nwk.neighbor.base_neighbor[ZG->nwk.neighbor.base_neighbor_used];
#endif
        ...
        TRACE_MSG(TRACE_NWK3, "new ent %p, used %d, r.w.i %d, dev_t %d",
                  (FMT__P_D_D_D,
                   ent,
                   ZG->nwk.neighbor.base_neighbor_used, ent->rx_on_when_idle,
                   ent->device_type));
        /* relationship is unknown. */
        ent->relationship = ZB_NWK_RELATIONSHIP_NONE_OF_THE_ABOVE;
        ent->used = 1;
...
zb_ret_t zb_nwk_neighbor_get_by_short(zb_uint16_t short_addr,
...
zb_ret_t zb_nwk_neighbor_get_by_ieee(zb_ieee_addr_t long_addr,
                                     zb_neighbor_tbl_ent_t **nbt)
{
    zb_address_ieee_ref_t addr_ref;
...
#define I 0
#endif
        zb_int16_t diff = key_seq_number -
                          ZG->nwk.neighbor.base_neighbor[I].key_seq_number;
...
zb_ushort_t zb_nwk_neighbor_next_rx_on_i(zb_ushort_t i)
{
    while (i < ZG->nwk.neighbor.base_neighbor_used
        ...
        TRACE_MSG(TRACE_SECUR2,
                  "skip nb ent %hd addr_ref %hd used %hd rel %hd dev_t %hd rx_on %hd",
                  (FMT__H_P_H_H_H_H,
    ...
    TRACE_MSG(TRACE_SECUR2, "<< zb_nwk_neighbor_next_ze_children_i next ze %hd",
              (FMT__H, i));
    return i;
}
