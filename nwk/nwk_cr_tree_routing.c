 ...
 */
static zb_neighbor_tbl_ent_t *zb_nwk_tree_routing_find_router(
    zb_uint16_t dest_address) ZB_SDCC_REENTRANT
{
    ...
    TRACE_MSG(TRACE_NWK1, ">>zb_nwk_tree_routing_find_router dest_address %d",
              (FMT__D, dest_address));

    /* try to find exact address first */
    if (zb_address_by_short(dest_address, ZB_FALSE, ZB_FALSE,
                            &ieee_ref) == RET_OK
        && ZG->nwk.neighbor.addr_to_neighbor[ieee_ref] != (zb_uint8_t)-1) {
        ret =
            &ZG->nwk.neighbor.base_neighbor[ ZG->nwk.neighbor.addr_to_neighbor[
                                                 ieee_ref] ];
    }
    ...
    if (!ret) {
        ...
        for (i = 0; i < ZG->nwk.nib.max_routers; i++) {
            ...
            if (dest_address > lower_address
                ...
                if (zb_address_by_short(lower_address, ZB_FALSE, ZB_FALSE,
                                        &ieee_ref) == RET_OK
                    && ZG->nwk.neighbor.addr_to_neighbor[ieee_ref] !=
                    (zb_uint8_t)-1) {
                    ret =
    ...
    TRACE_MSG(TRACE_NWK1, "<<zb_nwk_tree_routing_find_router %p",
              (FMT__P, ret));
    return ret;
}
...
ZB_SDCC_REENTRANT
{
    ...
    TRACE_MSG(TRACE_NWK1, ">>zb_nwk_tree_routing_route dest_address %d",
              (FMT__D, dest_address));
    ...
    cskip_parent = (ZG->nwk.nib.depth) ? zb_nwk_daa_calc_cskip(
        ZG->nwk.nib.depth - 1) : 0;

    /* Route direction */
    route_down =
        ((dest_address > ZG->mac.pib.mac_short_address) &&
         (dest_address < ZG->mac.pib.mac_short_address + cskip_parent));
