...
static zb_bool_t ieee_search(zb_ieee_addr_compressed_t *ieee_compressed,
                             zb_address_ieee_ref_t *ref_p) ZB_SDCC_REENTRANT;

static zb_bool_t short_search(zb_uint16_t short_addr,
                              zb_address_ieee_ref_t *ref_p) ZB_SDCC_REENTRANT;
...
static void add_short_sorted(zb_address_ieee_ref_t ref,
                             zb_uint16_t short_addr) ZB_SDCC_REENTRANT;

static void clock_tick() ZB_SDCC_REENTRANT;


static zb_ushort_t zb_check_bit_in_bit_vector(zb_uint8_t *v,
                                              zb_ushort_t b) ZB_SDCC_REENTRANT
{
    return ZB_CHECK_BIT_IN_BIT_VECTOR(v, b);
}

static void zb_set_bit_in_bit_vector(zb_uint8_t *v,
                                     zb_ushort_t b) ZB_SDCC_REENTRANT
{
    ZB_SET_BIT_IN_BIT_VECTOR(v, b);
}

#define ZB_ADDRESS_PAN_ID_HASH(pan_id) (ZB_4INT_HASH_FUNC((int)pan_id[0], \
                                                          (int)pan_id[2], \
                                                          (int)pan_id[4], \
                                                          (int)pan_id[6]) % \
                                        ZB_PANID_TABLE_SIZE)
...
zb_ret_t zb_address_set_pan_id(zb_uint16_t short_pan_id, zb_ext_pan_id_t pan_id,
                               zb_address_pan_id_ref_t *ref) ZB_CALLBACK
{
    zb_ret_t ret = RET_OK;
    ...
    TRACE_MSG(TRACE_COMMON1,
              ">>zb_address_add_pan_id short_pan_id %d pan_id " TRACE_FORMAT_64 " ref %p",
              (FMT__D_A_P,
               (
                   int)short_pan_id, TRACE_ARG_64(pan_id), ref));

    /* try to find pan id first */
    TRACE_MSG(TRACE_COMMON3, "start search pan_id reference from h_i %hd",
              (FMT__H, h_i));
    i = h_i;
    do {
        if (zb_check_bit_in_bit_vector(ZG->addr.used_pan_addr, i)) {
            if (ZB_EXTPANID_CMP(ZG->addr.pan_map[i].long_panid, pan_id)) {
                TRACE_MSG(TRACE_COMMON1, "pan_id reference found %d",
                          (FMT__D, (int)i));
                found = ZB_TRUE;
                break;
            }
    ...
    if (found) {
        *ref = i;
        ...
        if (free_i != (zb_address_pan_id_ref_t)(-1)) {
            TRACE_MSG(TRACE_COMMON1, "pan_id ref after find free place %d",
                      (FMT__D, (int)free_i));
...
void zb_address_get_pan_id(zb_address_pan_id_ref_t pan_id_ref,
                           zb_ext_pan_id_t pan_id) ZB_CALLBACK
{
    TRACE_MSG(TRACE_COMMON1,
              ">>zb_address_get_pan_id pan_id_reference %d pan_id %p",
              (FMT__D_P, (int)pan_id_ref, pan_id));

    ZB_ASSERT( pan_id_ref < ZB_PANID_TABLE_SIZE
               && zb_check_bit_in_bit_vector(ZG->addr.used_pan_addr,
                                             pan_id_ref));
                                   ...
                                   zb_address_pan_id_ref_t *ref) ZB_CALLBACK
{
    ...
    TRACE_MSG(TRACE_COMMON1,
              ">>zb_address_get_pan_id_ref pan_id " TRACE_FORMAT_64 " ref %p",
              (FMT__A_P, TRACE_ARG_64(pan_id), ref));
...
void zb_address_get_short_pan_id(zb_address_pan_id_ref_t pan_id_ref,
                                 zb_uint16_t *pan_id_p) ZB_CALLBACK
{
    TRACE_MSG(TRACE_COMMON1,
              ">>zb_address_get_pan_id pan_id_reference %d pan_id %p",
              (FMT__D_P, (int)pan_id_ref, pan_id_p));
                                       ...
                                       zb_ext_pan_id_t pan_id) ZB_CALLBACK
{
    ...
    TRACE_MSG(TRACE_COMMON1,
              ">>zb_address_cmp_pan_id_by_ref pan_id_reference %d pan_id %p",
              (FMT__D_P, (int)pan_id_ref, pan_id));
    ...
    TRACE_MSG(TRACE_COMMON1, "<<zb_address_cmp_pan_id_by_ref %d",
              (FMT__D, ret));
    return ret;
}
...
void zb_ieee_addr_compress(zb_ieee_addr_t address,
                           zb_ieee_addr_compressed_t *compressed_address)
ZB_CALLBACK
{
    ...
    for (i = 0; i < ZB_DEV_MANUFACTORER_TABLE_SIZE; i++) {
        if (zb_check_bit_in_bit_vector(ZG->addr.used_manufacturer, i)) {
            if (!ZB_MEMCMP(&ZG->addr.dev_manufacturer[i].device_manufacturer[0],
                           &address[0], 3)) {
                break;
            }
        }
    ...
    if (i != ZB_DEV_MANUFACTORER_TABLE_SIZE) {
        ...
        ZB_MEMCPY(&ZG->addr.dev_manufacturer[i].device_manufacturer[0],
                  &address[0], 3);
...
void zb_ieee_addr_decompress(zb_ieee_addr_t address,
                             zb_ieee_addr_compressed_t *compressed_address)
ZB_CALLBACK
{
...
static zb_address_ieee_ref_t zb_address_compressed_ieee_hash(
    zb_ieee_addr_compressed_t *ieee_compressed) ZB_SDCC_REENTRANT
{
    return ZB_ADDRESS_COMPRESSED_IEEE_HASH_MACRO(ieee_compressed);
}

zb_ret_t zb_address_by_ieee(zb_ieee_addr_t ieee, zb_bool_t create,
                            zb_bool_t lock,
                            zb_address_ieee_ref_t *ref_p) ZB_CALLBACK
{
    zb_ret_t ret = RET_OK;
    ...
    if (ieee_search(&ieee_compressed, ref_p)) {
        ...
        ret =
            (create) ? addr_add(&ieee_compressed, (zb_uint16_t) ~0,
                                ref_p) : RET_NOT_FOUND;
    }
...
zb_ret_t zb_address_by_short(zb_uint16_t short_address, zb_bool_t create,
                             zb_bool_t lock,
                             zb_address_ieee_ref_t *ref_p) ZB_CALLBACK
{
    zb_ret_t ret = RET_OK;
    ...
    TRACE_MSG(TRACE_NWK1, "zb_address_by_short %d ret %hd ref %hd",
              (FMT__D_H, short_address, ret, *ref_p));
    return ret;
}


zb_ret_t zb_address_update(zb_ieee_addr_t ieee_address,
                           zb_uint16_t short_address, zb_bool_t lock,
                           zb_address_ieee_ref_t *ref_p) ZB_CALLBACK
{
    zb_ret_t ret = RET_OK;
    ...
    TRACE_MSG(TRACE_COMMON2,
              "zb_address_update short 0x%x long " TRACE_FORMAT_64 " ref %hd ret %d",
...
void zb_address_by_ref(zb_ieee_addr_t ieee_address,
                       ...
                       zb_address_ieee_ref_t ref) ZB_CALLBACK
{
...
zb_ret_t zb_address_ieee_by_short(zb_uint16_t short_addr,
                                  zb_ieee_addr_t ieee_address) ZB_CALLBACK
{
    zb_address_ieee_ref_t ref;
...
void zb_address_lock(zb_address_ieee_ref_t ref) ZB_CALLBACK
{
    ...
    TRACE_MSG(TRACE_COMMON1, ">>zb_address_ieee_lock ref %d",
              (FMT__D, (int)ref));
...
{
#ifndef ZB_LIMITED_FEATURES
    TRACE_MSG(TRACE_COMMON1, ">>zb_address_ieee_unlock ref %d",
              (FMT__D, (int)ref));
                             ...
                             zb_address_ieee_ref_t *ref_p) ZB_SDCC_REENTRANT
{
    ...
    for (cnt = 0; cnt < ZB_IEEE_ADDR_TABLE_SIZE;
         ++cnt, i = (i + 1) % ZB_IEEE_ADDR_TABLE_SIZE) {
        if (ZG->addr.addr_map[i].used
            && ZB_ADDRESS_COMPRESSED_CMP(ZG->addr.addr_map[i].ieee_addr,
                                         *ieee_compressed)) {
            TRACE_MSG(TRACE_COMMON1, "ieee reference found %d",
                      (FMT__D, (int)i));
            *ref_p = i;
            return ZB_TRUE;
        }
...
static zb_ret_t addr_add(zb_ieee_addr_compressed_t *ieee_compressed,
                         ...
                         zb_address_ieee_ref_t *ref_p) ZB_SDCC_REENTRANT
{
    zb_address_ieee_ref_t i = ieee_compressed ? zb_address_compressed_ieee_hash(
        ieee_compressed) : 0;
    ...
    for (i = 0; i < ZG->addr.n_sorted_elements; ++i) {
        if (ZG->addr.short_sorted[i] == ref) {
            ...
            if (i != ZG->addr.n_sorted_elements - 1) {
                TRACE_MSG(TRACE_NWK3, "move to %hd %hd %hd",
                          (FMT__H_H_H, i, i + 1,
                           ZG->addr.n_sorted_elements - i));
                ZB_MEMMOVE(&ZG->addr.short_sorted[i],
                           &ZG->addr.short_sorted[i + 1],
                           ZG->addr.n_sorted_elements - i);
            }
        }
    }
                             ...
                             zb_uint16_t short_addr) ZB_SDCC_REENTRANT
{
    ...
    if (ZG->addr.n_sorted_elements > 1) {
        ...
        while (l < r) {
            ...
            if (short_addr >=
                ZG->addr.addr_map[ZG->addr.short_sorted[i]].addr) {
                l = i + 1;
            }
            else {
                r = i;
    ...
    if (l < ZG->addr.n_sorted_elements) {
        ZB_MEMMOVE(&ZG->addr.short_sorted[l + 1], &ZG->addr.short_sorted[l],
                   ZG->addr.n_sorted_elements - l);
    }
...
static void clock_tick() ZB_SDCC_REENTRANT
{
    ...
    zb_ushort_t cnt = 0;
    zb_bool_t found = ZB_FALSE;
    while (ZG->addr.clock_i < ZB_IEEE_ADDR_TABLE_SIZE &&
           cnt < ZG->addr.n_elements) {
...
zb_bool_t zb_address_compressed_cmp(zb_ieee_addr_compressed_t *one,
                                    zb_ieee_addr_compressed_t *two) ZB_CALLBACK
{
    return (zb_bool_t)((one)->dev_manufacturer == (two)->dev_manufacturer
                       && !ZB_MEMCMP(&(one)->device_id[0], &(two)->device_id[0],
                                     5));
}
