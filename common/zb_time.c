...
void zb_timer_start(zb_time_t timeout)
{
    ...
    TRACE_MSG(TRACE_OSIF3, "tmo %d, stop at %d; t 0x%x hi 0x%x lo 0x%x freq %d",
              (FMT__D_D_D_D_D_D, (int)timeout, (int)ZB_TIMER_CTX().timer_stop,
               (zb_uint16_t)ZB_8051_TIMER_VALUE, (zb_uint16_t)ZB_TIMER_HI_BYTE,
               (zb_uint16_t)ZB_TIMER_LOW_BYTE, (int)ZB_SHORT_XTAL_FREQ));
#else
    TRACE_MSG(TRACE_OSIF3, "tmo %d, stop at %d",
              (FMT__D_D, (int)timeout, (int)ZB_TIMER_CTX().timer_stop));
#endif
#endif
}
