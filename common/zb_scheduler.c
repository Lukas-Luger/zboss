    ...
    do {
        ...
        /* First execute regular (immediate) callbacks */
        {
            ...
            while ((ent = ZB_RING_BUFFER_PEEK(&ZG->sched.cb_q)) != NULL) {
                TRACE_MSG(TRACE_COMMON3,
                          "cb_q written %hd, read_i %hd, write_i %hd",
                          (FMT__H_H_H, ZG->sched.cb_q.written,
                           ZG->sched.cb_q.read_i,
                           ZG->sched.cb_q.write_i));
                did_something = 1;
                if ((ZB_GET_TRANS_INT()) && ZB_IN_BUF_AVAILABLE()) {
                    break;
                }
                TRACE_MSG(TRACE_COMMON2,
                          "%p calling cb %p param %hd in_b %hd len %hd",
                          (FMT__P_P_H_H, ent, ent->func, ent->param,
                           ent->param ? ZB_BUF_FROM_REF(ent->param)->u.hdr.
                           is_in_buf : (zb_uint8_t)-1,
                           ent->param ? ZB_BUF_FROM_REF(ent->param)->u.hdr.len :
                           -1
                          ));
        ...
        if (ZG->sched.tm_queue != 0) {
            ...
            while ((ent = ZB_LIST_GET_HEAD(ZG->sched.tm_queue, next)) != NULL
                ...
                /* call the callback */
                TRACE_MSG(TRACE_COMMON2, "%p calling alarm cb %p param %hd",
        ...
        if (ZG->sched.tm_queue) {
            ZB_TIMER_START(ZB_TIME_SUBTRACT(ZB_LIST_GET_HEAD(ZG->sched.tm_queue,
                                                             next)->run_time,
                                            t));
        }
        else {
...
/* temprally disabled */
    ...
    if (((ZB_RING_BUFFER_IS_EMPTY(&MAC_CTX().mac_rx_queue)) &&
         !MAC_CTX().recv_buf_full) && !MAC_CTX().tx_wait_cb) {
        ZB_TRANS_GO_IDLE();
        ZB_GO_IDLE();
    }
#endif
                            ...
                            zb_uint8_t param) ZB_SDCC_REENTRANT
{
    ...
    if (ent) {
        ...
        TRACE_MSG(TRACE_ERROR, "Callbacks rb overflow! param %hd",
                  (FMT__H, param));
        ret = RET_OVERFLOW;
    }
    return ret;
...
zb_ret_t zb_schedule_alarm(zb_callback_t func, zb_uint8_t param,
                           zb_time_t run_after) ZB_SDCC_REENTRANT                                       /* __reentrant for sdcc, to save DSEG space */
{
    zb_ret_t ret = RET_OK;
    zb_tm_q_ent_t *nent;
    zb_tm_q_ent_t *ent;
    ...
    if (nent != 0) {
        ...
        TRACE_MSG(TRACE_COMMON2,
                  "%p scheduled alarm %p, run_after %d, at %d, param %hd",
                  (FMT__P_P_D_D_H,
                   nent,
                   nent->func, (int)run_after,
                   (
                       int)nent->run_time, nent->param));

        if ((ZG->sched.tm_queue == 0)       /* queue is empty */
            || ZB_TIME_GE(
                ZB_LIST_GET_HEAD(ZG->sched.tm_queue, next)->run_time,
                nent->run_time)) {                                               /* new time is before
                                                                                  * queue start */
        ...
        ZB_LIST_ITERATE(ZG->sched.tm_queue, next, ent){
            TRACE_MSG(TRACE_COMMON3, "entry %p cb %p run_time %d",
                      (FMT__P_P_D, (void *)ent, ent->func, ent->run_time));
        }
#endif
        ...
        ent = ZB_LIST_GET_HEAD(ZG->sched.tm_queue, next);
        t = ZB_TIMER_GET();
        TRACE_MSG(TRACE_COMMON3, "cur t %d, head t %d",
                  (FMT__D_D, t, ent->run_time));
        if (ent->run_time != t
...
zb_ret_t zb_schedule_alarm_cancel(zb_callback_t func,
                                  zb_uint8_t param) ZB_SDCC_REENTRANT                     /* __reentrant for sdcc, to save DSEG space */
{
    ...
    for (ent = ZB_LIST_GET_HEAD(ZG->sched.tm_queue, next); ent; ent = next) {
        ...
        if (ent->func == func &&
            (param == ZB_ALARM_ANY_PARAM || param == ZB_ALARM_ALL_CB ||
             param == ent->param)) {
            TRACE_MSG(TRACE_COMMON3, "%p alarm cancel func %p, param %hd",
                      (FMT__P_P_H, ent, ent->func, ent->param));
            /* remove from scheduled alarms, add to free list */
            ZB_LIST_REMOVE(ZG->sched.tm_queue, next, ent);
            ZB_STK_PUSH(ZG->sched.tm_freelist, next, ent);
            TRACE_MSG(TRACE_INFO3, "tm_queue -> tm_freelist, ent %p",
                      (FMT__P, (void *)ent));
            ret = RET_OK;
