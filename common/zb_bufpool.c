...
void trace_bufs()
{
    ...
    for (p = ZG->bpool.head; p; p = p->u.next) {
        TRACE_MSG(TRACE_MAC1, "buf %p %hd next %p",
                  (FMT__P_H_P, p, ZB_REF_FROM_BUF(p), p->u.next));
    }
}
...
void verify_buf(zb_buf_t *b)
{
    ...
    for (p = ZG->bpool.head; p; p = p->u.next) {
        if (p == b) {
            TRACE_MSG(TRACE_MAC1, "ALREADY FREE buf %p %hd next %p",
                      (FMT__P_H_P, p, ZB_REF_FROM_BUF(p), p->u.next));
        }
    }
}
...
//     }
    ...
    TRACE_MSG( TRACE_MAC1,
               "zb_get_buf %hd: buffer %p, ref %hd, head %p, allocated %hd / %hd",
               (FMT__H_P_H_P_H, is_in, buf, ZB_REF_FROM_BUF(buf),
                ZG->bpool.head, ZG->bpool.bufs_allocated[0],
                ZG->bpool.bufs_allocated[1]));
#endif
...
void zb_free_buf(zb_buf_t *buf)
{
    ...
    TRACE_MSG(TRACE_NWK3,
              "zb_free_buf %p, ref %hd, in buf %hi allocated in %hd out %hd",
              (FMT__P_H_H_H_H, buf, ZB_REF_FROM_BUF(buf), buf->u.hdr,
               buf->u.hdr.is_in_buf,
                         ...
                         zb_uint8_t size) ZB_SDCC_REENTRANT
{
    ...
    if (ZB_IO_BUF_SIZE - (zbbuf->u.hdr.data_offset + zbbuf->u.hdr.len) < size) {
        zb_ushort_t delta = size -
                            (ZB_IO_BUF_SIZE -
                             (zbbuf->u.hdr.data_offset + zbbuf->u.hdr.len));
        zb_uint8_t *p = ZB_BUF_BEGIN(zbbuf);
...
{
    ZB_ASSERT((zbbuf) && ZB_BUF_LEN(
                  zbbuf) + (zbbuf)->u.hdr.data_offset + (size) <=
              ZB_IO_BUF_SIZE);
...
#ifdef ZB_DEBUG_BUFFERS
    TRACE_MSG(TRACE_NWK3,
              "zb_reuse_buf %p, ref %hd, in buf %hi allocated in %hd out %hd",
              (FMT__P_H_H_H_H, zbbuf, ZB_REF_FROM_BUF(zbbuf), zbbuf->u.hdr,
               zbbuf->u.hdr.is_in_buf,
               ZG->bpool.bufs_allocated[1], ZG->bpool.bufs_allocated[0]));
#endif
    ZB_BZERO(&zbbuf->u, sizeof(zbbuf->u));
    zbbuf->u.hdr.is_in_buf = is_in_buf;
...
zb_void_t temp_debug_dump_buf_hdr(zb_buf_t *buf)
{
    TRACE_MSG(TRACE_MAC2,
              "!!! buf %p, len %hd, data_offset %hd, handle %hd, mac_hdr_offset %hd, status %d",
              (FMT__P_H_H_H_H_D, (buf), (buf)->u.hdr.len,
               (buf)->u.hdr.data_offset,
               (buf)->u.hdr.handle, (buf)->u.hdr.mac_hdr_offset,
               (buf)->u.hdr.status));
}
#endif
