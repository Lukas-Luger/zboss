...
void zb_test_profile_indication(zb_uint8_t param) ZB_CALLBACK
{
    ...
    TRACE_MSG(TRACE_ZDO1, "zb_test_profile_indication %hd clu 0x%hx",
...
void tp_send_counted_packet(zb_uint8_t param) ZB_CALLBACK
{
    ...
    TRACE_MSG(TRACE_ZDO3, "send_counted_packet pack num %d, counter %d",
              (FMT__D_D,
               ZG->zdo.test_prof_ctx.tp_counted_packets.params.packets_number,
    ...
    if (ZG->zdo.test_prof_ctx.tp_counted_packets.params.packets_number >
        ...
        ZB_BUF_INITIAL_ALLOC(ZB_BUF_FROM_REF(
                             ...
                             ZG->zdo.test_prof_ctx.tp_counted_packets.params.len,
                             req);

        req->len = ZG->zdo.test_prof_ctx.tp_counted_packets.params.len +
                   sizeof(zb_uint16_t);                                                   /* data length + sequence counter length */
        req->counter = ZG->zdo.test_prof_ctx.tp_counted_packets.counter++;
        ZB_MEMSET((zb_uint8_t *)(req + 1), 0xAA,
                  ZG->zdo.test_prof_ctx.tp_counted_packets.params.len);                           /* put some test data */
        tp_send_req_by_short(TP_TRANSMIT_COUNTED_PACKETS_CLID, param,
                             ZG->zdo.test_prof_ctx.tp_counted_packets.params.dst_addr,
                             ZB_TEST_PROFILE_EP, ZB_TEST_PROFILE_EP,
                             /* ZB_APSDE_TX_OPT_ACK_TX */ 0);

        ZB_SCHEDULE_ALARM(tp_start_send_counted_packet, 0,
                          ZB_MILLISECONDS_TO_BEACON_INTERVAL(ZG->zdo.
                                                             test_prof_ctx.
                                                             tp_counted_packets.
                                                             params.idle_time));
    }
    else {
        ZB_SCHEDULE_CALLBACK(ZG->zdo.test_prof_ctx.tp_counted_packets.user_cb,
                             param);
    }
}
...
{
    zb_tp_transmit_counted_packets_param_t *params = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                                          param),
                                                                      zb_tp_transmit_counted_packets_param_t);

    TRACE_MSG(TRACE_ZDO3, "transmit_counted_packets_req param %hd",
              (FMT__H, param));

    ZB_MEMCPY(&ZG->zdo.test_prof_ctx.tp_counted_packets.params, params,
...
void tp_send_req_by_short(zb_uint16_t command_id, zb_uint8_t param,
                          zb_uint16_t addr, zb_uint8_t src_ep,
                          zb_uint8_t dst_ep,
...
void tp_send_req_by_EP(zb_uint16_t command_id, zb_uint8_t param,
                       zb_uint8_t src_ep, zb_uint8_t dst_ep,
                       zb_uint8_t tx_options) ZB_SDCC_REENTRANT
{
    zb_apsde_data_req_t *dreq =
        ZB_GET_BUF_TAIL(ZB_BUF_FROM_REF(param), sizeof(zb_apsde_data_req_t));
    ...
    TRACE_MSG(TRACE_ZDO3,
              "tp_send_req_by_bind_addr param %hd, src_ep %hd, dst_ep %hd",
              (FMT__H_H_H, param, src_ep, dst_ep));
    ZB_BZERO(dreq, sizeof(*dreq));
...
#if 0                           /* not used */
void tp_send_req_by_EP_brdcast(zb_uint16_t command_id, zb_uint8_t param,
                               zb_uint8_t src_ep, zb_uint8_t dst_ep,
                               zb_uint8_t tx_options) ZB_SDCC_REENTRANT
{
    zb_apsde_data_req_t *dreq =
...
void buffer_test_req_timeout(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (ZG->zdo.test_prof_ctx.zb_tp_buffer_test_request.user_cb) {
        ZB_SCHEDULE_CALLBACK(
            ZG->zdo.test_prof_ctx.zb_tp_buffer_test_request.user_cb,
            ZB_TP_BUFFER_TEST_FAIL);
        ZG->zdo.test_prof_ctx.zb_tp_buffer_test_request.user_cb = NULL;
    }
}

/* 6.7	Buffer test request, CID=0x1c */
void zb_tp_buffer_test_request(zb_uint8_t param,
                               zb_callback_t cb) ZB_SDCC_REENTRANT
{
    zb_buf_t *buf = ZB_BUF_FROM_REF(param);
    zb_buffer_test_req_param_t *req_param = ZB_GET_BUF_PARAM(buf,
                                                             zb_buffer_test_req_param_t);
    zb_buffer_test_req_t *req;

    TRACE_MSG(TRACE_ZDO2, "zb_tp_buffer_test_request param %hd, len %hd",
              (FMT__H_H, param, req_param->len));
    ...
    TRACE_MSG(TRACE_ZDO3, "sched buffer_test_req_timeout, timeout %d",
              (FMT__D, 10 * ZB_TIME_ONE_SECOND));
    ...
    tp_send_req_by_short(TP_BUFFER_TEST_REQUEST_CLID, param,
                         req_param->dst_addr, req_param->src_ep,
                         req_param->dst_ep, 0);
}

/* 6.7	Buffer test request, CID=0x1c */
void zb_tp_buffer_test_request_EP(zb_uint8_t param,
                                  zb_callback_t cb) ZB_SDCC_REENTRANT
{
    zb_buf_t *buf = ZB_BUF_FROM_REF(param);
    zb_buffer_test_req_param_EP_t *req_param = ZB_GET_BUF_PARAM(buf,
                                                                zb_buffer_test_req_param_EP_t);
    zb_buffer_test_req_t *req;

    TRACE_MSG(TRACE_ZDO3, "zb_tp_buffer_test_request_EP param %hd, len %hd",
              (FMT__H_H, param, req_param->len));
    ZB_BUF_INITIAL_ALLOC(buf, sizeof(zb_buffer_test_req_t), req);
    req->len = req_param->len;

    ZG->zdo.test_prof_ctx.zb_tp_buffer_test_request.user_cb = cb;
    TRACE_MSG(TRACE_ZDO3, "zb_tp_buffer_test_request_EP, timeout %d",
              (FMT__D, 10 * ZB_TIME_ONE_SECOND));
    /* use callback to determine that response is not received */
    ZB_SCHEDULE_ALARM(buffer_test_req_timeout, 0, 30 * ZB_TIME_ONE_SECOND);

    TRACE_MSG(TRACE_ZDO3, "request_EP src ep %hd, dst ep %hd",
              (FMT__H_H, req_param->src_ep, req_param->dst_ep));
    tp_send_req_by_EP(TP_BUFFER_TEST_REQUEST_CLID, param, req_param->src_ep,
                      req_param->dst_ep, 0);
}
...
static void tp_buffer_test_request_handler(zb_uint8_t param) ZB_SDCC_REENTRANT
{
    ...
    TRACE_MSG(TRACE_ZDO3, "src ept %hd, dst ept %hd",
              (FMT__H_H, ind->src_endpoint, ind->dst_endpoint));
    ZB_APS_HDR_CUT(asdu);
...
void tp_buffer_test_response(zb_uint8_t param) ZB_SDCC_REENTRANT
{
    ...
    send_len =
        (test_param->status == ZB_TP_BUFFER_TEST_OK) ? test_param->len : 0;
    TRACE_MSG(TRACE_ZDO3, "len %hd, status %hd, send_len %hd",
              (FMT__H_H_H, test_param->len, test_param->status, send_len));
    ZB_BUF_INITIAL_ALLOC(buf, sizeof(zb_buffer_test_response_t) + send_len,
                         test_resp);
    TRACE_MSG(TRACE_ZDO3, "src ept resp %hd, dst ept resp%hd",
              (FMT__H_H, test_param->src_ep, test_param->dst_ep));
    ...
    tp_send_req_by_short(TP_BUFFER_TEST_RESPONSE_CLID, param,
                         test_param->dst_addr, test_param->src_ep,
                         test_param->dst_ep,
...
static void tp_buffer_test_response_handler(zb_uint8_t param) ZB_SDCC_REENTRANT
{
    ...
    TRACE_MSG(TRACE_ZDO2, "tp_buffer_test_response_handler %x",
              (FMT__D, param));
    ZB_APS_HDR_CUT(asdu);

    test_resp = (zb_buffer_test_response_t *)ZB_BUF_BEGIN(asdu);
    TRACE_MSG(TRACE_ZDO2, "status %hd, len %hd",
              (FMT__H_H, test_resp->status, test_resp->len));
...
void tp_packet_ack(zb_uint8_t param) ZB_SDCC_REENTRANT
{
    ...
    if (aps_hdr.clusterid == TP_BUFFER_TEST_RESPONSE_CLID) {
        if (asdu->u.hdr.status == 0) {
            ...
            test_resp = (zb_buffer_test_response_t *)ZB_BUF_BEGIN(asdu);
            test_param =
                ZB_GET_BUF_PARAM(asdu, zb_buffer_test_response_param_t);
        ...
        TRACE_MSG(TRACE_APS3, "unknown cluster id %x",
                  (FMT__D, aps_hdr.clusterid));
        zb_free_buf(asdu);
    }
