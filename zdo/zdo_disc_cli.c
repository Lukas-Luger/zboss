...
void zb_zdo_nwk_addr_req(zb_uint8_t param, zb_callback_t cb) ZB_SDCC_REENTRANT
{
    ...
    zdo_send_req_by_short(ZDO_NWK_ADDR_REQ_CLID, param, cb, req_param->dst_addr,
                          1);
}
...
{
    zb_zdo_ieee_addr_req_t *req = (zb_zdo_ieee_addr_req_t *)ZB_BUF_BEGIN(ZB_BUF_FROM_REF(
                                                                             param));
...
{
    zb_zdo_node_desc_req_t *req = (zb_zdo_node_desc_req_t *)ZB_BUF_BEGIN(ZB_BUF_FROM_REF(
                                                                             param));
    zb_uint16_t addr = req->nwk_addr;
...
{
    zb_zdo_power_desc_req_t *req = (zb_zdo_power_desc_req_t *)ZB_BUF_BEGIN(ZB_BUF_FROM_REF(
                                                                               param));
    zb_uint16_t addr = req->nwk_addr;
...
{
    zb_zdo_simple_desc_req_t *req = (zb_zdo_simple_desc_req_t *)ZB_BUF_BEGIN(ZB_BUF_FROM_REF(
                                                                                 param));
    zb_uint16_t addr = req->nwk_addr;
...
{
    zb_zdo_active_ep_req_t *req = (zb_zdo_active_ep_req_t *)ZB_BUF_BEGIN(ZB_BUF_FROM_REF(
                                                                             param));
    zb_uint16_t addr = req->nwk_addr;
...
void zb_zdo_match_desc_req(zb_uint8_t param, zb_callback_t cb) ZB_SDCC_REENTRANT
{
    ...
    match_param = ZB_GET_BUF_TAIL(buf, sizeof(zb_zdo_match_desc_param_t) +
                                  (match_param_init->num_in_clusters +
                                   match_param_init->num_out_clusters) *
                                  sizeof(zb_uint16_t));
    ZB_MEMCPY(match_param, match_param_init, sizeof(zb_zdo_match_desc_param_t) +
              (match_param_init->num_in_clusters +
               match_param_init->num_out_clusters) * sizeof(zb_uint16_t));

    ZB_BUF_INITIAL_ALLOC(buf,
                         sizeof(zb_zdo_match_desc_req_head_t) + match_param->num_in_clusters *
                         sizeof(zb_uint16_t) +
                         sizeof(zb_zdo_match_desc_req_tail_t) + match_param->num_out_clusters *
                         sizeof(zb_uint16_t),
                         req_head);
    ...
    TRACE_MSG(TRACE_ZDO3,
              "addr %d, profile id %d, num in cl %hd, num out cl %hd",
              (FMT__D_D_H_H, req_head->nwk_addr, req_head->profile_id,
               req_head->num_in_clusters, req_tail->num_out_clusters));

    zdo_send_req_by_short(ZDO_MATCH_DESC_REQ_CLID, param, cb,
                          match_param->nwk_addr, 1);
