...
void zb_set_node_descriptor(zb_logical_type_t device_type, zb_int8_t power_src,
                            zb_int8_t rx_on_when_idle,
                            zb_int8_t alloc_addr) ZB_SDCC_REENTRANT
{
    ...
    ZB_MAC_CAP_SET_DEVICE_TYPE(mac_cap,
                               (device_type == ZB_COORDINATOR ||
                                device_type == ZB_ROUTER));
    ...
    TRACE_MSG(TRACE_ZDO3, "node desc, server_mask %x mac cap 0x%x",
              (FMT__D_H, ZB_ZDO_NODE_DESC()->server_mask,
               ZB_ZDO_NODE_DESC()->mac_capability_flags));
 ...
 */
void zb_set_ed_node_descriptor(zb_int8_t power_src, zb_int8_t rx_on_when_idle,
                               zb_int8_t alloc_addr) ZB_SDCC_REENTRANT
{
    TRACE_MSG(TRACE_ZDO3, "zb_set_ed_node_descriptor", (FMT__0));
    zb_set_node_descriptor(ZB_END_DEVICE, power_src, rx_on_when_idle,
                           alloc_addr);
}
 ...
 */
void zb_set_node_power_descriptor(zb_current_power_mode_t current_power_mode,
                                  zb_uint8_t available_power_sources,
                                  zb_uint8_t current_power_source,
                                  zb_power_source_level_t current_power_source_level)
ZB_SDCC_REENTRANT
{
    ZB_SET_POWER_DESC_CUR_POWER_MODE(
        ZB_ZDO_NODE_POWER_DESC(), current_power_mode);
    ZB_SET_POWER_DESC_AVAIL_POWER_SOURCES(
        ZB_ZDO_NODE_POWER_DESC(), available_power_sources);
    ZB_SET_POWER_DESC_CUR_POWER_SOURCE(
        ZB_ZDO_NODE_POWER_DESC(), current_power_source);
    ZB_SET_POWER_DESC_CUR_POWER_SOURCE_LEVEL(
        ZB_ZDO_NODE_POWER_DESC(), current_power_source_level);
}
...
void zb_set_input_cluster_id(zb_af_simple_desc_1_1_t *simple_desc,
                             ...
                             zb_uint16_t cluster_id) ZB_SDCC_REENTRANT
{
    TRACE_MSG(TRACE_ZDO2,
              "cluster_id 0x%hx, cluster_number %hu, input count %hd",
              (FMT__H_H_H, cluster_id, cluster_number,
...
{
    TRACE_MSG(TRACE_ZDO2,
              "cluster_id 0x%hx, cluster_number %hd, output count %hd",
              (FMT__H_H_H, cluster_id, cluster_number,
               simple_desc->app_output_cluster_count));
    ZB_ASSERT(cluster_number < simple_desc->app_output_cluster_count);
    simple_desc->app_cluster_list[simple_desc->app_input_cluster_count +
...
void zb_set_zdo_descriptor() ZB_SDCC_REENTRANT
{
    zb_set_simple_descriptor(
        (zb_af_simple_desc_1_1_t *)ZB_ZDO_SIMPLE_DESC(),
        0 /* endpoint */,                0 /* app_profile_id */,
        0 /* app_device_id */,           0 /* app_device_version*/,
        7 /* app_input_cluster_count */,
        8 /* app_output_cluster_count */);
...

    {
        ...
        for (i = 0; i < sizeof(s_iclids) / sizeof(s_iclids[0]); ++i) {
            zb_set_input_cluster_id(
                (zb_af_simple_desc_1_1_t *)ZB_ZDO_SIMPLE_DESC(), i,
                s_iclids[i]);
        }
...
#if 0
    zb_set_input_cluster_id(
        (zb_af_simple_desc_1_1_t *)ZB_ZDO_SIMPLE_DESC(), 0,
        ZDO_NWK_ADDR_RESP_CLID);
    zb_set_input_cluster_id(
        (zb_af_simple_desc_1_1_t *)ZB_ZDO_SIMPLE_DESC(), 1,
        ZDO_IEEE_ADDR_RESP_CLID);
    zb_set_input_cluster_id(
        (zb_af_simple_desc_1_1_t *)ZB_ZDO_SIMPLE_DESC(), 2,
        ZDO_NODE_DESC_RESP_CLID);
    zb_set_input_cluster_id(
        (zb_af_simple_desc_1_1_t *)ZB_ZDO_SIMPLE_DESC(), 3,
        ZDO_POWER_DESC_RESP_CLID);
    zb_set_input_cluster_id(
        (zb_af_simple_desc_1_1_t *)ZB_ZDO_SIMPLE_DESC(), 4,
        ZDO_SIMPLE_DESC_RESP_CLID);
    zb_set_input_cluster_id(
        (zb_af_simple_desc_1_1_t *)ZB_ZDO_SIMPLE_DESC(), 5,
        ZDO_ACTIVE_EP_RESP_CLID);
    zb_set_input_cluster_id(
        (zb_af_simple_desc_1_1_t *)ZB_ZDO_SIMPLE_DESC(), 6,
        ZDO_MATCH_DESC_RESP_CLID);
    ...
    zb_set_output_cluster_id(
        ...
        ZDO_MATCH_DESC_REQ_CLID);
    zb_set_output_cluster_id(
        (zb_af_simple_desc_1_1_t *)ZB_ZDO_SIMPLE_DESC(), 7,
        ZDO_DEVICE_ANNCE_CLID);
#endif
}
...
ZB_SDCC_REENTRANT
{
    ...
    zb_set_node_power_descriptor(ZB_POWER_MODE_SYNC_ON_WHEN_IDLE,
                                 ...
                                 ZB_POWER_SRC_CONSTATNT | ZB_POWER_SRC_RECHARGEABLE_BATTERY,
                                 ZB_POWER_LEVEL_100);
...
ZB_SDCC_REENTRANT
{
    ...
    if (ZB_ZDO_SIMPLE_DESC_NUMBER() < ZB_MAX_EP_NUMBER) {
        ...
        TRACE_MSG(TRACE_ERROR, "Error, simple_desc_number overflows %i",
                  (FMT__D, (int)ZB_ZDO_SIMPLE_DESC_NUMBER()));
        ret = RET_OVERFLOW;
    }
    return ret;
}
