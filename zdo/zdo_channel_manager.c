...
void zb_zdo_start_long_timer(zb_callback_t func, zb_uint8_t param,
                             zb_uint8_t long_timer);
...
void zb_zdo_check_fails(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (ZB_NIB_NWK_TX_TOTAL() > ZB_TX_TOTAL_THRESHOLD &&
        !ZB_ZDO_GET_CHECK_FAILS()) {
        if (ZB_NIB_NWK_TX_FAIL() >
            (ZB_NIB_NWK_TX_TOTAL() / ZB_FAILS_PERCENTAGE)) {
            ZB_ZDO_SET_CHECK_FAILS();
            ZB_SCHEDULE_ALARM(zb_zdo_channel_check_timer_cb, 0,
                              ZB_ZDO_CHANNEL_CHECK_TIMEOUT);
            ...
            ZG->zdo.zdo_ctx.nwk_upd_req.scan_channels =
                ZB_DEFAULT_APS_CHANNEL_MASK;
            ZG->zdo.zdo_ctx.nwk_upd_req.scan_duration =
...
void zb_nlme_ed_scan_confirm(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (buf) {
        if (ZB_APS_GET_ZDO_ED_SCAN_FLAG()) {
            ...
            zb_mac_scan_confirm_t *confirm_param = ZB_GET_BUF_PARAM(buf,
                                                                    zb_mac_scan_confirm_t);
            zb_zdo_mgmt_nwk_update_notify_param_t *notify_param =
                ZB_GET_BUF_PARAM(buf, zb_zdo_mgmt_nwk_update_notify_param_t);
            ...
            for (i =
                 ...
                 i <= ZB_MAC_MAX_CHANNEL_NUMBER;
                 i++) {
                if (channel_mask & (channel_mask_mul)) {
                    channel_mask_mul *= 2;
                    TRACE_MSG(TRACE_NWK1, "chan num %hd, item %hd, energy %hd",
                              (FMT__H_H_H, i,
                               notify_param->hdr.scanned_channels_list_count,
                               confirm.list.energy_detect[i -
                                                          ZB_MAC_START_CHANNEL_NUMBER
                               ]));
                    notify_param->energy_values[notify_param->hdr.
...
{
    zb_buf_t *buf = (zb_buf_t *)ZB_BUF_FROM_REF(param);
    zb_mac_scan_confirm_t *scan_result = ZB_GET_BUF_PARAM(
        MAC_CTX().pending_buf, zb_mac_scan_confirm_t);
    zb_bool_t free_buf = ZB_TRUE;

    TRACE_MSG(TRACE_MAC2, ">> zb_zdo_channel_check_scan_result %d",
              (FMT__D, param));

    ZB_APS_CLEAR_CHANNEL_MANAGER_ED_SCAN_FLAG();

    TRACE_MSG(TRACE_MAC3, "status %hd, unscanned %x %x",
              (FMT__H_D_D, scan_result->status,
               *((zb_uint16_t *)&
                 scan_result->unscanned_channels),
               *((zb_uint16_t *)&scan_result->unscanned_channels + 1)));
    if (scan_result->status == MAC_SUCCESS &&
        !(scan_result->unscanned_channels &
          (1l << ZB_MAC_GET_CURRENT_LOGICAL_CHANNEL()))) {
        TRACE_MSG(TRACE_MAC3, "cur chan %hd, cur energy %hd",
                  (FMT__H_H, ZB_MAC_GET_CURRENT_LOGICAL_CHANNEL(),
                   scan_result->list.energy_detect[
                       ZB_MAC_GET_CURRENT_LOGICAL_CHANNEL() -
                       ZB_MAC_START_CHANNEL_NUMBER]));

        if (scan_result->list.energy_detect[ZB_MAC_GET_CURRENT_LOGICAL_CHANNEL()
                                            - ZB_MAC_START_CHANNEL_NUMBER] >
...
void zdo_change_channel(zb_uint8_t param) ZB_SDCC_REENTRANT
{
    ...
    zb_zdo_mgmt_nwk_update_notify_hdr_t *upd_notify =
        (zb_zdo_mgmt_nwk_update_notify_hdr_t *)body;
    ...
    TRACE_MSG(TRACE_MAC3, "ch chaged %hd, status %hd, count %hd",
              (FMT__H_H_H, ZB_ZDO_GET_CHANNEL_CHANGED(), upd_notify->status,
               upd_notify->scanned_channels_list_count));
    if (!ZB_ZDO_GET_CHANNEL_CHANGED()) {
        body += sizeof(zb_zdo_mgmt_nwk_update_notify_hdr_t);
        if (upd_notify->status == ZB_ZDP_STATUS_SUCCESS &&
            upd_notify->scanned_channels_list_count) {
            scan_result = body;
            /* parse scan results */
            for (i =
                ...
                if (upd_notify->scanned_channels & (1l << i)) {
                    TRACE_MSG(TRACE_MAC3, "scan_result %hd",
                              (FMT__H, *scan_result));
            ...
            TRACE_MSG(TRACE_MAC3,
                      "current_energy %hd, min_energy %hd, new_channel %hd",
                      (FMT__H_H_H, current_energy, min_energy,
                       new_channel_num));
            if (current_energy > ZB_CHANNEL_BUSY_ED_VALUE) {
                if (new_channel_num && min_energy < ZB_CHANNEL_FREE_ED_VALUE) {
                    ...
                    zb_zdo_start_long_timer(zb_zdo_channel_change_timer_cb, 0,
                                            ZB_ZDO_APS_CHANEL_TIMER);
                }
            }
        }
    }
...
void zb_zdo_long_timer_cb(zb_uint8_t param) ZB_CALLBACK
{
    ZVUNUSED(param);
    ZG->zdo.long_timer--;
    TRACE_MSG(TRACE_MAC3, "long_timer_cb timer val %hd",
              (FMT__H, ZG->zdo.long_timer));
