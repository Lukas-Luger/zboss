    ...
    if (ZB_NIB_NETWORK_ADDRESS() == addr_of_interest) {
        ...
        switch (ind->clusterid) {
            case ZDO_NODE_DESC_REQ_CLID:
                ...
                zb_copy_node_desc((zb_af_node_desc_t *)desc_body,
                                  ZB_ZDO_NODE_DESC());
                resp_id = ZDO_NODE_DESC_RESP_CLID;
                break;

            case ZDO_POWER_DESC_REQ_CLID:
                ZB_BUF_ALLOC_RIGHT(buf, sizeof(zb_af_node_power_desc_t),
                                   desc_body);
                zb_copy_power_desc((zb_af_node_power_desc_t *)desc_body,
                                   ZB_ZDO_NODE_POWER_DESC());
                resp_id = ZDO_POWER_DESC_RESP_CLID;
                break;

            default:
                TRACE_MSG(TRACE_ZDO3, "unknown cluster id %i",
                          (FMT__D, ind->clusterid));
                ZB_ASSERT(0);
        }
    }
    else {
 ...
 */
    ...
    if (ZB_NIB_NETWORK_ADDRESS() == addr_of_interest) {
        if (!ep) {
            ...
            for (i = 0; i < ZB_ZDO_SIMPLE_DESC_NUMBER(); i++) {
                if (ZB_ZDO_SIMPLE_DESC_LIST()[i]->endpoint == ep) {
                    src_desc =
                        (zb_af_simple_desc_1_1_t *)ZB_ZDO_SIMPLE_DESC_LIST()[i];
                }
            }
        }
        ...
        if (src_desc) {
            ...
            resp_hdr->length = sizeof(zb_af_simple_desc_1_1_t) +
                               ...
                               (src_desc->app_input_cluster_count +
                                src_desc->app_output_cluster_count - 2) *
                               sizeof(zb_uint16_t);
    ...
    ZB_LETOH16(&resp_hdr->nwk_addr, &addr_of_interest);
    TRACE_MSG(TRACE_ZDO1, "simple_desc length: %hd",
              (FMT__H, resp_hdr->length));
    zdo_send_resp_by_short(ZDO_SIMPLE_DESC_RESP_CLID, param, tsn,
                           ind->src_addr);
...
void zb_copy_node_desc(zb_af_node_desc_t *dst_desc,
                       zb_af_node_desc_t *src_desc) ZB_SDCC_REENTRANT
{
    ...
    ZB_LETOH16(&dst_desc->max_incoming_transfer_size,
               &src_desc->max_incoming_transfer_size);
    ZB_LETOH16(&dst_desc->server_mask, &src_desc->server_mask);
    ZB_LETOH16(&dst_desc->max_outgoing_transfer_size,
               &src_desc->max_outgoing_transfer_size);
}

void zb_copy_power_desc(zb_af_node_power_desc_t *dst_desc,
                        zb_af_node_power_desc_t *src_desc) ZB_SDCC_REENTRANT
{
    ZB_LETOH16(&dst_desc->power_desc_flags, &src_desc->power_desc_flags);
}

void zb_copy_simple_desc(zb_af_simple_desc_1_1_t *dst_desc,
                         zb_af_simple_desc_1_1_t *src_desc) ZB_SDCC_REENTRANT
{
    zb_ushort_t i;
    zb_uint8_t out_count;

    ZB_MEMCPY(dst_desc, src_desc,
              sizeof(zb_af_simple_desc_1_1_t) - sizeof(zb_uint16_t) * 2);
    ZB_LETOH16_XOR(dst_desc->app_profile_id);
    ZB_LETOH16_XOR(dst_desc->app_device_id);
    out_count = src_desc->app_output_cluster_count;
    TRACE_MSG(TRACE_ZDO1, "simple_desc outclusters count %hd",
              (FMT__H, out_count));
    TRACE_MSG(TRACE_ZDO1, "simple_desc inclusters count %hd",
              (FMT__H, src_desc->app_input_cluster_count));

    for (i = 0; i < src_desc->app_input_cluster_count; i++) {
        ZB_LETOH16(((zb_uint8_t *)(&dst_desc->app_cluster_list[i]) - 1),
                   &src_desc->app_cluster_list[i]);
    }
    for (i = src_desc->app_input_cluster_count;
         i <
         src_desc->app_input_cluster_count + src_desc->app_output_cluster_count;
         i++) {
        ZB_LETOH16((&dst_desc->app_cluster_list[i]),
                   (&src_desc->app_cluster_list[i]));
    }
    ZB_MEMCPY((zb_uint8_t *)(dst_desc->app_cluster_list +
                             src_desc->app_input_cluster_count) - 1, &out_count,
              1);
...
void zdo_device_nwk_addr_res(zb_uint8_t param, zb_uint8_t fc) ZB_SDCC_REENTRANT
{
    ...
    TRACE_MSG(TRACE_ZDO3, "zdo_dev_nwk_addr_res %hd, fc %hd",
              (FMT__H_H, param, fc));
    ...
    ZB_MEMCPY(&ind, ZB_GET_BUF_PARAM(buf, zb_apsde_data_indication_t),
              sizeof(ind));

    /* TODO: make HTOL conversion */

    TRACE_MSG(TRACE_ZDO3, "request_type %hd, start_index %hd",
              (FMT__H_H, req.request_type, req.start_index));
        ...
        if (ZB_64BIT_ADDR_CMP(ZB_PIB_EXTENDED_ADDRESS(), req.ieee_addr)) {
            ...
            for (i = 0; i < ZB_NEIGHBOR_TABLE_SIZE; i++) {
                if (ZG->nwk.neighbor.base_neighbor[i].used) {
                    zb_address_ieee_by_ref(ieee_addr,
                                           ZG->nwk.neighbor.base_neighbor[i].addr_ref);
                    if (ZB_64BIT_ADDR_CMP(ieee_addr, req.ieee_addr)) {
                        zb_address_short_by_ref(&nwk_addr,
                                                ZG->nwk.neighbor.base_neighbor[i].addr_ref);
                        TRACE_MSG(TRACE_ZDO3, "remote found %d",
                                  (FMT__D, nwk_addr));
                        break;
                    }
                }
    ...
    if ((status == ZB_ZDP_STATUS_SUCCESS) &&
        ...
        *ptr++ = status;
...

        {
            ...
            if ((addr_counter * sizeof(zb_uint16_t)) + headers_size >
                ZB_IO_BUF_SIZE) {
                addr_counter = 0;
                do {
                    len += 2;
        ...
        *ptr = 0;
        ptr++;
        {
            ...
            for (i = 0, ent = &ZG->nwk.neighbor.base_neighbor[0];
                 i < ZB_NEIGHBOR_TABLE_SIZE && addr_counter; i++) {
                if (ent->used) {
                    found++;
                    if (found >= req.start_index) {
                        ...
                        addr_counter--;
                        ent++;
                        TRACE_MSG(TRACE_ZDO3, "associated device addr: %d",
                                  (FMT__D, addr));
                    }
...
void zdo_device_ieee_addr_res(zb_uint8_t param, zb_uint8_t fc) ZB_SDCC_REENTRANT
{
    ...
    TRACE_MSG(TRACE_ZDO3, "zdo_dev_ieee_addr_res %hd, fc %hd",
              (FMT__H_H, param, fc));
...
static zb_uint8_t add_ep_sorted(zb_uint8_t *ep_list, zb_uint8_t ep_num,
                                zb_uint8_t endpoint) ZB_SDCC_REENTRANT
{
    zb_uint8_t i;
    zb_uint8_t index;

    TRACE_MSG(TRACE_ZDO3, "add_ep_sorted ep_num %hd, ep %hd",
              (FMT__H_H, ep_num, endpoint));
    for (index = 0; index < ep_num; index++) {
        if (endpoint <= ep_list[index]) {
            break;
        }
    }
    TRACE_MSG(TRACE_ZDO3, "index %hd, ep[i] %hd",
              (FMT__H_H, index, ep_list[index]));
                            ...
                            zb_uint8_t cluster_num) ZB_SDCC_REENTRANT
{
    ...
    for (i = 0; i < cluster_num; i++) {
        ZB_HTOLE16(cluster_dst + i * sizeof(zb_uint16_t),
                   cluster_src + i * sizeof(zb_uint16_t));
    }
...
void zdo_match_desc_res(zb_uint8_t param) ZB_SDCC_REENTRANT
{
    ...
    match_params.num_out_clusters =
        *(aps_body + match_params.num_in_clusters * sizeof(zb_uint16_t));
    ...
    TRACE_MSG(TRACE_ZDO3,
              "nwk_addr %d, profile_id %d, num in clust %hd, num out clust %hd",
              (FMT__D_D_H_H, match_params.nwk_addr, match_params.profile_id,
    ...
    cluster_id =
        ZB_GET_BUF_TAIL(buf,
                        (match_params.num_in_clusters +
                         match_params.num_out_clusters) *
                        sizeof(zb_uint16_t));
    aps_body = copy_cluster_id((zb_uint8_t *)cluster_id, aps_body,
                               match_params.num_in_clusters);

    aps_body++; /* num_out_clusters field */
    aps_body =
        copy_cluster_id(
            (zb_uint8_t *)(cluster_id + match_params.num_in_clusters),
    ...
    if ((ZB_PIB_SHORT_ADDRESS() != match_params.nwk_addr) &&
        ...
        TRACE_MSG(TRACE_ZDO3, "simple desc num %hd",
                  (FMT__H, ZB_ZDO_SIMPLE_DESC_NUMBER()));
        for (i = 0; i < ZB_ZDO_SIMPLE_DESC_NUMBER(); i++) {
            found = ZB_FALSE;
            TRACE_MSG(TRACE_ZDO3, "app_profile_id %d",
                      (FMT__D, ZB_ZDO_SIMPLE_DESC_LIST()[i]->app_profile_id));
            if (ZB_ZDO_SIMPLE_DESC_LIST()[i]->app_profile_id ==
                match_params.profile_id) {
                TRACE_MSG(TRACE_ZDO3, "app in cluster count %hd",
                          (FMT__H,
                ...
                for (j = 0;
                     ...
                     ZB_ZDO_SIMPLE_DESC_LIST()[i]->app_input_cluster_count &&
                     !found; j++) {
                    for (k = 0; k < match_params.num_in_clusters && !found;
                         k++) {
                        if (ZB_ZDO_SIMPLE_DESC_LIST()[i]->app_cluster_list[j] ==
                            cluster_id[k]) {
                            ep_num = add_ep_sorted(ep_list, ep_num,
                                                   ZB_ZDO_SIMPLE_DESC_LIST()[i]->endpoint);
                            found = ZB_TRUE;
                        }
                    }
                } /* for j .. app_output_cluster_count */

                TRACE_MSG(TRACE_ZDO3, "app out cluster count %hd",
                          (FMT__H,
                           ZB_ZDO_SIMPLE_DESC_LIST()[i]->
                           app_output_cluster_count));
                for (j = 0;
                     j <
                     ZB_ZDO_SIMPLE_DESC_LIST()[i]->app_output_cluster_count &&
                     !found; j++) {
                    for (k = 0; k < match_params.num_out_clusters && !found;
                         k++) {
                        if (ZB_ZDO_SIMPLE_DESC_LIST()[i]->app_cluster_list[
                                ZB_ZDO_SIMPLE_DESC_LIST()[i]->
    ...
    ZB_BUF_INITIAL_ALLOC(buf,
                         sizeof(zb_zdo_match_desc_resp_t) + ep_num *
                         sizeof(zb_uint8_t), match_resp);
    ZB_HTOLE16(&match_resp->nwk_addr, &match_params.nwk_addr);
    match_resp->match_len = ep_num;
    if (ep_num) {
        status = ZB_ZDP_STATUS_SUCCESS;
        ZB_MEMCPY((zb_uint8_t *)(match_resp + 1), ep_list,
                  ep_num * sizeof(zb_uint8_t));
    }
    else {
    ...
    if ((ZB_PIB_SHORT_ADDRESS() != req.nwk_addr) &&
        ...
        status =
            zb_address_by_short(req.nwk_addr, ZB_FALSE, ZB_FALSE, &addr_ref);
        if (status == RET_OK) {
            for (i = 0, nbt = &ZG->nwk.neighbor.base_neighbor[i];
                 i < ZB_NEIGHBOR_TABLE_SIZE; i++) {
                if ((nbt->relationship == ZB_NWK_RELATIONSHIP_CHILD) &&
                    ...
                    if (ZG->af_desc.simple_desc_number) {
                        ...
                        for (j = 0; j < ZG->af_desc.simple_desc_number; j++) {
                            if (ZG->af_desc.simple_desc_list[j]->app_profile_id
                                == req.profile_id) {
                                zb_uint8_t k = 0;
                                ...
                                aps_body++;
                                if ((cluster_count != 0) &&
                                    (ZG->af_desc.simple_desc_list[j]->
                                     app_cluster_list[0] != 0)) {
                                    zb_uint16_t in_cluster = 0;
                                    zb_uint8_t m = 0;

                                    for (m = 0; m < cluster_count; m++) {
                                        ZB_MEMCPY(&in_cluster, aps_body,
                                                  sizeof(in_cluster));
                                        for (k = 0;
                                             k <
                                             ZG->af_desc.simple_desc_list[j]->
                                            ...
                                            ZB_MEMCPY(&in_cluster, aps_body,
                                                      sizeof(in_cluster));
                                            if (ZG->af_desc.simple_desc_list[j]
                                                ->app_cluster_list[k] ==
                                                in_cluster) {
                                                if (!found) {
                                                    found = 1;
                                                ...
                                                (*match_len)++;
                                                ZB_BUF_ALLOC_LEFT(out_buf, 1,
                                                                  ptr);
                                                *ptr =
                                                    ZG->af_desc.simple_desc_list
                                                    [j]
                                                    ->endpoint;
                                            }
                                        }
                                ...
                                if (!found) {
                                    ...
                                    if ((cluster_count != 0) &&
                                        ...
                                        for (m = 0; m < cluster_count; m++) {
                                            ZB_MEMCPY(&out_cluster, aps_body,
                                                      sizeof(out_cluster));
                                            for (k = 0;
                                                 k <
                                                 ZG->af_desc.simple_desc_list[j]
                                                 ->
                                                 app_input_cluster_count; k++) {
                                                if (ZG->af_desc.simple_desc_list
                                                    [j]->app_cluster_list[k] ==
                                                    out_cluster) {
                                                    (*match_len)++;
                                                    ZB_BUF_ALLOC_LEFT(out_buf,
                                                                      1, ptr);
                                                    *ptr =
                                                        ZG->af_desc.
                                                        simple_desc_list
