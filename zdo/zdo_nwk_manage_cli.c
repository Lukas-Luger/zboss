                                ...
                                zb_callback_t cb) ZB_SDCC_REENTRANT
{
    ...
    TRACE_MSG(TRACE_ZDO2, ">> zb_zdo_mgmt_nwk_update_req param %hd",
              (FMT__D, param));
    req_param = ZB_GET_BUF_PARAM(buf, zb_zdo_mgmt_nwk_update_req_t);

    ZB_BUF_INITIAL_ALLOC(buf,
                         sizeof(zb_zdo_mgmt_nwk_update_req_hdr_t) +
                         sizeof(zb_uint8_t), req);
        ...
        *ptr_8 = req_param->update_id;
        ...
        if (req->scan_duration == ZB_ZDO_NEW_CHANNEL_MASK) {
            ...
            TRACE_MSG(TRACE_ZDO2, "set new channel %hd after %d",
                      (FMT__H_D, i, ZB_NWK_BROADCAST_DELIVERY_TIME()));
            ZB_SCHEDULE_ALARM(zb_zdo_new_channel_cb, i,
                              ZB_NWK_BROADCAST_DELIVERY_TIME());
        }
    }
    zdo_send_req_by_short(ZDO_MGMT_NWK_UPDATE_REQ_CLID, param, cb,
                          req_param->dst_addr, resp_counter);
                                        ...
                                        zb_callback_t cb) ZB_SDCC_REENTRANT
{
    ...
    ZB_BUF_INITIAL_ALLOC(buf, sizeof(zb_zdo_system_server_discovery_param_t),
                         req);

    ZB_HTOLE16(&req->server_mask, &req_param->server_mask);

    TRACE_MSG(TRACE_ZDO3, "zb_zdo_system_server_discovery_req server_mask %x",
              (FMT__D, req->server_mask));
    ZDO_CTX().system_server_discovery_cb = cb;
    zdo_send_req_by_short(ZDO_SYSTEM_SERVER_DISCOVERY_REQ_CLID, param, NULL,
                          ZB_NWK_BROADCAST_RX_ON_WHEN_IDLE, 1);
}
...
void zb_zdo_mgmt_lqi_req(zb_uint8_t param, zb_callback_t cb) ZB_SDCC_REENTRANT
{
    ...
    zdo_send_req_by_short(ZDO_MGMT_LQI_REQ_CLID, param, cb, req_param->dst_addr,
                          1);
}
...
void zdo_mgmt_leave_req(zb_uint8_t param, zb_callback_t cb) ZB_SDCC_REENTRANT
{
    ...
    ZB_MEMCPY(&req_param, ZB_GET_BUF_PARAM(buf,
    ...
    if (req_param.dst_addr == ZB_PIB_SHORT_ADDRESS()) {
        ...
        zdo_send_req_by_short(ZDO_MGMT_LEAVE_REQ_CLID, param, cb,
...
void zdo_mgmt_leave_cli(zb_uint8_t param, zb_callback_t cb) ZB_SDCC_REENTRANT
{
    ...
    if (i == ZB_ZDO_PENDING_LEAVE_SIZE) {
        ...
        *status_p = ZB_ZDP_STATUS_INSUFFICIENT_SPACE;
        ...
        ZB_MEMCPY(&req, ZB_BUF_BEGIN(ZB_BUF_FROM_REF(param)),
                  sizeof(zb_zdo_mgmt_leave_req_t));
                                    ...
                                    zb_callback_t cb) ZB_SDCC_REENTRANT
{
    ...
    TRACE_MSG(TRACE_ZDO3, ">> zb_zdo_mgmt_permit_joining_req param %hd cd %p",
              (FMT__H_P, param, cb));

    ZB_MEMCPY(&req_param,
              ZB_GET_BUF_PARAM(buf, zb_zdo_mgmt_permit_joining_req_param_t),
              sizeof(req_param));
