...
void zb_zdo_force_child_leave(zb_uint8_t param,
                              zb_uint16_t child_addr) ZB_SDCC_REENTRANT;
...
static void init_config_attr() ZB_SDCC_REENTRANT
{
    ...
    ZDO_CTX().conf_attr.permit_join_duration =
        ZB_DEFAULT_PRMIT_JOINING_DURATION;
...
zb_ret_t zdo_dev_start() ZB_SDCC_REENTRANT
{
    ...
    if (ZB_EXTPANID_IS_ZERO(ZB_NIB_EXT_PAN_ID())) {
        ...
        if (ZB_AIB().aps_designated_coordinator) {
            ...
            zb_nlme_network_formation_request_t *req = ZB_GET_BUF_PARAM(buf,
            ...
            if (!ZB_EXTPANID_IS_ZERO(ZB_AIB().aps_use_extended_pan_id)) {
                ZB_IEEE_ADDR_COPY(ZB_NIB_EXT_PAN_ID(),
             ...
             */
            ret = ZB_SCHEDULE_CALLBACK(zb_nlme_network_formation_request, ZB_REF_FROM_BUF(
                                           buf));

#else
            TRACE_MSG(TRACE_MAC1, "Coordinator role is not supported",
                      (FMT__0));
            ...
            ZDO_CTX().zdo_ctx.discovery_ctx.disc_count =
                ZDO_CTX().conf_attr.nwk_scan_attempts;
            ret = ZB_SCHEDULE_CALLBACK(zb_nlme_network_discovery_request, ZB_REF_FROM_BUF(
                                           buf));
        }
    }
    else {
...
zb_ret_t zdo_initiate_rejoin(zb_buf_t *buf) ZB_SDCC_REENTRANT
{
    ...
    if (ZB_NIB_DEVICE_TYPE() == ZB_NWK_DEVICE_TYPE_NONE) {
        ...
        TRACE_MSG(TRACE_APS1, "Rejoin to pan " TRACE_FORMAT_64 " as ZR",
                  (FMT__A, TRACE_ARG_64(ZB_AIB().aps_use_extended_pan_id)));
    }
    else
#endif
    {
        TRACE_MSG(TRACE_APS1, "Rejoin to pan " TRACE_FORMAT_64 " as ZE",
                  (FMT__A, TRACE_ARG_64(ZB_AIB().aps_use_extended_pan_id)));
...
{
    zb_nlme_permit_joining_request_t *request = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                                     param),
                                                                 zb_nlme_permit_joining_request_t);

    TRACE_MSG(TRACE_NWK1, "formation conf st %hd",
...
void zb_nlme_permit_joining_confirm(zb_uint8_t param) ZB_CALLBACK
{
    ...
    TRACE_MSG(TRACE_NWK1, "permit j conf st %hd",
              (FMT__H, ((zb_buf_t *)ZB_BUF_FROM_REF(param))->u.hdr.status));
...
void zb_nlme_join_indication(zb_uint8_t param) ZB_CALLBACK
{
    ...
    TRACE_MSG(TRACE_NWK1,
              "JOINED (st %hd) dev 0x%x/" TRACE_FORMAT_64 " cap: dev type %hd rx.w.i. %hd rejoin %hd secur %hd",
              (FMT__H_D_A_H_H_H_H,
...
void zb_nlme_network_discovery_confirm(zb_uint8_t param) ZB_CALLBACK
{
    ...
    for (i = 0; i < cnf->network_count; ++i) {
        TRACE_MSG(TRACE_NWK1,
                  "net %hd: xpanid " TRACE_FORMAT_64 ", ch %hd, s.prof %hd, zb v %hd, beacon_ord %hd, superf_ord %hd, permit_j %hd, rtr_cap %hd, ed_cap %hd",
                  (FMT__H_A_H_H_H_H_H_H_H_H,
                   i,
                   TRACE_ARG_64(dsc->extended_pan_id),
                   (
                       int)dsc->logical_channel, (int)dsc->stack_profile,
                   (int)dsc->zigbee_version,
                   (
                       int)dsc->beacon_order,
                   (
                       int)dsc->superframe_order, (int)dsc->permit_joining,
                   (
                       int)dsc->router_capacity,
                   (int)dsc->end_device_capacity));
        dsc++;
    }
#endif
    ...
    for (i = 0; i < cnf->network_count; ++i) {
        if (ZB_EXTPANID_IS_ZERO(ZB_AIB().aps_use_extended_pan_id)
            || ZB_EXTPANID_CMP(dsc->extended_pan_id,
                               ZB_AIB().aps_use_extended_pan_id)) {
            /*
...
/* joined_pro, here's one of the key moments */
            ...
            {
                if (MAC_PIB().mac_rx_on_when_idle) {
                    ZB_MAC_CAP_SET_RX_ON_WHEN_IDLE(req->capability_information,
                                                   1);
                }
            }
            ZB_MAC_CAP_SET_ALLOCATE_ADDRESS(req->capability_information, 1);
...
void zdo_join_done(zb_uint8_t param) ZB_CALLBACK
{
    ...
    TRACE_MSG(TRACE_COMMON1,
              "Join done, scheduling poll request with appropriate tmout",
              (FMT__0));
...
{
    zb_nlme_join_confirm_t *confirm = ZB_GET_BUF_PARAM((zb_buf_t *)ZB_BUF_FROM_REF(
                                                           param),
    ...
    if (confirm->status == 0) {
        TRACE_MSG(TRACE_COMMON1,
                  "CONGRATULATIONS! joined status %hd, addr %d, xpanid " TRACE_FORMAT_64 ", ch %hd, addr 0x%x",
                  (FMT__H_D_A_H_D,
                   confirm
                   ->status, confirm->network_address,
                   TRACE_ARG_64(
        ...
        if (ZB_NIB_DEVICE_TYPE() == ZB_NWK_DEVICE_TYPE_ROUTER) {
            ...
            request = ZB_GET_BUF_PARAM((zb_buf_t *)ZB_BUF_FROM_REF(
                                           param),
        ...
        zb_nlme_network_discovery_request_t *req = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                                        param),
                                                                    zb_nlme_network_discovery_request_t);
        TRACE_MSG(TRACE_ZDO1, "rejoin failed st %hd - try assoc",
                  (FMT__H, (int)confirm->status));
        ...
        TRACE_MSG(TRACE_ZDO1, "assoc j failed st %hd",
                  (FMT__H, (int)confirm->status));
        ZB_BUF_FROM_REF(param)->u.hdr.status = ZB_NWK_STATUS_NOT_PERMITTED;
        ZB_SCHEDULE_CALLBACK(zb_zdo_startup_complete, param);
    }
...
void zb_apsde_data_confirm(zb_uint8_t param) ZB_CALLBACK
{
    ...
    TRACE_MSG(TRACE_APS3,
              "apsde_data_conf: param %hd status %hd dev_annce %hd key_sw %hd",
              (FMT__H_H_H_H, param, buf->u.hdr.status, ZG->zdo.handle.dev_annce,
               ZG->zdo.handle.key_sw));
    ...
    if (ZG->zdo.handle.dev_annce == param) {
        ...
        zb_address_by_short(ZB_PIB_SHORT_ADDRESS(), ZB_TRUE, ZB_TRUE,
                            &addr_ref);
        TRACE_MSG(TRACE_ZDO1, "was device_annce, start compl, st %hd",
        ...
        TRACE_MSG(TRACE_SECUR3, "switch nwk key after this frame sent",
                  (FMT__0));
        ...
        if (buf->u.hdr.status == 0 ||
            buf->u.hdr.status == (zb_uint8_t)RET_NO_ACK) {
            TRACE_MSG(TRACE_ZDO1,
                      "buffer status %hd - call zb_apsde_data_acknowledged",
                      (FMT__H, buf->u.hdr.status));
            ZB_SCHEDULE_CALLBACK(zb_apsde_data_acknowledged, param);
        }
        else {
            TRACE_MSG(TRACE_ZDO1, "buffer status %hd - free buf",
                      (FMT__H, buf->u.hdr.status));
            zb_free_buf(buf);
        }
#ifdef APS_RETRANSMIT_TEST
...
void zb_nlme_sync_confirm(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (ZB_BUF_FROM_REF(param)->u.hdr.status == MAC_SUCCESS
        ...
        if (ZDO_CTX().parent_threshold_retry >=
            ZDO_CTX().max_parent_threshold_retry) {
            sched_poll = ZB_FALSE;
            /* rejoin to current pan */
            ZB_EXTPANID_COPY(
                ZB_AIB().aps_use_extended_pan_id, ZB_NIB_EXT_PAN_ID());
...
#endif
    if (sched_poll) {
        TRACE_MSG(TRACE_NWK1, "schedule poll if needed rx_on_when_idle %hd",
                  (FMT__H, MAC_PIB().mac_rx_on_when_idle));
...
void zb_zdo_reschedule_poll_parent(zb_uint16_t timeout)
{
    ...
    if (!(MAC_PIB().mac_rx_on_when_idle
        ...
        ZB_SCHEDULE_ALARM(zb_zdo_poll_parent, 0,
                          ZB_MAC_GET_PENDING_DATA() ? 1 : timeout);
    }
}
...
void zb_zdo_poll_parent(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (!param) {
        ...
        zb_nlme_sync_request_t *request = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                               param),
...
{
    zb_buf_t *buf = (zb_buf_t *)ZB_BUF_FROM_REF(param);
    zb_nlme_status_indication_t *status = ZB_GET_BUF_PARAM(buf,
    ...
    TRACE_MSG(TRACE_NWK1, "parent link failure %hd",
              (FMT__H, ZDO_CTX().parent_link_failure));
#ifndef ZB_LIMITED_FEATURES
                              ...
                              zb_uint16_t child_addr) ZB_SDCC_REENTRANT
{
    ...
    TRACE_MSG(TRACE_NWK1, "<<zb_zdo_force_child_leave status %d",
              (FMT__D, ret));
}
...
void zb_zdo_reset(zb_uint8_t param, zb_uint8_t warm_start,
                  zb_callback_t cb) ZB_SDCC_REENTRANT
{
    zb_buf_t *buf = (zb_buf_t *)ZB_BUF_FROM_REF(param);
    zb_nlme_reset_request_t *request = ZB_GET_BUF_PARAM(buf,
...
void zb_nlme_leave_indication(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (ZB_IEEE_ADDR_IS_ZERO(request->device_address)) {
        ...
        if (zb_nwk_neighbor_get_by_ieee(request->device_address,
                                        &nbt) == RET_OK) {
#ifdef ZB_SECURITY
#ifdef ZB_ROUTER_ROLE
            if (!ZG->nwk.handle.is_tc
                ...
                zb_apsme_update_device_req_t *req = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                                         param),
                                                                     zb_apsme_update_device_req_t);
                TRACE_MSG(TRACE_SECUR3,
                          "sending update-device.request (device left) to TC",
                          (FMT__0));

                req->status = ZB_DEVICE_LEFT;
                ZB_IEEE_ADDR_COPY(req->dest_address,
                                  ZB_AIB().trust_center_address);
                zb_address_short_by_ref(&req->device_short_address,
                                        nbt->addr_ref);
...
#endif      /* security */
            /* forget this device */
            TRACE_MSG(TRACE_ZDO2, "forget device by addr ref %hd",
                      (FMT__H, nbt->addr_ref));
...
{
    zb_nlme_leave_confirm_t *lc = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                       param),
                                                   zb_nlme_leave_confirm_t);
    zb_uint8_t will_leave =
        (!lc->status && ZB_IEEE_ADDR_IS_ZERO(lc->device_address));
    zb_neighbor_tbl_ent_t *ent = NULL;

    TRACE_MSG(TRACE_ZDO2, "LEAVE.CONFIRM satus %hd will_leave %hd",
              (FMT__H_H, lc->status, will_leave));
...
void zb_mlme_set_confirm(zb_uint8_t param) ZB_CALLBACK
{
...
#ifdef ZB_TRACE_LEVEL
    {
        zb_mlme_set_confirm_t *conf =
            ( zb_mlme_set_confirm_t *)ZB_BUF_BEGIN(buf);
        TRACE_MSG(TRACE_APS2, "<<zb_mlme_set_confirm status %hd",
                  (FMT__H, conf->status));
    }
#endif
    zb_free_buf(buf);
