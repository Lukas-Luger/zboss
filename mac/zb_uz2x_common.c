...
/* ds-2400 4.3.1. Transmit Packet in Normal FIFO */

    {
        ...
        ubec_fifo_header->frame_length = ZB_BUF_LEN(buf) -
                                         sizeof(zb_ubec_fifo_header_t);
    }
    ...
    if (need_tx) {
        ...
        zb_uint8_t need_ack = (!((ZB_FCF_GET_FRAME_TYPE(fc) == MAC_FRAME_BEACON
                                  || (ZB_FCF_GET_DST_ADDRESSING_MODE(fc) ==
...
void zb_transceiver_set_channel(zb_uint8_t channel_number)
{
    ...
    TRACE_MSG(TRACE_MAC2, ">> zb_transceiver_set_channel, chan num %hd",
              (FMT__D, channel_number));
 ...
 */
        ...
        ZB_MAC_GET_BYTE_VALUE() |=
            (channel_number - ZB_TRANSCEIVER_START_CHANNEL_NUMBER) << 4;
...
static zb_ret_t zb_uz_secur_process()
{
    ...
    while (1) {
        if (!(ZB_MAC_GET_SECURITY_PROCESS())) {
            break;
        }
    ...
    zb_buf_t *buf)
{
    ...
    TRACE_MSG(TRACE_MAC2, ">>zb_ccm_encrypt_n_auth a_len %hd m_len %hd",
              (FMT__H_H, string_a_len, string_m_len));
    ...
    ZB_TRANS_READ_TXNFIFO(buf,
                          string_m_len + string_a_len + ZB_CCM_M +
                          ZB_FIFO_HEADER_SIZE);
    ZB_DUMP_INCOMING_DATA(buf);
    (void)zb_buf_cut_left(buf, ZB_FIFO_HEADER_SIZE);

    TRACE_MSG(TRACE_MAC2, "<<zb_ccm_encrypt_n_auth ret %d len %hd",
              (FMT__D_H, ret, ZB_BUF_LEN(buf)));
    return ret;
}
    ...
    zb_ushort_t string_c_len)
{
    ...
    do {
        ...
        ZB_WRITE_LONG_REG(0x01,
                          string_c_len + string_a_len + ZB_FIFO_HEADER_SIZE);
...
void zb_uz_secur_handle_rx()
{
    if (ZB_UBEC_GET_SECUR_STATUS()) {
        ...
        TRACE_MSG(TRACE_MAC2, "Got security RX. Try to ignore security",
                  (FMT__0));
    ...
    if (ZB_UBEC_GET_SECUR_STATUS()) {
        ...
        zb_uz2400_fifo_read(0, ZG->mac.mac_ctx.encryption_buf,
                            hlen + ZB_MAC_PACKET_LENGTH_SIZE);

        hlen = zb_parse_mhr(&mhr, ZB_BUF_BEGIN(
                                ZG->mac.mac_ctx.encryption_buf) +
                            ZB_MAC_PACKET_LENGTH_SIZE);
        ...
        for (i = 0; i < MAC_PIB().mac_device_table_entries; ++i) {
            if (MAC_PIB().mac_device_table[i].pan_id == mhr.src_pan_id) {
                if (is_long_addr) {
                    if (MAC_PIB().mac_device_table[i].short_address ==
                        mhr.src_addr.addr_short) {
                        break;
                    }
                }
                else {
                    if (ZB_IEEE_ADDR_CMP(MAC_PIB().mac_device_table[i].
                                         long_address,
        ...
        if (i == MAC_PIB().mac_device_table_entries) {
            ...
            TRACE_MSG(TRACE_SECUR1, "device not found - MAC unsecure failed",
                      (FMT__0));
            ret = RET_ERROR;
        }
        else if (MAC_PIB().mac_device_table[i].frame_counter > mhr.frame_counter
                 || MAC_PIB().mac_device_table[i].frame_counter ==
                 (zb_uint32_t) ~0) {
            ret = RET_ERROR;
            TRACE_MSG(TRACE_SECUR1,
                      "frame cnt %ld->%ld shift back - MAC unsecure failed",
                      (FMT__L_L,
                       MAC_PIB().mac_device_table[i].frame_counter >
                       mhr.frame_counter));
        }
        ...
        if (ret != RET_OK) {
            ...
            if (!is_long_addr) {
                ...
                for (j = 0; j < 8; ++j) {
                    ZB_WRITE_LONG_REG(ZB_LREG_SRCADR_0 + j,
                                      MAC_PIB().mac_device_table[i].long_address[
                                          j]);
                }
            }
            ...
            /* nonce - see 7.6.3.2 CCM* Nonce. */
            {
                ...
                ZB_IEEE_ADDR_COPY(nonce.source_address,
                                  MAC_PIB().mac_device_table[i].long_address);
