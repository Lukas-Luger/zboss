 ...
 */
    ...
    ZB_MEMCPY(&mhr.src_addr, MAC_PIB().mac_extended_address,
 ...
 */
    ...
    ZB_MEMCPY((zb_uint8_t *)ptr + mhr_len, &coord_realign_cmd,
              sizeof(zb_coord_realignment_cmd_t));

    MAC_ADD_FCS(MAC_CTX().operation_buf);
    ret = ZB_TRANS_SEND_COMMAND(mhr_len, MAC_CTX().operation_buf);

    TRACE_MSG( TRACE_MAC1, "<< zb_tx_coord_realignment_cmd, ret %i",
               (FMT__D, ret));
    return ret;
}
 ...
 */
    ...
    if (ZB_CHECK_BEACON_MODE_ON()) {
        start_req_params = ZB_GET_BUF_PARAM(
            (zb_buf_t *)MAC_CTX().pending_buf, zb_mlme_start_req_t);
        ...
        beacon_frame_params.superframe_order =
            start_req_params->superframe_order;
 ...
 */
    mhr_len = zb_mac_calculate_mhr_length(beacon_frame_params->src_addr_mode,
                                          ZB_ADDR_NO_ADDR,
                                          0);
    packet_length = mhr_len + sizeof(zb_super_frame_spec_t) + 2 *
                    sizeof(zb_uint8_t) + MAC_PIB().mac_beacon_payload_length;

    TRACE_MSG(TRACE_MAC1, "packet length %hd, payload len %hd",
 ...
 */
    ...
    ZB_FCF_SET_SRC_ADDRESSING_MODE(mhr.frame_control,
                                   beacon_frame_params->src_addr_mode);
    /* MAC_FRAME_VERSION defined in zb_config.h */
    ZB_FCF_SET_FRAME_VERSION(mhr.frame_control, MAC_FRAME_VERSION);
    ZB_FCF_SET_FRAME_PENDING_BIT(mhr.frame_control,
                                 beacon_frame_params->frame_pending);
 ...
 */
    ...
    if (beacon_frame_params->src_addr_mode == ZB_ADDR_64BIT_DEV) {
        ...
        TRACE_MSG(TRACE_MAC1, "beacon set short addr %x",
                  (FMT__D, mhr.src_addr.addr_short));
    }
    /* TODO: fill Aux Security Header */
 ...
 */
    ...
    ZB_SUPERFRAME_SET_SUPERFRAME_ORDER(ptr,
                                       beacon_frame_params->superframe_order);
 ...
 * const == 440 and doesn't fit into 4 bits; other stacks use value 0x0f */
    ...
    TRACE_MSG(TRACE_MAC3, "superframe %x ass perm %hd",
              (FMT__D_H, *(zb_uint16_t *)ptr,
               (zb_uint8_t)MAC_PIB().mac_association_permit));
 ...
 */
    ZB_MEMCPY(ptr, (zb_uint8_t *)&MAC_PIB().mac_beacon_payload,
...
void zb_handle_beacon_req(zb_uint8_t param) ZB_CALLBACK
{
    ...
    TRACE_MSG(TRACE_MAC1,
              ">> zb_handle_beacon_req dev_type 0x%x mac_beacon_payload_length 0x%x",
              (FMT__D_D, MAC_CTX().dev_type,
               MAC_PIB().mac_beacon_payload_length));
    ...
    if (MAC_CTX().dev_type == MAC_DEV_FFD
        ...
        TRACE_MSG(TRACE_MAC1,
                  "we are %d (!FFD) or no b.payload - ignore beacon req",
                  (FMT__D, MAC_CTX().dev_type));
    }
...
void zb_mlme_handle_orphan_response(zb_uint8_t param) ZB_CALLBACK
{
    ...
    ZB_MEMCPY(&oresp,
              ZB_GET_BUF_PARAM(MAC_CTX().pending_buf, zb_mac_orphan_response_t),
              sizeof(oresp));
    req = ZB_GET_BUF_PARAM((zb_buf_t *)ZG->mac.mac_ctx.pending_buf,
                           zb_mlme_start_req_t);
    ...
    TRACE_MSG(TRACE_MAC1,
              "<<zb_mlme_handle_orphan_response, continue scheduled", (FMT__0));
}
...
void zb_mlme_handle_orphan_response_continue(zb_uint8_t param) ZB_CALLBACK
{
    ...
    ret = zb_check_cmd_tx_status();
    ZVUNUSED(param);
    TRACE_MSG(TRACE_MAC1, ">>zb_mlme_handle_orphan_response_continue",
              (FMT__0));
    ZB_MEMCPY(&oresp,
              ZB_GET_BUF_PARAM(MAC_CTX().pending_buf, zb_mac_orphan_response_t),
              sizeof(oresp));
    if (ret == RET_OK) {
        zb_mlme_comm_status_indication_t *ind = ZB_GET_BUF_PARAM(
            MAC_CTX().pending_buf, zb_mlme_comm_status_indication_t);
        ...
        ZB_SCHEDULE_CALLBACK(zb_mlme_comm_status_indication,
