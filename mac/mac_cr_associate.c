 ...
 */
    ...
    params = ZB_GET_BUF_PARAM(
        MAC_CTX().pending_buf, zb_mlme_associate_response_t);
    ZB_ASSERT(params);
 ...
 */
    ...
    mhr_len = zb_mac_calculate_mhr_length(ZB_ADDR_64BIT_DEV, ZB_ADDR_64BIT_DEV,
                                          1);
    packet_length = mhr_len;
    packet_length +=
        (sizeof(zb_uint8_t) + sizeof(zb_uint16_t) + sizeof(zb_uint8_t));
 ...
 */
    ...
    TRACE_MSG(TRACE_MAC3, "ASS RESP dst mod %hi addr " TRACE_FORMAT_64,
              (FMT__H_A,
    ...
    TRACE_MSG(TRACE_MAC3, "ASS RESP src mod %hi addr " TRACE_FORMAT_64,
              (FMT__H_A,
    ...
    *ptr = params->status;
    ...
    if (ret == RET_ERROR) {
        ...
        zb_mac_send_comm_status(MAC_CTX().pending_buf,
                                ZB_GET_MAC_STATUS(), MAC_CTX().pending_buf);
        MAC_CTX().pending_buf = NULL;
    }
    TRACE_MSG(TRACE_MAC2, "<<handle_ass_resp set mac 1 st %hi ret %i",
              (FMT__D, ret));
    return ret;
}
...
#endif
    if (ret == RET_OK) {
        zb_mlme_associate_indication_t *ass_indication = ZB_GET_BUF_PARAM(
            MAC_CTX().pending_buf, zb_mlme_associate_indication_t);
/*
   Coordinator side
