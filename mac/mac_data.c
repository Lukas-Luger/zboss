 ...
 */
...
#if defined ZB_TRANSPORT_8051_DATA_SPI && defined ZB_ED_ROLE && \
    !defined ZB_DBG_NO_IDLE
    ...
    if (params->dst_addr_mode != ZB_ADDR_NO_ADDR) {
        mhr_len = zb_mac_calculate_mhr_length(params->src_addr_mode,
                                              params->dst_addr_mode, 1);
    }
    else {
        TRACE_MSG(TRACE_MAC1,
                  "dst addr mode ZB_ADDR_NO_ADDR is not supported (beacon frame case)",
                  (FMT__0));
        ZB_ASSERT(0);
    }
    ...
    while (ret == RET_OK) {
        ...
        if (ZB_MAC_GET_ACK_OK()) {
            ...
            if (ZB_MAC_GET_ACK_FAIL() || ZB_MAC_GET_ACK_TIMEOUT()) {
                ...
                ZB_SCHEDULE_ALARM_CANCEL(zb_mac_ack_timeout,
                                         ZB_ALARM_ANY_PARAM);             /* cancel scheduled alarm */
...
#endif
    if (ret == RET_OK) {
        TRACE_MSG( TRACE_MAC1, "pending data %hd",
                   (FMT__H, ZB_MAC_GET_PENDING_DATA()));
        if (ZB_MAC_GET_PENDING_DATA()) {
            ZB_MAC_SET_INDIRECT_DATA_REQUEST();
            MAC_CTX().rt_ctx.indirect_data.buf_ref = 0;
            ZB_SCHEDULE_ALARM(zb_mac_indirect_data_timeout,
                              data_req_cmd_params->cb_type,
...
zb_ret_t zb_mcps_data_request_fill_hdr(zb_buf_t *data_req)
{
    ...
    TRACE_MSG(TRACE_MAC2, ">> zb_mcps_data_request_fill_hdr p %p",
              (FMT__P, data_req));
...
/* TODO: add correct parameters check */
    ...
    data_req_params->mhr_len = zb_mac_calculate_mhr_length(
        ZB_ADDR_16BIT_DEV_OR_BROADCAST, ZB_ADDR_16BIT_DEV_OR_BROADCAST, 1);
#else
    data_req_params->mhr_len = zb_mac_calculate_mhr_length(
    ...
    if (data_req_params->security_level) {
        ...
        if (MAC_PIB().mac_frame_counter == (zb_uint32_t) ~0) {
            ...
            TRACE_MSG(TRACE_MAC2, "<< zb_mcps_data_request_fill_hdr ret %d",
                      (FMT__D, ret));
            return ret;
        }
    }
    ...
    ZB_ASSERT(
        (data_req->u.hdr.len + (zb_ushort_t)data_req_params->mhr_len) <= 255);
 ...
 */
    ...
    ZB_FCF_SET_ACK_REQUEST_BIT(
        mhr.frame_control,
        !!(data_req_params->tx_options & MAC_TX_OPTION_ACKNOWLEDGED_BIT));
#ifndef ZB_MAC_EXT_DATA_REQ
    ...
    ZB_MEMCPY(&mhr.dst_addr, &data_req_params->dst_addr,
              sizeof(union zb_addr_u));
    mhr.src_pan_id = MAC_PIB().mac_pan_id;
    ZB_MEMCPY(&mhr.src_addr, &data_req_params->src_addr,
              sizeof(union zb_addr_u));
#endif
    ...
    if (data_req_params->tx_options & MAC_TX_OPTION_INDIRECT_TRANSMISSION_BIT) {
        ...
        ZB_MEMCPY(&pend_data.dst_addr, &data_req_params->dst_addr,
                  sizeof(union zb_addr_u));
#endif
    ...
    if (ret == RET_OK || ret == RET_PENDING) {
        ...
        if (data_req_params->security_level) {
            ...
            ptr +=
                (data_req_params->mhr_len - MAC_SECUR_LEV5_KEYID1_AUX_HDR_SIZE);
            ...
            *ptr = data_req_params->key_index;
             ...
             ZB_GET_MAC_STATUS() == MAC_TRANSACTION_OVERFLOW) {
...
zb_ret_t zb_mac_check_security(zb_buf_t *data_buf)
{
    ...
    TRACE_MSG(TRACE_MAC2, ">> zb_mac_check_security data_buf %p",
              (FMT__P, data_buf));
...
void zb_handle_mcps_data_req(zb_uint8_t param) ZB_CALLBACK
{
    ...
    TRACE_MSG(TRACE_MAC2, "is_unicast %hd, dst_addr 0x%x",
              (FMT__H_D, is_unicast, data_req_params->dst_addr));
#else
    is_unicast =
        !(data_req_params->dst_addr_mode == ZB_ADDR_16BIT_DEV_OR_BROADCAST
          && ZB_NWK_IS_ADDRESS_BROADCAST(data_req_params->dst_addr.
                                         addr_short));
    TRACE_MSG(TRACE_MAC2, "is_unicast %hd", (FMT__H, is_unicast));
#endif

    TRACE_MSG(TRACE_MAC1, "msdu_handle %hd tx_options %hd",
              (FMT__H_H, data_req_params->msdu_handle,
               data_req_params->tx_options));
...
send_command:
        ...
        if (ret == RET_OK) {
            ...
            if (MAC_CTX().pending_buf->u.hdr.encrypt_type != ZB_SECUR_NO_ENCR) {
                ...
                ret = ZB_TRANS_SEND_COMMAND(data_req_params->mhr_len,
                                            MAC_CTX().encryption_buf);
            }
            else
#endif
            {
        ...
        if (ret != RET_PENDING) {
            ...
            }
            {
                zb_mcps_data_confirm_params_t *confirm_params =
                    ZB_GET_BUF_PARAM(
                        MAC_CTX().pending_buf, zb_mcps_data_confirm_params_t);
            ...
            TRACE_MSG(TRACE_MAC3, "mac status 0x%hx %hd",
                      (FMT__H_H, ZB_GET_MAC_STATUS(),
...
void zb_handle_data_frame(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (ret == RET_OK) {
        ZB_SCHEDULE_CALLBACK(zb_mcps_data_indication,
                             ZB_REF_FROM_BUF(MAC_CTX().pending_buf));
    }
...
void indirect_data_callback_caller(zb_uint8_t param) ZB_CALLBACK
{
    call_indirect_data_callback(MAC_CTX().rt_ctx.indirect_data.cb_type,
                                MAC_CTX().rt_ctx.indirect_data.cb_status,
                                ZB_BUF_FROM_REF(param));
}

/*
   Calls specific callback during indirect data transfer
 */
zb_ret_t call_indirect_data_callback(zb_callback_type_t cb_type,
                                     zb_uint8_t cb_status, zb_buf_t *buf)
{

    TRACE_MSG(TRACE_MAC3, ">>call_ind_cb %i, %hd, %p",
              (FMT__D_H_P, (int)cb_type, cb_status, buf));
    if (cb_type == MAC_ASS_CONFIRM_CALLBACK) {
        ZB_MAC_CLEAR_ASS_REQUEST();
        if (buf) {
            zb_mlme_associate_confirm_t *ass_confirm_param;
            ass_confirm_param = ZB_GET_BUF_PARAM(buf,
            ...
            ZB_SCHEDULE_CALLBACK(zb_mlme_associate_confirm,
                                 ZB_REF_FROM_BUF(buf));
        }
    }
