...
void zb_mlme_scan_request(zb_uint8_t param) ZB_CALLBACK
{
    ...
    params = ZB_GET_BUF_PARAM((zb_buf_t *)ZB_BUF_FROM_REF(
                                  param), zb_mlme_scan_params_t);
    ZB_ASSERT(params);
    MAC_CTX().mac_status = MAC_SUCCESS;
    scan_type = params->scan_type;

    if (params->scan_duration > ZB_MAX_SCAN_DURATION_VALUE &&
        scan_type != ORPHAN_SCAN) {
        ret = RET_ERROR;
        MAC_CTX().mac_status = MAC_INVALID_PARAMETER;
    }
    ...
    if (!handle_scan_called) {
        ...
        scan_confirm = ZB_GET_BUF_PARAM(
            MAC_CTX().pending_buf, zb_mac_scan_confirm_t);
        scan_confirm->status = (ret == RET_OK) ? MAC_SUCCESS :
                               MAC_CTX().mac_status !=
                               MAC_SUCCESS ? MAC_CTX().mac_status :
                               MAC_INVALID_PARAMETER;
        scan_confirm->scan_type = scan_type;
        ZB_SCHEDULE_CALLBACK(zb_mlme_scan_confirm,
 ...
 */
    ...
    {
        zb_mlme_scan_params_t *scan_params = ZB_GET_BUF_PARAM(
            MAC_CTX().pending_buf, zb_mlme_scan_params_t);
        MAC_CTX().unscanned_channels = scan_params->channels;
        /* timeout is calculated in beacon intervals */
        MAC_CTX().rt_ctx.ed_scan.scan_timeout =
            (1l << scan_params->scan_duration) + 1;
    }
    ZB_BUF_REUSE(MAC_CTX().pending_buf);
    scan_confirm =
        ZB_GET_BUF_PARAM(MAC_CTX().pending_buf, zb_mac_scan_confirm_t);
    ...
    ret = ZB_SCHEDULE_ALARM(zb_mlme_scan_step, 0,
                            MAC_CTX().rt_ctx.ed_scan.scan_timeout);

#else  /* ZB_NS_BUILD */
    ZB_BZERO(&scan_confirm->list.energy_detect[0],
             sizeof(scan_confirm->list.energy_detect));
    scan_confirm->result_list_size = ZB_MAC_SUPPORTED_CHANNELS;
    ret =
        ZB_SCHEDULE_CALLBACK(zb_mlme_scan_confirm, ZB_REF_FROM_BUF(
                                 MAC_CTX().pending_buf));
#ifdef ZB_CHANNEL_ERROR_TEST
    /* channel interference test, show energy on current channel */
    TRACE_MSG(TRACE_MAC3, "ch_err_test %hd, logical_channel %hd, ch index %hd",
              (FMT__H_H_H,
               ZB_MAC_GET_CHANNEL_ERROR_TEST(),
               ZB_MAC_GET_CURRENT_LOGICAL_CHANNEL(),
               ZB_MAC_GET_CURRENT_LOGICAL_CHANNEL()
               - ZB_MAC_START_CHANNEL_NUMBER));

    if (ZB_MAC_GET_CHANNEL_ERROR_TEST()) {
        scan_confirm->list.energy_detect[ZB_MAC_GET_CURRENT_LOGICAL_CHANNEL() -
                                         ZB_MAC_START_CHANNEL_NUMBER] =
...
void zb_mlme_scan_step(zb_uint8_t param) ZB_CALLBACK
{
    ...
    for (;
         channel_number <
         ZB_MAC_START_CHANNEL_NUMBER + ZB_MAC_MAX_CHANNEL_NUMBER;
         channel_number++) {
    ...
    if (channel_number ==
        ...
        if (scan_params->scan_type == ACTIVE_SCAN) {
            ...
            if (MAC_CTX().rt_ctx.active_scan.pan_desc_buf_param !=
                ZB_UNDEFINED_BUFFER) {
                desc_list_buf = ZB_BUF_FROM_REF(
                    MAC_CTX().rt_ctx.active_scan.pan_desc_buf_param);
                ZB_BUF_COPY(MAC_CTX().pending_buf, desc_list_buf);
                desc_count = ZB_BUF_LEN(desc_list_buf) /
                             sizeof(zb_pan_descriptor_t);
                TRACE_MSG(TRACE_MAC3, "copied %hd pan desc", (FMT__H,
                                                              desc_count));
        ...
        if (scan_params->scan_type == ED_SCAN) {
            TRACE_MSG(TRACE_MAC1, "restoring original channel after ed_scan",
                      (FMT__0));
        ...
        scan_confirm = ZB_GET_BUF_PARAM(
            MAC_CTX().pending_buf, zb_mac_scan_confirm_t);
        TRACE_MSG(TRACE_MAC3, "beacon found %hd",
                  (FMT__H, MAC_CTX().rt_ctx.active_scan.beacon_found));
        if (scan_params->scan_type == ED_SCAN ||
            MAC_CTX().rt_ctx.active_scan.beacon_found
 ...
 */
    ...
    TRACE_MSG(TRACE_MAC3, "set beacon mode ret %d, param channels 0x%x",
              (FMT__D_D, ret, scan_params->channels));
    MAC_CTX().unscanned_channels = scan_params->channels;
    ...
    TRACE_MSG(TRACE_MAC2, "chan mask %x %x , chan %hd",
              (FMT__D_D_H, ((zb_uint16_t *)&scan_params->channels)[0],
               ((zb_uint16_t *)&scan_params->channels)[1],
               channel_number));
...
zb_ret_t zb_mlme_orphan_scan() ZB_SDCC_REENTRANT
{
    ...
    ret =
        ZB_SCHEDULE_ALARM(zb_mlme_scan_step, 0, ZB_MAC_PIB_RESPONSE_WAIT_TIME);
 ...
 */
    ...
    mhr_len = zb_mac_calculate_mhr_length(ZB_ADDR_64BIT_DEV,
                                          ZB_ADDR_16BIT_DEV_OR_BROADCAST, 1);
    {
...
void zb_mac_store_pan_desc(zb_buf_t *beacon_buf)
{
    ...
    if (MAC_CTX().rt_ctx.active_scan.pan_desc_buf_param ==
        ...
        MAC_CTX().rt_ctx.active_scan.pan_desc_buf_param = ZB_REF_FROM_BUF(
            beacon_buf);
    }
    else {
        desc_list_buf = ZB_BUF_FROM_REF(
    ...
    if ((ZB_BUF_GET_FREE_SIZE(desc_list_buf) >=
         (sizeof(zb_pan_descriptor_t) + sizeof(zb_mac_scan_confirm_t))) &&
        desc_count < ZB_ACTIVE_SCAN_MAX_PAN_DESC_COUNT) {
        ZB_BUF_ALLOC_RIGHT(desc_list_buf, sizeof(zb_pan_descriptor_t),
                           pan_desc_buf);
