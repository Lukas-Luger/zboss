...
void zb_accept_data_request_cmd(zb_uint8_t param) ZB_CALLBACK
{
    ...
    TRACE_MSG(TRACE_MAC2, "<< zb_accept_data_request_cmd, ret: %d", (FMT__H,
                                                                     ret));
}



static zb_int8_t check_pending_data(zb_mac_mhr_t *mhr,
                                    zb_uint8_t index) ZB_SDCC_REENTRANT
{
    zb_int8_t i;
    ...
    TRACE_MSG(TRACE_MAC2, "check_pending_data: src_addr_mode %hd",
              (FMT__H, src_addr_mode));
    ...
    for (i = index; i < ZB_MAC_PENDING_QUEUE_SIZE; i++) {
        ...
        if (MAC_CTX().pending_data_queue[i].pending_data != NULL &&
            ...
            if (src_addr_mode == ZB_ADDR_64BIT_DEV) {
                data_found = ZB_IEEE_ADDR_CMP(
                    MAC_CTX().pending_data_queue[i].dst_addr.addr_long,
                    mhr->src_addr.addr_long);
                TRACE_MSG(TRACE_MAC2,
                          "data_found %hd pending dst " TRACE_FORMAT_64 " src " TRACE_FORMAT_64,
                          (FMT__H_A_A, data_found,
                           TRACE_ARG_64(MAC_CTX().pending_data_queue[i].dst_addr
                                        .addr_long),
                ...
                data_found =
                    (mhr->src_addr.addr_short ==
                     MAC_CTX().pending_data_queue[i].dst_addr.addr_short);
            }
        }
        if (data_found) {
            break;
 ...
 */
static zb_ret_t fill_empty_frame(zb_mac_mhr_t *mhr_req,
                                 zb_buf_t *buf) ZB_SDCC_REENTRANT
{
    zb_ret_t ret = RET_OK;
    zb_mac_mhr_t mhr;
    zb_uint_t mhr_len;
    ...
    TRACE_MSG(TRACE_MAC3, ">> fill_empty_frame mhr_req %p, buf%p",
              (FMT__P_P, mhr_req, buf));

    mhr_len =
        zb_mac_calculate_mhr_length(ZB_FCF_GET_DST_ADDRESSING_MODE(mhr_req->
                                                                   frame_control),
                                    ZB_FCF_GET_SRC_ADDRESSING_MODE(mhr_req
                                                                   ->
                                                                   frame_control),
                                    1);
 ...
 */
    ...
    ZB_FCF_SET_DST_ADDRESSING_MODE(mhr.frame_control, ZB_FCF_GET_SRC_ADDRESSING_MODE(
                                       mhr_req->frame_control));
    ZB_FCF_SET_FRAME_VERSION(mhr.frame_control, MAC_FRAME_VERSION);
    ZB_FCF_SET_SRC_ADDRESSING_MODE(mhr.frame_control, ZB_FCF_GET_DST_ADDRESSING_MODE(
                                       mhr_req->frame_control));
...
zb_ret_t zb_handle_data_request_cmd() ZB_SDCC_REENTRANT
{
    ...
    if (!MAC_CTX().pending_buf) {
        ...
        TRACE_MSG(TRACE_MAC2,
                  "<< zb_handle_data_request_cmd ret (invalid params) %i",
                  (FMT__D, ret));
        return ret;
    }

    {
        ...
        data_req_ctx = ZB_GET_BUF_PARAM(
            MAC_CTX().pending_buf, zb_mac_data_req_ctx_t);
    ...
    if (data_found_index >= 0) {
        TRACE_MSG(TRACE_MAC3, "pending dat %p",
                  (FMT__P,
                   MAC_CTX().pending_data_queue[data_found_index].pending_data));
        fcf =
            ZB_MAC_GET_FCF_PTR(ZB_BUF_BEGIN(MAC_CTX().pending_data_queue[
                                                data_found_index
                                            ].pending_data));
        data_found = 1;

        /* get MHR stored in pending_data, need to analyse its frame type*/
        cmd_ptr = ZB_BUF_BEGIN(
            MAC_CTX().pending_data_queue[data_found_index].pending_data);
        mhr_pend_len = zb_parse_mhr(&mhr_pend, cmd_ptr);
    }
    TRACE_MSG(TRACE_MAC2, "data_found_index %hd, fcf %p, data_found %hd",
              (FMT__H_P_H, data_found_index, fcf, data_found));
...
send_data_resp:
    ...
    if (ret == RET_OK && data_found) {
        ...
        TRACE_MSG(TRACE_MAC3, "will send data, frame type %i",
                  (FMT__D, ZB_FCF_GET_FRAME_TYPE(fcf)));
        ...
        switch (ZB_FCF_GET_FRAME_TYPE(fcf)) {
            ...
            case MAC_FRAME_DATA: /* data frame is sent */
            {
                ...
                if (ret == RET_OK) {
                    ...
                    ZB_TX_TOTAL_INC();
#ifdef ZB_SECURITY
                    if (MAC_CTX().pending_data_queue[data_found_index].
                        pending_data->u.hdr.encrypt_type != ZB_SECUR_NO_ENCR) {
                        b_tmp = MAC_CTX().encryption_buf;
                        /* we need to add fcs to not not encrypted buffer, we need it later to reencrypt and resend */
                        MAC_ADD_FCS(MAC_CTX().pending_data_queue[
                                        data_found_index].pending_data);
                    }
                    else
#endif
                    {
                        b_tmp =
                            MAC_CTX().pending_data_queue[data_found_index].
                            pending_data;
                    }

                    if (check_pending_data(&mhr_req,
                                           data_found_index + 1) != -1) {
...
#ifdef ZB_MANUAL_ACK
        if (ret == RET_OK && fcf) {
            TRACE_MSG(TRACE_MAC2, "ack req bit %i",
                      (FMT__D, ZB_FCF_GET_ACK_REQUEST_BIT(fcf)));
            if (ZB_FCF_GET_ACK_REQUEST_BIT(fcf)) {
                if (ret == RET_OK) {
                    ...
                    if (ret == RET_PENDING) {
                        /* retry data send */
                        TRACE_MSG(TRACE_MAC2, "ack is not received, try again",
                                  (FMT__0));
                        ret = RET_OK;
                        goto send_data_resp;
                    }
...
void zb_handle_data_request_cmd_continue(zb_uint8_t param) ZB_CALLBACK
{
    ...
    zb_uint8_t data_found_index =
        MAC_CTX().mac_storage.mac_data_req.data_found_index;
    zb_uint8_t mhr_pend_len = MAC_CTX().mac_storage.mac_data_req.mhr_pend_len;
    zb_mac_mhr_t mhr_pend;

    TRACE_MSG(TRACE_MAC2, ">> zb_handle_data_request_cmd ret_continue",
              (FMT__0));
    ...
    if (ret != (zb_uint8_t)RET_BLOCKED && data_found && fcf) {
        TRACE_MSG(TRACE_MAC2, "frame type %i data_found_index %hd",
                  (FMT__D_H, ZB_FCF_GET_FRAME_TYPE(fcf), data_found_index));
        if (ZB_FCF_GET_FRAME_TYPE(fcf) == MAC_FRAME_COMMAND) {
            ...
            if (*cmd_ptr == MAC_CMD_ASSOCIATION_RESPONSE) {
                ...
                if (ret == RET_OK) {
                    ...
                    status =
                        (ZB_GET_MAC_STATUS() !=
                         MAC_SUCCESS) ? ZB_GET_MAC_STATUS() :
                        MAC_INVALID_PARAMETER;
                }
                /* checked only 2nd parameter, coz it used only for zb_send_comm_status */
                TRACE_MSG(TRACE_MAC3, "calling zb_mac_send_comm_status",
                          (FMT__0));
                zb_mac_send_comm_status(
                    MAC_CTX().pending_data_queue[
                        data_found_index].pending_data, status,
                    MAC_CTX().pending_buf);
                MAC_CTX().pending_buf = NULL;
            }
        }
        ...
        if (data_found) {
            ...
            if (ZB_FCF_GET_FRAME_TYPE(&mhr_pend.frame_control) ==
                MAC_FRAME_COMMAND) {
                TRACE_MSG(TRACE_MAC3, "mac cmd, free buffer %p",
                          (FMT__P,
                           MAC_CTX().pending_data_queue[data_found_index].
                           pending_data));
                zb_free_buf(
                ...
                ZB_SCHEDULE_CALLBACK(zb_mcps_data_confirm,
                                     ZB_REF_FROM_BUF(MAC_CTX().
                                                     pending_data_queue[
                                                         data_found_index].
                                                     pending_data));
            }
