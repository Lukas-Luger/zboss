...
#endif
    FREQCTRL =
        (ZB_TRANSCEIVER_START_CHANNEL_NUMBER +
         (channel_number - ZB_TRANSCEIVER_START_CHANNEL_NUMBER) * 5);
}
 ...
 */
...
#ifdef ZB_TRAFFIC_DUMP_ON
    {
        ...
        ZB_BUF_INITIAL_ALLOC(
            MAC_CTX().operation_buf, sizeof(zb_uint16_t), data_ptr);
        ZB_ASSERT(data_ptr);

        /* Produce same traffic dump as 2400 */
        ZB_HTOBE16_VAL(data_ptr,
                       ((tx_fifo ? ZB_NORMAL_TX_FIFO : ZB_RX_FIFO) << 5) |
                       0x8000 );
    }
    ...
    /* actual i/o */
    {
        ...
        xptr =
            (zb_uint8_t ZB_XDATA *)(tx_fifo ? ZB_NORMAL_FIFO_ADDR :
                                    ZB_NORMAL_RXFIFO_ADDR);
        if (len == 0) {
                          ...
                          zb_buf_t *buf) ZB_SDCC_REENTRANT
{
...
#ifdef ZB_TRAFFIC_DUMP_ON
    {
        ...
        ZB_BUF_ALLOC_LEFT(buf, sizeof(zb_uint16_t) + sizeof(zb_uint8_t),
                          data_ptr);
        ZB_ASSERT( data_ptr );
        /* change for TI? */
        ZB_HTOBE16_VAL(data_ptr, (long_addr << 5) | 0x8010);
        /* wireshark plugin compatibility */
        *(zb_uint8_t *)((zb_uint8_t *)data_ptr +
                        sizeof(zb_uint16_t)) =
            *(zb_uint8_t *)(ZB_GET_BUF_PARAM(buf, zb_uint8_t));
        *(zb_uint8_t *)((zb_uint8_t *)data_ptr + sizeof(zb_uint16_t) +
                        sizeof(zb_uint8_t)) -= ZB_CC2530_FIFO_TAIL;
    }
    ZB_DUMP_OUTGOING_DATA(buf);

    /* remove headers */
    ZB_BUF_CUT_LEFT2((buf),
                     sizeof(zb_ubec_fifo_header_t) + sizeof(zb_uint16_t) +
                     sizeof(zb_uint8_t) /*header length, for wireshark compatibility*/);
#else
