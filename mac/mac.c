...
void zb_mac_init() /* __reentrant for sdcc, to save DSEG space */
{
     ...
     */
#if defined (ZB_CONFIGURABLE_MAC_PIB) || defined(ZB_NS_BUILD) || \
    defined(ZB_CC25XX)
    MAC_PIB().mac_ack_wait_duration =
        (ZB_UINT_BACKOFF_PERIOD + ZB_TURNAROUND_TIME + ZB_PHY_SHR_DURATION +
         (zb_uint16_t)(6 * ZB_PHY_SYMBOLS_PER_OCTET));
 ...
 */
    MAC_PIB().mac_base_superframe_duration = ZB_MAC_BASE_SLOT_DURATION *
 ...
 * 0xffff. unit period = aBaseSuperframeDuration (== beacon interval)*/
    MAC_PIB().mac_transaction_persistence_time =
        ZB_MAC_TRANSACTION_PERSISTENCE_TIME;
#endif
...
#else
    ZB_SCHEDULE_CALLBACK(zb_mac_parse_recv_data,
                         ZB_REF_FROM_BUF(MAC_CTX().recv_buf));
#endif /* ZB_NS_BUILD */
...
#endif
    {
...
#endif
        {
            if (ZB_FCF_GET_SECURITY_BIT(mhr.frame_control)
                && ZB_FCF_GET_FRAME_VERSION(mhr.frame_control) <
                MAC_FRAME_IEEE_802_15_4) {
                TRACE_MSG(TRACE_MAC1, "unsupported 2003 security frame dropped",
                          (FMT__0));
            }
            else {
                ret = ZB_TRUE;
...
void zb_mac_main_loop()
{
    ...
    if (MAC_CTX().tx_wait_cb) {
        ...
        if (MAC_CTX().tx_cnt) {
            ...
            MAC_CTX().tx_cnt = 0;
            TRACE_MSG(TRACE_COMMON2, "tx waiting callback called, func: %p",
                      (FMT__P, MAC_CTX().tx_wait_cb));
...
/* high priority mac layer queue. Could be used to manage callbacks priority. Currently unused. */
    ...
    {   /* checking mac callback queue */
        zb_mac_cb_ent_t *mac_cb_ent;
        while ((mac_cb_ent = ZB_RING_BUFFER_PEEK(&ZG->sched.mac_cb_q)) !=
               NULL) {
...
void zb_mac_parse_recv_data(zb_uint8_t param) ZB_CALLBACK
{
    ...
    TRACE_MSG(TRACE_MAC2,
              "frame_control %hx:%hx, seq_number %hu, dst_pan_id 0x%x, src_pan_id 0x%x dst mode %hi addr " TRACE_FORMAT_64 " src mode %hi addr " TRACE_FORMAT_64,
              (FMT__H_H_H_D_D_H_A_H_A,
               mhr.frame_control[0], mhr.frame_control[1], mhr.seq_number,
               mhr.dst_pan_id, mhr.src_pan_id,
 ...
 * now. */
            ...
            if (check_frame_dst_addr(&mhr)) {
                ...
                ZB_SCHEDULE_ALARM_CANCEL(zb_mac_indirect_data_timeout,
                                         ZB_ALARM_ANY_PARAM);
                ...
                if (ZB_MAC_GET_POLL_REQUEST() /* poll request? */
                    ...
                    ZB_BUF_FROM_REF(MAC_CTX().rt_ctx.indirect_data.buf_ref)->u.
                    hdr.status = MAC_CTX().mac_status;
                    ZB_SCHEDULE_CALLBACK(zb_mlme_poll_confirm,
                                         MAC_CTX().rt_ctx.indirect_data.buf_ref);
                }
            }
        }
        ...
        switch (ZB_FCF_GET_FRAME_TYPE(fcf)) {
            case MAC_FRAME_DATA:
                ZB_SCHEDULE_CALLBACK(zb_handle_data_frame,
                                     ZB_REF_FROM_BUF(buf));
                break;
            case MAC_FRAME_BEACON:
...
#ifdef ZB_MAC_TESTING_MODE
                {
                    ...
                    payload_len = ZB_BUF_LEN(buf) - ZB_BEACON_PAYLOAD_OFFSET(
                        mhr_len) - ZB_BEACON_PAYLOAD_TAIL;
                    TRACE_MSG(TRACE_NWK1, "payload len %hd, auto_req %hd",
                              (FMT__H_H, payload_len,
 ...
 */
zb_uint8_t zb_mac_calculate_mhr_length(zb_uint8_t src_addr_mode,
                                       zb_uint8_t dst_addr_mode,
                                       zb_uint8_t panid_compression)
{
 ...
 */
    ...
    if (((*cmd_ptr) == MAC_CMD_BEACON_REQUEST) &&
        (!ZG->nwk.handle.joined_pro)) {                                       /* Command type check */
    ...
    if ((*cmd_ptr) == MAC_CMD_ASSOCIATION_RESPONSE) {
        zb_mlme_associate_confirm_t *ass_confirm_param =
            ZB_GET_BUF_PARAM((zb_buf_t *)ZB_BUF_FROM_REF(
                                 param), zb_mlme_associate_confirm_t);


        if (ZB_FCF_GET_SRC_ADDRESSING_MODE(mhr.frame_control) ==
            ZB_ADDR_64BIT_DEV) {
            ZB_IEEE_ADDR_COPY(ass_confirm_param->parent_address,
        ...
        if (ZB_MAC_GET_ASS_REQUEST()) {
            cmd_ptr++;
            zb_get_next_letoh16(&ass_confirm_param->assoc_short_address,
                                &cmd_ptr);
            MAC_PIB().mac_short_address =
                ass_confirm_param->assoc_short_address;
            TRACE_MSG(TRACE_MAC3, "saddr set: %d",
                      (FMT__D, MAC_PIB().mac_short_address));
            ZB_UPDATE_SHORT_ADDR();
            ass_confirm_param->status = *cmd_ptr;
            ZB_BUF_FROM_REF(param)->u.hdr.status = ass_confirm_param->status;
            TRACE_MSG(TRACE_MAC1, "zb_mlme_asociate_confirm scheduled",
                      (FMT__0));
    ...
    if ((*cmd_ptr) == MAC_CMD_COORDINATOR_REALIGNMENT) {
        ...
        if (MAC_CTX().mlme_scan_in_progress
            && ZB_GET_BUF_PARAM(MAC_CTX().pending_buf,
                                zb_mlme_scan_params_t)->scan_type ==
            ORPHAN_SCAN) {
            ...
            TRACE_MSG(TRACE_MAC3, "saddr set: %d",
                      (FMT__0, MAC_PIB().mac_short_address));
            ZB_UPDATE_SHORT_ADDR();
            TRACE_MSG(TRACE_MAC1, "set pan id %d", (FMT__D,
                                                    MAC_PIB().mac_pan_id));
            ZB_UPDATE_PAN_ID();
        }
    }
    ...
    if ((*cmd_ptr) == MAC_CMD_ORPHAN_NOTIFICATION) {
        if (ZB_FCF_GET_SRC_ADDRESSING_MODE(mhr.frame_control) ==
            ZB_ADDR_64BIT_DEV) {
            zb_mac_orphan_ind_t *orphan_ind = ZB_GET_BUF_PARAM((zb_buf_t *)ZB_BUF_FROM_REF(
                                                                   param),
                                                               zb_mac_orphan_ind_t);
            ...
            /* mailformed packet */
            TRACE_MSG(TRACE_MAC1, "orph ind should have long src addr",
                      (FMT__0));
        }
    }
    else
 ...
 */
    if (MAC_CTX().ack_dsn == mhr.seq_number) {
        ZB_MAC_SET_ACK_OK();
        TRACE_MSG(TRACE_MAC2,
                  "ack ok dsn %hi, frame_pending_bit %d, frame_ctrl %hx %hx",
                  (FMT__H_D_H_H, MAC_CTX().ack_dsn,
                   ZB_FCF_GET_FRAME_PENDING_BIT(mhr.frame_control),
                   mhr.frame_control[0],
                   mhr.frame_control[1]));
/* check if there is pending data, 7.2.2.3.1 Acknowledgment frame MHR fields */
        ...
        TRACE_MSG(TRACE_MAC2, "ack failed dsn %hi, recv %hi - ignore",
                  (FMT__H_H, MAC_CTX().ack_dsn, mhr.seq_number));
    }
#endif
    TRACE_MSG(TRACE_MAC3, "free buf %p", (FMT__P, request));
    zb_free_buf(request);
...
zb_void_t zb_fcf_set_src_addressing_mode(zb_uint8_t *p_fcf,
 ...
 */
zb_void_t zb_fcf_set_dst_addressing_mode(zb_uint8_t *p_fcf,
                                         zb_uint8_t addr_mode)
{
    ZB_ASSERT((addr_mode) == 0 ||
              (addr_mode) == 1 ||
...
void zb_mac_resp_timeout(zb_uint8_t param) ZB_CALLBACK
{
    ...
    ZVUNUSED(param);
    params =
        ZB_GET_BUF_PARAM(MAC_CTX().pending_buf, zb_mlme_associate_params_t);
    TRACE_MSG(TRACE_MAC2, "mac_resp_tmo", (FMT__0));
    {
        ...
        ZB_MEMCPY(&data_req_cmd_params.src_addr,
                  &MAC_PIB().mac_extended_address, sizeof(zb_ieee_addr_t));
        data_req_cmd_params.dst_addr_mode = params->coord_addr_mode;
        ZB_MEMCPY(&data_req_cmd_params.dst_addr, &params->coord_addr,
...
void zb_mac_indirect_data_timeout(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (MAC_CTX().rt_ctx.indirect_data.buf_ref) {
        call_indirect_data_callback((zb_callback_type_t)param, MAC_NO_DATA, ZB_BUF_FROM_REF(
                                        MAC_CTX().rt_ctx.indirect_data.buf_ref));
...
zb_ret_t zb_check_cmd_tx_status()
{
    ...
    if (ZB_TRANS_CHECK_CHANNEL_ERROR()) {
        ...
        TRACE_MSG(TRACE_MAC1,
                  "error busy 0x%hx, tx retry exceeded 0x%hx, mac_status 0x%hx",
                  (FMT__H_H,
...
static zb_bool_t check_frame_dst_addr(zb_mac_mhr_t *mhr) ZB_SDCC_REENTRANT
{
    TRACE_MSG(TRACE_MAC3, "dst addr mode %i",
              (FMT__D, ZB_FCF_GET_DST_ADDRESSING_MODE(mhr->frame_control)));
    if (ZB_FCF_GET_DST_ADDRESSING_MODE(mhr->frame_control) ==
        ...
        TRACE_MSG(TRACE_MAC3, "mhr dst %i, pib short a %i", (FMT__D_D,
                                                             ...
                                                             MAC_PIB().
                                                             mac_short_address));
        return (zb_bool_t)(mhr->dst_addr.addr_short ==
                           MAC_PIB().mac_short_address);
    }
    else {
        /* 64 bit case */
        TRACE_MSG(TRACE_MAC3,
                  "mhr addr " TRACE_FORMAT_64 " pib ext" TRACE_FORMAT_64,
                  (FMT__A_A,
                   TRACE_ARG_64(
                       mhr->dst_addr.addr_long),
                   TRACE_ARG_64(
                       MAC_PIB().mac_extended_address)));
        return ZB_64BIT_ADDR_CMP(mhr->dst_addr.addr_long,
                                 MAC_PIB().mac_extended_address);
    }
}
...
void zb_poll_request(zb_uint8_t param) ZB_CALLBACK
{
    if (!param) {
        ...
        zb_buf_t *buf = (zb_buf_t *)ZB_BUF_FROM_REF(param);
        zb_mlme_poll_request_t *req = ZB_GET_BUF_PARAM(buf,
...
void zb_handle_poll_request(zb_uint8_t param) ZB_CALLBACK
{
    ...
    data_req.src_addr_mode =
        (ZB_PIB_SHORT_ADDRESS() <
         ZB_MAC_SHORT_ADDR_NOT_ALLOCATED) ? ZB_ADDR_16BIT_DEV_OR_BROADCAST :
        ZB_ADDR_64BIT_DEV;
    if (data_req.src_addr_mode == ZB_ADDR_64BIT_DEV) {
        ZB_IEEE_ADDR_COPY(data_req.src_addr.addr_long,
    ...
    if (data_req.dst_addr_mode == ZB_ADDR_64BIT_DEV) {
        ZB_IEEE_ADDR_COPY(data_req.dst_addr.addr_long,
                          request->coord_addr.addr_long);

        TRACE_MSG(TRACE_MAC3, "poll_request: dst_addr (long) " TRACE_FORMAT_64,
    ...
    ret = zb_mac_get_indirect_data(&data_req);
    {
        if (ret == RET_OK) {
            ...
            MAC_CTX().rt_ctx.indirect_data.buf_ref = ZB_REF_FROM_BUF(
                MAC_CTX().pending_buf);
        }
        else {
            MAC_CTX().pending_buf->u.hdr.status =
                (MAC_CTX().mac_status ==
                 MAC_SUCCESS) ? MAC_INVALID_PARAMETER : MAC_CTX().mac_status;
            ZB_SCHEDULE_CALLBACK(zb_mlme_poll_confirm,
                                 ZB_REF_FROM_BUF(MAC_CTX().pending_buf));
        }
        MAC_CTX().pending_buf = NULL;
    }
...
void zb_mlme_get_request(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (req.pib_attr == ZB_PHY_PIB_CURRENT_CHANNEL) {
        ZB_BUF_INITIAL_ALLOC(buf,
                             sizeof(zb_mlme_get_confirm_t) + sizeof(zb_uint8_t),
                             conf);
        conf->pib_length = sizeof(zb_uint8_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_mlme_get_confirm_t)) = MAC_PIB().phy_current_channel;
        conf->pib_index = 0;
    }
        ...
        conf->pib_length = sizeof(zb_uint8_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_mlme_get_confirm_t)) = MAC_PIB().phy_current_page;
        conf->pib_index = 0;
    }
    else if (req.pib_attr == ZB_PIB_ATTRIBUTE_ACK_WAIT_DURATION) {
        ZB_BUF_INITIAL_ALLOC(buf,
                             sizeof(zb_mlme_get_confirm_t) + sizeof(zb_uint16_t),
                             conf);
        conf->pib_length = sizeof(zb_uint16_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_mlme_get_confirm_t)) = ZB_MAC_PIB_ACK_WAIT_DURATION;
        conf->pib_index = 0;
    }
        ...
        conf->pib_length = sizeof(zb_uint8_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_mlme_get_confirm_t)) = MAC_PIB().mac_association_permit;
        conf->pib_index = 0;
    }
        ...
        conf->pib_length = sizeof(zb_uint8_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_mlme_get_confirm_t)) = MAC_PIB().mac_batt_life_ext;
        conf->pib_index = 0;
    }
    else if (req.pib_attr == ZB_PIB_ATTRIBUTE_BEACON_PAYLOAD) {
        ZB_BUF_INITIAL_ALLOC(buf,
                             sizeof(zb_mlme_get_confirm_t) +
                             sizeof(ZB_PIB_BEACON_PAYLOAD()), conf);
        ...
        conf->pib_length = sizeof(zb_uint8_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_mlme_get_confirm_t)) = MAC_PIB().mac_beacon_payload_length;
        conf->pib_index = 0;
    }
        ...
        conf->pib_length = sizeof(zb_uint8_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_mlme_get_confirm_t)) = MAC_PIB().mac_beacon_order;
        conf->pib_index = 0;
    }
        ...
        *(((zb_uint8_t *)conf) + sizeof(zb_mlme_get_confirm_t)) = ZB_MAC_BSN();
        ...
        conf->pib_length = sizeof(zb_uint16_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_mlme_get_confirm_t)) = ZB_PIB_COORD_SHORT_ADDRESS();
        conf->pib_index = 0;
    }
        ...
        *(((zb_uint8_t *)conf) + sizeof(zb_mlme_get_confirm_t)) = ZB_MAC_DSN();
        ...
        conf->pib_length = sizeof(zb_uint16_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_mlme_get_confirm_t)) = ZB_PIB_SHORT_PAN_ID();
        conf->pib_index = 0;
    }
        ...
        conf->pib_length = sizeof(zb_uint8_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_mlme_get_confirm_t)) = ZB_PIB_RX_ON_WHEN_IDLE();
        conf->pib_index = 0;
    }
        ...
        conf->pib_length = sizeof(zb_uint16_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_mlme_get_confirm_t)) = ZB_PIB_SHORT_ADDRESS();
        conf->pib_index = 0;
    }
        ...
        conf->pib_length = sizeof(zb_uint16_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_mlme_get_confirm_t)) = MAC_PIB().mac_superframe_order;
        conf->pib_index = 0;
    }
        ...
        *(((zb_uint16_t *)conf) +
        ...
        conf->pib_length = sizeof(zb_uint16_t);
        *(((zb_uint16_t *)conf) +
          sizeof(zb_mlme_get_confirm_t)) = ZB_MAC_PIB_MAX_FRAME_TOTAL_WAIT_TIME;
        conf->pib_index = 0;
    }
        ...
        conf->pib_length = sizeof(zb_uint8_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_mlme_get_confirm_t)) = ZB_MAC_MAX_FRAME_RETRIES;
        conf->pib_index = 0;
    }
        ...
        conf->pib_length = sizeof(zb_uint8_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_mlme_get_confirm_t)) = ZB_MAC_PIB_RESPONSE_WAIT_TIME;
        conf->pib_index = 0;
    }
    else {
        ZB_BUF_INITIAL_ALLOC(buf, sizeof(zb_mlme_get_confirm_t), conf);
    ...
    if ((conf->pib_attr == ZB_PIB_ATTRIBUTE_BEACON_PAYLOAD_LENGTH)
        ...
        *(((zb_uint8_t *)conf) +
        ...
        TRACE_MSG(TRACE_NWK2, "attr %hd get param %hd",
                  (FMT__H_H, conf->pib_attr,
                   *(((zb_uint8_t *)conf) + sizeof(zb_mlme_get_confirm_t))));
    }

    TRACE_MSG(TRACE_APS2, "<<zb_mlme_get_confirm status %hd",
              (FMT__H, conf->status));
    zb_free_buf(buf);
}
...
void zb_mlme_set_request(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (req->pib_attr == ZB_PHY_PIB_CURRENT_CHANNEL) {
        ...
        ZB_EXTPANID_COPY(ZG->mac.pib.mac_coord_extended_address,
                         (zb_uint8_t *)ptr);
    }
    else if (req->pib_attr == ZB_PIB_ATTRIBUTE_DSN) {
        ZB_MAC_DSN() = *ptr;
        ...
        TRACE_MSG(TRACE_APS3, "set SHORT_ADDRESS %x",
                  (FMT__D, *((zb_uint16_t *)ptr)));
...
static zb_bool_t mac_is_frame_visible(zb_mac_mhr_t *mhr)
{
    ...
    if (ZB_FCF_GET_SRC_ADDRESSING_MODE(mhr->frame_control) ==
        ...
        for (i = 0; i < MAC_CTX().n_invisible_short_addr; ++i) {
            if (mhr->src_addr.addr_short ==
                MAC_CTX().invisible_short_addreesses[i]) {
                return ZB_FALSE;
            }
        }
    }
    ...
    switch (ZB_FCF_GET_SRC_ADDRESSING_MODE(mhr->frame_control)) {
        case ZB_ADDR_16BIT_DEV_OR_BROADCAST:
        {
            for (i = 0; i < MAC_CTX().n_visible_addr; ++i) {
                ...
                if (zb_address_ieee_by_short(mhr->src_addr.addr_short,
                                             long_addr) == RET_OK
                    && ZB_IEEE_ADDR_CMP(long_addr,
                                        MAC_CTX().visible_addresses[i])) {
                    TRACE_MSG(TRACE_MAC2, "added visible dev " TRACE_FORMAT_64,
        ...
        case ZB_ADDR_64BIT_DEV:
            for (i = 0; i < MAC_CTX().n_visible_addr; ++i) {
                if (ZB_IEEE_ADDR_CMP(mhr->src_addr.addr_long,
                                     MAC_CTX().visible_addresses[i])) {
                    return ZB_TRUE;
                }
            }
    ...
    TRACE_MSG(TRACE_COMMON1, "dropped frame from %hd " TRACE_FORMAT_64,
              (FMT__H_A, ZB_FCF_GET_SRC_ADDRESSING_MODE(mhr->frame_control),
               TRACE_ARG_64(mhr->src_addr.addr_long)));
...
{
    ZB_IEEE_ADDR_COPY(
        MAC_CTX().visible_addresses[MAC_CTX().n_visible_addr], long_addr);
...
{
    MAC_CTX().invisible_short_addreesses[MAC_CTX().n_invisible_short_addr] =
        addr;
