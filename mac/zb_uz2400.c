 ...
 */
    ...
    TRACE_MSG(TRACE_MAC1, "interrupt ISRSTS: 0x%hx",
...
static void zb_uz_compl_reg_read()
{
    ...
    ZG->mac.mac_ctx.rw_reg.value.byte_value =
        *(( zb_uint8_t * )ZB_BUF_BEGIN( MAC_CTX().operation_recv_buf ));
...
void zb_uz2400_fifo_write(zb_uint16_t long_addr,
                          zb_buf_t *buf) ZB_SDCC_REENTRANT
{
    TRACE_MSG(TRACE_MAC2, ">> zb_uz2400_fifo_write buf %hd len %hd",
              (FMT__H_H, ZB_REF_FROM_BUF(buf), ZB_BUF_LEN(buf)));
...
/*  MAC_SET_STATE_FOR_LAYER(ZB_MAC_IO_LAYER_TX, ZB_MAC_STATE_IDLE);*/
    ...
    TRACE_MSG( TRACE_MAC2, "<< zb_uz2400_fifo_write, l %hd",
               (FMT__H_D, ZB_BUF_LEN(buf)));
}
