...
void zb_mlme_associate_request(zb_uint8_t param) ZB_CALLBACK
{
    ...
    params =
        ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(param), zb_mlme_associate_params_t);
    ZB_ASSERT(params);
    if (ret == RET_OK) {
        if (params->coord_addr_mode == ZB_ADDR_64BIT_DEV) {
            ...
            ret = zb_mac_setup_for_associate(params->logical_channel,
                                             params->pan_id,
                                             (zb_uint16_t) ~0,
                                             params->coord_addr.addr_long);
        }
        else {
...
void zb_mlme_send_association_req_continue(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (ret == RET_OK) {
        ...
        ret = ZB_SCHEDULE_ALARM(zb_mac_resp_timeout, 0,
                                ZB_MAC_PIB_RESPONSE_WAIT_TIME);
    }
    TRACE_MSG(TRACE_MAC2, "<<mlme_ass_req_continue", (FMT__0));
}
