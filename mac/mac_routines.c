 ...
 */
    ...
    if (ret != RET_BLOCKED) {
        ...
        ret =
            ZB_SCHEDULE_CALLBACK(zb_mlme_start_confirm,
...
void zb_mlme_start_request(zb_uint8_t param) ZB_CALLBACK
{
    ...
    params = ZB_GET_BUF_PARAM(( zb_buf_t * )ZB_BUF_FROM_REF(
                                  param), zb_mlme_start_req_t);

    /* mac spec 7.1.14.1 MLME-START.request */
    /* Table 72 - MLME-START.request parameters */
    /* 7.5.1.1 Superframe structure */
    if (params->beacon_order > ZB_TURN_OFF_ORDER ||
        (params->superframe_order > params->beacon_order &&
         params->superframe_order != ZB_TURN_OFF_ORDER)) {
        /* mac spec 7.1.14.1.3 Effect on receipt */
        TRACE_MSG( TRACE_ERROR, "zb_mlme_start_request : invalid parameter!",
                   (FMT__0));
 ...
 */
    ...
    mhr_len = zb_mac_calculate_mhr_length(ZB_ADDR_16BIT_DEV_OR_BROADCAST,
                                          ZB_ADDR_NO_ADDR, 0);
                                 ...
                                 zb_buf_t *buffer) ZB_SDCC_REENTRANT
{
    ...
    TRACE_MSG(TRACE_MAC2,
              ">> zb_mac_send_comm_status pend %p, status %hi, buf %p",
              (FMT__P_H_P,
               pending_buf,
               mac_status, buffer));
    ...
    ZB_MEMCPY(&ind_params->src_addr, &mhr_pend.src_addr,
              sizeof(union zb_addr_u));
    ind_params->src_addr_mode = ZB_FCF_GET_SRC_ADDRESSING_MODE(
        &mhr_pend.frame_control);
    ZB_MEMCPY(&ind_params->dst_addr, &mhr_pend.dst_addr,
              sizeof(union zb_addr_u));
    ind_params->dst_addr_mode = ZB_FCF_GET_DST_ADDRESSING_MODE(
        &mhr_pend.frame_control);

    ZB_SCHEDULE_CALLBACK(zb_mlme_comm_status_indication,
                         ZB_REF_FROM_BUF(buffer));
...
{
    zb_ushort_t i;
    zb_mlme_purge_request_t *req = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                        param),
    ...
    if (i == ZB_MAC_PENDING_QUEUE_SIZE) {
        TRACE_MSG(TRACE_MAC1, "Purge: handle 0x%hx not found",
                  (FMT__H, req->msdu_handle));
...
//         od_hex_dump(ZB_BUF_BEGIN(pend_data->pending_data), ZB_BUF_LEN(pend_data->pending_data), 16);
    ...
    TRACE_MSG(TRACE_MAC2, "<< zb_mac_put_data_to_pending_queue, ret %i",
              (FMT__D, ret));
    return ret;
}

void zb_mac_put_request_to_queue(zb_buf_t *request,
                                 zb_mac_request_type_e req_type) ZB_CALLBACK
{
    ...
    TRACE_MSG(TRACE_MAC3,
              "put request to queue ent %p, request %p; r.b. empty %hd",
