...
void mgmt_nwk_update_ok_cb(zb_uint8_t param)
{
    ...
    TRACE_MSG(TRACE_APS3,
              ...
              (FMT__H_D_D_H_H_H_H, notify_resp->status,
               (zb_uint16_t)notify_resp->scanned_channels,
               *((zb_uint16_t *)&notify_resp->scanned_channels + 1),
               notify_resp->total_transmissions,
               notify_resp->transmission_failures,
    ...
    if (!g_errors) {
        ...
        TRACE_MSG(TRACE_APS2, "Test is finished, status: Failed, errors %d",
                  (FMT__D, g_errors));
    }
...
void mgmt_nwk_update_error_cb(zb_uint8_t param)
{
    ...
    if (notify_resp->status == ZB_NWK_STATUS_INVALID_REQUEST) {
        TRACE_MSG(TRACE_APS2,
                  "mgmt_nwk_update_notify received, invalid status - Ok",
                  (FMT__0));
    }
    else {
        TRACE_MSG(TRACE_ERROR,
...
void mgmt_nwk_update_error(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (!asdu) {
        ...
        zb_zdo_mgmt_nwk_update_req(ZB_REF_FROM_BUF(
                                       asdu), mgmt_nwk_update_error_cb);
    }
