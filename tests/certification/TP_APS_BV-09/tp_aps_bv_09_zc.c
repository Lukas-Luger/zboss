...
void test_buffer_request(zb_uint8_t param)
{
    ...
    ZB_BUF_INITIAL_ALLOC(buf, sizeof(zb_tp_transmit_counted_packets_req_t),
                         req);
    req->len = T_DATA_SIZE;
    req->counter = 0x0;
...
void zb_zdo_startup_complete(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (buf->u.hdr.status == RET_OK) {
        ...
        ZB_SCHEDULE_ALARM(test_buffer_request, 0,
                          (30 + HW_SLEEP_ADDITIONAL) * ZB_TIME_ONE_SECOND);
        ZB_SCHEDULE_ALARM(test_buffer_request, 0,
                          (32 + HW_SLEEP_ADDITIONAL) * ZB_TIME_ONE_SECOND);
        ZB_SCHEDULE_ALARM(test_buffer_request, 0,
                          (34 + HW_SLEEP_ADDITIONAL) * ZB_TIME_ONE_SECOND);
    }
    else {
