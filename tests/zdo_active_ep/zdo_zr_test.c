...
void active_ep_callback(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (status == ZB_ZDP_STATUS_SUCCESS) {
        ...
        TRACE_MSG(TRACE_ERROR, "Active EP : ERROR receiving %x", (FMT__D,
                                                                  status));
    }
}
...
static void send_data(zb_buf_t *buf)
{
    ...
    ZB_SCHEDULE_ALARM(zb_apsde_data_request, ZB_REF_FROM_BUF(
                          buf), ZB_MILLISECONDS_TO_BEACON_INTERVAL(10000));
}
