...
MAIN(){
    ...
    zb_set_simple_descriptor((zb_af_simple_desc_1_1_t *)&test_simple_desc,
                             ...
                             123 /* app_profile_id */,
                             222 /* app_device_id */,
                             333 /* app_device_version*/,
                             3 /* app_input_cluster_count */, 2 /*
                                                                 * app_output_cluster_count */);

    zb_set_input_cluster_id(
        (zb_af_simple_desc_1_1_t *)ZB_ZDO_SIMPLE_DESC(), 0,  1000);
    zb_set_input_cluster_id(
        (zb_af_simple_desc_1_1_t *)ZB_ZDO_SIMPLE_DESC(), 1,  1001);
    zb_set_input_cluster_id(
        (zb_af_simple_desc_1_1_t *)ZB_ZDO_SIMPLE_DESC(), 2,  1002);

    zb_set_output_cluster_id(
        (zb_af_simple_desc_1_1_t *)ZB_ZDO_SIMPLE_DESC(), 0,  2000);
    zb_set_output_cluster_id(
        (zb_af_simple_desc_1_1_t *)ZB_ZDO_SIMPLE_DESC(), 1,  2001);
...
void zb_zdo_startup_complete(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (buf->u.hdr.status == 0) {
        ...
        TRACE_MSG(TRACE_ERROR, "Device STARTE FAILED status %d",
                  (FMT__D, (int)buf->u.hdr.status));
    }
    zb_free_buf(buf);
}
...
void data_indication(zb_uint8_t param) ZB_CALLBACK
{
    ...
    TRACE_MSG(TRACE_APS3, "apsde_data_indication: packet %p len %d handle 0x%x",
              (FMT__P_D_D,
               asdu,
