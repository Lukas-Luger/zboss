...
void group_add_conf1(zb_uint8_t param) ZB_CALLBACK
{
    zb_apsme_add_group_req_t *req = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                         param),
...
void data_indication(zb_uint8_t param) ZB_CALLBACK
{
    ...
    TRACE_MSG(TRACE_APS3,
              "apsde_data_indication: param %hd len %hd group %d dst_endp %hd src %d",
              (FMT__H_H_D_H, param, ZB_BUF_LEN(asdu), ind->group_addr,
               ind->dst_endpoint, ind->src_addr));
    ...
    if (ind->dst_endpoint != wants_endp) {
        TRACE_MSG(TRACE_ERROR, "Got packet to endpoint %hd, wants to %hd",
                  (FMT__H_H, ind->dst_endpoint, wants_endp));
        was_err = 1;
    }
    if (wants_endp == 99) {
