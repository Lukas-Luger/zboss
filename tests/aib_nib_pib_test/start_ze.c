...
void zb_zdo_startup_complete(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (buf->u.hdr.status == 0) {
        ...
        ZB_SCHEDULE_ALARM(get_aib_information, param, ZB_MILLISECONDS_TO_BEACON_INTERVAL(
                              20000));

        set_nib_information(param);
        ZB_SCHEDULE_ALARM(get_nib_information, param, ZB_MILLISECONDS_TO_BEACON_INTERVAL(
                              30000));

        set_pib_information(param);
        ZB_SCHEDULE_ALARM(get_pib_information, param, ZB_MILLISECONDS_TO_BEACON_INTERVAL(
                              40000));
    }
    else {
...
void set_aib_design_coord(zb_uint8_t param)
{
    ...
    ZB_BUF_INITIAL_ALLOC(buf,
                         sizeof(zb_apsme_set_request_t) + sizeof(zb_uint8_t),
                         req);
...
void set_aib_ext_pan_id(zb_uint8_t param)
{
    ...
    ZB_BUF_INITIAL_ALLOC(buf,
                         sizeof(zb_apsme_set_request_t) + sizeof(zb_ieee_addr_t),
                         req);
...
void set_nib_information(zb_uint8_t param)
{
    ...
    TRACE_MSG(TRACE_NWK2, "sequence number %hd",
              (FMT__H, ZB_NIB_SEQUENCE_NUMBER()));
    TRACE_MSG(TRACE_NWK2, "max_broadcast_retries %hd",
              (FMT__H, ZG->nwk.nib.max_broadcast_retries));
    TRACE_MSG(TRACE_NWK2, "max_children %hd",
              (FMT__H, ZG->nwk.nib.max_children));
...
void set_nib_sequence_number(zb_uint8_t param)
{
    ...
    ZB_BUF_INITIAL_ALLOC(buf,
                         sizeof(zb_nlme_set_request_t) + sizeof(zb_uint8_t),
                         req);
...
void set_pib_information(zb_uint8_t param)
{
    ...
    TRACE_MSG(TRACE_MAC2, "mac_beacon_payload_length %hd",
              (FMT__H, MAC_PIB().mac_beacon_payload_length));
    TRACE_MSG(TRACE_MAC2, "mac_beacon_order %hd",
              (FMT__H, MAC_PIB().mac_beacon_order));
...
void set_pib_beacon_payload_length(zb_uint8_t param)
{
    ...
    ZB_BUF_INITIAL_ALLOC(buf,
                         sizeof(zb_mlme_set_request_t) + sizeof(zb_uint8_t),
                         req);
