...
void func1()
{
    ...
    if (!asdu) {
        ...
        zb_zdo_mgmt_nwk_update_req(ZB_REF_FROM_BUF(asdu),
                                   new_active_channel_cb);
    }
}
...
void new_active_channel_cb(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (buf->u.hdr.status == RET_OK) {
        TRACE_MSG(TRACE_APS1,
                  "zb_zdo_mgmt_nwk_update_req: new active channel Ok",
                  (FMT__0));
    }
    else {
        TRACE_MSG(TRACE_APS1,
                  "Error in zb_zdo_mgmt_nwk_update_req: new active channel",
                  (FMT__0));
        g_error_counter++;
    }
...
void new_channel_mask_cb(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (buf->u.hdr.status == RET_OK) {
        TRACE_MSG(TRACE_APS1, "zb_zdo_mgmt_nwk_update_req: new channel mask Ok",
                  (FMT__0));
    }
    else {
        TRACE_MSG(TRACE_APS1,
                  "Error in zb_zdo_mgmt_nwk_update_req: new channel mask",
                  (FMT__0));
        g_error_counter++;
    }
...
void channel_scan_cb(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (buf->u.hdr.status == RET_OK) {
        TRACE_MSG(TRACE_APS1,
                  "zb_zdo_mgmt_nwk_update_req: channel scan %d Ok, total scan count %d",
                  (FMT__D_D, g_test_scan_count, TEST_SCAN_COUNT));
    }
    else {
        TRACE_MSG(TRACE_APS1,
                  "Error in zb_zdo_mgmt_nwk_update_req: channel scan %d Ok, total scan count %d",
                  (FMT__D_D, g_test_scan_count, TEST_SCAN_COUNT));
        g_error_counter++;
        start_test = 1;
    }
    ...
    if (g_test_scan_count == TEST_SCAN_COUNT) {
        TRACE_MSG(TRACE_APS1,
                  "zb_zdo_mgmt_nwk_update_req: channel scan is finished OK",
                  (FMT__0));
        start_test = 1;
    }
    ...
    if (start_test) {
        /* start next test */
        req = ZB_GET_BUF_PARAM(buf, zb_zdo_mgmt_nwk_update_req_t);
        req->hdr.scan_channels = TEST_CHANNEL_1 | TEST_CHANNEL_2 |
                                 TEST_CHANNEL_3;
        ...
        zb_zdo_mgmt_nwk_update_req(ZB_REF_FROM_BUF(buf),
                                   channel_scan_cb_error_1);
    }
    else {
        zb_free_buf(buf);
    }
...
void channel_scan_cb_error_1(zb_uint8_t param) ZB_CALLBACK
{
    ...
    zb_zdo_mgmt_nwk_update_notify_hdr_t *notify_resp =
        (zb_zdo_mgmt_nwk_update_notify_hdr_t *)ZB_BUF_BEGIN(buf);

    if (notify_resp->status == RET_OK) {
        TRACE_MSG(TRACE_APS1,
                  "zb_zdo_mgmt_nwk_update_req: error should come, received Ok - Error",
                  (FMT__0));
        g_error_counter++;
    }
    else {
        TRACE_MSG(TRACE_APS1,
                  "zb_zdo_mgmt_nwk_update_req: error code in response - Ok",
                  (FMT__0));
...
void channel_scan_cb_error_2(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (notify_resp->status == RET_OK) {
        TRACE_MSG(TRACE_APS1,
                  "zb_zdo_mgmt_nwk_update_req: error (2) should come, received Ok - Error",
                  (FMT__0));
        g_error_counter++;
    }
    else {
        TRACE_MSG(TRACE_APS1,
                  "zb_zdo_mgmt_nwk_update_req: error (2) code in response - Ok",
                  (FMT__0));
    ...
    if (g_error_counter) {
        TRACE_MSG(TRACE_APS1, "Errors found in test %d",
                  (FMT__D, g_error_counter));
    }
    else {
...
void data_indication(zb_uint8_t param)
{
    ...
    TRACE_MSG(TRACE_APS3, "data_indication: packet %p len %d handle 0x%x",
              (FMT__P_D_D,
               asdu,
               (int)ZB_BUF_LEN(asdu), asdu->u.hdr.status));
