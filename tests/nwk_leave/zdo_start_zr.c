...
void zb_zdo_startup_complete(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (buf->u.hdr.status == 0) {
        ...
        if (flag) {
            zb_leave_req(param);
            ZB_SCHEDULE_ALARM(send_data, param, ZB_MILLISECONDS_TO_BEACON_INTERVAL(
                                  60000));
            ZB_SCHEDULE_ALARM(zb_start_join, param, ZB_MILLISECONDS_TO_BEACON_INTERVAL(
                                  10000));
