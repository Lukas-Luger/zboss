...
{
    zb_nlme_join_request_t *req = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                                       param),
    ...
    TRACE_MSG(TRACE_ERROR,
              "Device started OK first time - initiate orphan rejoin",
              (FMT__0));
...
void zb_zdo_startup_complete(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (buf->u.hdr.status == 0) {
        if (cnt == 0) {
            ZB_SCHEDULE_ALARM(do_rejoin, param, ZB_MILLISECONDS_TO_BEACON_INTERVAL(
                                  10000));
            cnt++;
        }
        else {
            TRACE_MSG(TRACE_ERROR,
                      "Device started second time - test pass SUCCESS",
                      (FMT__0));
            zb_free_buf(buf);
        }
    }
