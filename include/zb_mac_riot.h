...
#define ZB_TRANS_SEND_COMMAND(header_length, buf)          \
    zb_transceiver_send_fifo_packet((header_length), ZB_NORMAL_TX_FIFO, (buf), \
                                    1)
...
/**
   ...
   @param reg_value - value to write
 */
#define ZB_WRITE_SHORT_REG(addr, reg_value)  zb_spidev_short_write(addr, \
                                                                   reg_value)
...
/**
   ...
   @param addr - register address
 */
#define ZB_READ_SHORT_REG(addr) zb_spidev_short_read(addr, \
                                                     &ZG->mac.mac_ctx.rw_reg.value.byte_value)
...
/**
   ...
   @param addr - register address
 */
#define ZB_READ_LONG_REG(addr) zb_spidev_long_read(addr, \
                                                   &ZG->mac.mac_ctx.rw_reg.value.byte_value)
...
#define ZB_START_NORMAL_FIFO_TX(retries, need_ack)  \
    (MAC_CTX().tx_cnt = 0, \
     (ZB_WRITE_SHORT_REG(ZB_SREG_TXNTRIG, \
                         ((retries << 5) & 0xE0) | ((need_ack) ? 0x65 : 0x01))))
...
extern void zb_set_pending_bit(int set);
#define ZB_SET_PENDING_BIT() zb_set_pending_bit(1)
...
#define ZB_CLEAR_PENDING_BIT() zb_set_pending_bit(0)
...
#define ZB_TRANS_CHECK_TX_RETRY_COUNT_EXCEEDED_ERROR() \
    ZB_IS_TX_RETRY_COUNT_EXCEEDED()                                 /* not 0 means cca fail error */
#ifdef ZB_CHANNEL_ERROR_TEST
#define ZB_TRANS_CHECK_CHANNEL_ERROR() (((ZG->nwk.nib.nwk_tx_total % 3) == 0 && \
                                         ZB_MAC_GET_CHANNEL_ERROR_TEST()) ? (1) \
                                                                          : 0)
#else
...
void zb_uz2400_fifo_write(zb_uint16_t long_addr,
...
/**
   Read from FIFO
 */
void zb_uz2400_fifo_read(zb_uint8_t tx_fifo, zb_buf_t *buf,
                         zb_uint8_t len) ZB_SDCC_REENTRANT;
...
extern void zb_transceiver_set_pan_id(uint16_t pan_id);
...
#define ZB_CLEAR_SHORT_ADDR() ZB_UPDATE_SHORT_ADDR(-1)
 ...
 */
void zb_mac_short_write_reg(zb_uint8_t short_addr,
                            zb_uint8_t byte_value) ZB_SDCC_REENTRANT;

/**
   Write long UZ2400 reg
 */
void zb_mac_long_write_reg(zb_uint16_t long_addr,
                           zb_uint8_t byte_value) ZB_SDCC_REENTRANT;
#endif
...
#ifdef ZB_RESERVED_REGS_DUMP
ZB_RING_BUFFER_DECLARE(zb_regs_queue, zb_reserved_regs_t *,
                       ZB_RX_QUEUE_CAP + 1);
...
#define DUMP_RESERVED_REGS() \
    { \
        ...
        data_ptr[0] = \
            MAC_CTX().regs_queue.ring_buf[MAC_CTX().regs_queue.read_i]->isrsts; \
        ZB_DUMP_INCOMING_DATA(ZG->mac.mac_ctx.operation_buf); \
        if (MAC_CTX().regs_queue.ring_buf[MAC_CTX().regs_queue.read_i]->txntrig \
            != -1) \
        { \
            data_ptr[0] = (ZB_SREG_TXNTRIG << 1); \
            ZB_DUMP_OUTGOING_DATA(ZG->mac.mac_ctx.operation_buf); \
            data_ptr[0] = \
                MAC_CTX().regs_queue.ring_buf[MAC_CTX().regs_queue.read_i]-> \
                txntrig; \
            ZB_DUMP_INCOMING_DATA(ZG->mac.mac_ctx.operation_buf); \
        } \
        if (MAC_CTX().regs_queue.ring_buf[MAC_CTX().regs_queue.read_i]->txsr != \
            -1) \
        { \
            data_ptr[0] = (ZB_SREG_TXSR << 1); \
            ZB_DUMP_OUTGOING_DATA(ZG->mac.mac_ctx.operation_buf); \
            data_ptr[0] = \
                MAC_CTX().regs_queue.ring_buf[MAC_CTX().regs_queue.read_i]->txsr; \
            ZB_DUMP_INCOMING_DATA(ZG->mac.mac_ctx.operation_buf);               \
        } \
...
#define ZB_GET_ISRSTS() \
    TRANS_CTX().int_status = \
        *((zb_uint8_t ZB_XDATA *)(ZB_SREG_ISRSTS | ZB_SHORT_REGS_BASE))

#define ZB_GET_TXSR() \
    TRANS_CTX().tx_status = \
        *((zb_uint8_t ZB_XDATA *)(ZB_SREG_TXSR | ZB_SHORT_REGS_BASE))
...
#define ZB_PUT_RX_QUEUE() \
    ...
    {                             \
        ZB_MEMCPY(ZB_BUF_BEGIN( \
                      MAC_CTX().mac_rx_queue.ring_buf[MAC_CTX(). \
                                                      mac_rx_queue.write_i]), \
                  (zb_uint8_t ZB_XDATA *)ZB_NORMAL_RXFIFO_ADDR,                 \
                  *((zb_uint8_t ZB_XDATA *)ZB_NORMAL_RXFIFO_ADDR) + ZB_MAC_PACKET_LENGTH_SIZE + \
                  ZB_MAC_EXTRA_DATA_SIZE); \
        MAC_CTX().mac_rx_queue.ring_buf[MAC_CTX().mac_rx_queue.write_i]->u.hdr. \
        len =   *((zb_uint8_t ZB_XDATA *)ZB_NORMAL_RXFIFO_ADDR) + \
              ZB_MAC_PACKET_LENGTH_SIZE + ZB_MAC_EXTRA_DATA_SIZE; \
        MAC_CTX().mac_rx_queue.written++;                                                     \
        MAC_CTX().mac_rx_queue.write_i = (MAC_CTX().mac_rx_queue.write_i + 1) % \
                                         ZB_RX_QUEUE_CAP;           \
    }
...
#define ZB_GET_RX_QUEUE()       \
    {                                             \
        ...
        MAC_CTX().recv_buf = \
            MAC_CTX().mac_rx_queue.ring_buf[MAC_CTX().mac_rx_queue.read_i]; \
        MAC_CTX().mac_rx_queue.ring_buf[MAC_CTX().mac_rx_queue.read_i] = \
            mac_rx_queue_tmp; \
