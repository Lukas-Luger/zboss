...
zb_void_t zb_assert(zb_char_t *file_name,
                    zb_int_t line_number /*, zb_char_t *expr_name*/);
#define ZB_ASSERT(expr) { if (!(expr)) { zb_assert(__FILE__, __LINE__); } }
#endif

#define ZB_INLINE_ASSERT_SIMPLE(expr)  ((expr) ? 1 : (zb_abort(__FILE__, \
                                                               __LINE__), 1))
...
/**
   ...
   @param expr - expression to check.
 */
#define ZB_ASSERT_COMPILE_TIME_EXPR(expr) ((zb_int_t)((zb_int_t(*)[(expr) ? 1 : \
                                                                   -1]) 0))
...
/**
   ...
   @param expr - expression to check.
 */
#define ZB_ASSERT_COMPILE_DECL(expr) typedef zb_int_t ZB_ASSERT_CAT (assert, \
                                                                     __LINE__)[( \
                                                                                   expr) \
                                                                               ? \
                                                                               1 \
 ...
 */
#define ZB_ASSERT_TYPE_SIZE_NOT_GREATER(type, limit) ZB_ASSERT_COMPILE_DECL( \
        sizeof(type) <= (limit))
...
/**
   ...
   @param limit - size limit
 */
#define ZB_ASSERT_TYPE_SIZE_EQ(type, limit) ZB_ASSERT_COMPILE_DECL( \
        sizeof(type) == (limit))
