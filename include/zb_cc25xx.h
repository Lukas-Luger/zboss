...
#define ZB_UPDATE_SHORT_ADDR() SHORT_ADDR0 =  MAC_PIB().mac_short_address & \
                                             0xFF; \
    SHORT_ADDR1 = (MAC_PIB().mac_short_address >> 8) & 0xFF
...
#define ZB_CLEAR_TX_STATUS() (TRANS_CTX().int_status_1 &= ~0x02, \
                              TRANS_CTX().tx_status = 0)

#define ZB_UBEC_CLEAR_NORMAL_FIFO_TX_STATUS() (TRANS_CTX().int_status_1 &= \
                                                   (~0x02))                         /* clear bit 0 */
...
#if 0
ZB_MEMCPY(ZB_BUF_BEGIN(MAC_CTX().mac_rx_queue.ring_buf[MAC_CTX().mac_rx_queue.
                                                       write_i]), \
...
#define ZB_PUT_RX_QUEUE() \
    ...
    {                             \
        ...
        len = len; \
        *(zb_uint8_t *)(ZB_BUF_BEGIN(MAC_CTX().mac_rx_queue.ring_buf[MAC_CTX(). \
                                                                     mac_rx_queue \
                                                                     .write_i])) \
            = len; \
        for (i = 1; i < len; i++) \
