...
#define ZB_DUMP_IEEE_ADDR(iee_addr) TRACE_MSG(TRACE_COMMON3, \
                                              "ieeeaddr " TRACE_FORMAT_64, \
                                              (FMT__A, TRACE_ARG_64(iee_addr)))
#else
...
#define ZB_ADDRESS_DECOMPRESS(address, copressed_address)               \
    ...
    {                                                                       \
        ZB_MEMCPY(&((address)[0]),                                            \
                  &(ZG->addr.dev_manufacturer[(copressed_address). \
                                              dev_manufacturer]. \
                    device_manufacturer[0]), \
                  (sizeof((address)[0]) * 3));                                \
        ZB_MEMCPY(&((address)[3]), &((copressed_address).device_id[0]), \
                  (sizeof((address)[0]) * 5)); \
    }                                                                       \
    while (0)
...
#define ZB_ADDRESS_COMPRESSED_CMP(one, two) zb_address_compressed_cmp(&one, \
                                                                      &two)

#define ZB_ADDRESS_COMPRESSED_COPY(dest, src)                       \
    ZB_MEMCPY(&(dest).dev_manufacturer, &(src).dev_manufacturer, \
              sizeof(zb_ieee_addr_compressed_t))

#define ZB_ADDRESS_COMPRESED_IS_ZERO(dest)      \
    (!ZB_MEMCMP(&(dest).dev_manufacturer, g_zero_addr, \
                sizeof(zb_ieee_addr_compressed_t)))
