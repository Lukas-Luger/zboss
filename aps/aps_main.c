...
static void aps_data_hdr_fill_datareq(zb_uint8_t fc, zb_apsde_data_req_t *req,
                                      zb_buf_t *apsdu) ZB_SDCC_REENTRANT;
static void aps_ack_send_handle(zb_buf_t *packet,
                                zb_aps_hdr_t *aps_hdr) ZB_SDCC_REENTRANT;
static zb_uint8_t save_ack_data(zb_uint8_t param, zb_apsde_data_req_t *req,
                                zb_uint8_t *ref) ZB_SDCC_REENTRANT;
static void aps_ack_frame_handle(zb_aps_hdr_t *aps_hdr) ZB_SDCC_REENTRANT;
#ifndef ZB_LIMITED_FEATURES
static zb_ret_t zb_search_dst_in_binding_table(zb_apsde_data_req_t *apsreq,
                                               zb_address_ieee_ref_t *dst_ref)
ZB_SDCC_REENTRANT;
#endif
static void done_with_this_ack(zb_ushort_t i, zb_uint8_t fc,
                               zb_uint8_t status) ZB_SDCC_REENTRANT;
static void fill_nldereq(zb_uint8_t param, zb_uint16_t addr,
                         zb_uint8_t secure) ZB_SDCC_REENTRANT;
...
ZB_SDCC_REENTRANT
{
    ...
    ret = zb_address_by_short(
        ZB_PIB_SHORT_ADDRESS(), ZB_FALSE, ZB_FALSE, &addr_ref);
    TRACE_MSG(TRACE_APS3, "address_by_short ret %d, short addr %d",
              (FMT__D_D, ret, ZB_PIB_SHORT_ADDRESS()));
    if (ret == RET_OK) {
        ind_src = aps_find_src_ref(addr_ref, apsreq->src_endpoint,
                                   apsreq->clusterid);
        TRACE_MSG(TRACE_APS3, "ind_src %hd, src ep %hd, clusterid %d",
                  (FMT__H_H_D, ind_src, apsreq->src_endpoint,
                   apsreq->clusterid));
        ...
        if (ind_src != (zb_uint8_t)-1) {
            ...
            do {
                if (ZG->aps.binding.dst_table[i].src_table_index == ind_src) {
                    if (ZG->aps.binding.dst_table[i].dst_addr_mode ==
                        ZB_APS_BIND_DST_ADDR_LONG) {
                        *dst_ref =
    ...
    switch ((int)apsreq->addr_mode) {
        ...
        case ZB_APS_ADDR_MODE_DST_ADDR_ENDP_NOT_PRESENT:
        {
            ...
            if (ret == RET_OK) {
                ...
                TRACE_MSG(TRACE_APS3, "apsde_data unicast, dst %d",
                          (FMT__D, nldereq.dst_addr));

                if (nldereq.dst_addr == ZB_UNKNOWN_SHORT_ADDR) {
                    /* TODO: we can fall into infinite loop here, asking for peer short address, fix it!!! */
                    zb_start_get_peer_short_addr(dst_ref, zb_apsde_data_request,
                                                 param);
                    TRACE_MSG(TRACE_APS1, "-apsde_data_req, wait for peer addr",
                              (FMT__0));
                    return;
                }
                ...
                TRACE_MSG(TRACE_APS3,
                          "dst addr and endpiont not present in binding table",
                          (FMT__0));
                                      ...
                                      zb_buf_t *apsdu) ZB_SDCC_REENTRANT
{
    ...
    if (req->tx_options & ZB_APSDE_TX_OPT_SECURITY_ENABLED) {
        ...
        if (req->tx_options & ZB_APSDE_TX_OPT_USE_NWK_KEY) {
            ...
            TRACE_MSG(TRACE_SECUR1, "Secure APS by app key: TODO - implment!",
                      (FMT__0));
...
zb_ushort_t zb_aps_full_hdr_size(zb_uint8_t *pkt)
{
    ...
    if (ZB_APS_FC_GET_SECURITY(*pkt)) {
        if (ZB_SECUR_AUX_HDR_GET_KEY_TYPE(((zb_aps_nwk_aux_frame_hdr_t *)&pkt[
                                               size])->secur_control) ==
            ZB_SECUR_DATA_KEY) {
...
#endif
    {
        ...
        for (i = 0; i < ZB_N_APS_RETRANS_ENTRIES; ++i) {
            if (ZG->aps.retrans.hash[i].buf == param) {
                if (ZG->aps.retrans.hash[i].state ==
                    ZB_APS_RETRANS_ENT_SENT_MAC_NOT_CONFIRMED_APS_ACKED_ALRM_RUNNING)
                {
                    ...
                    TRACE_MSG(TRACE_APS2,
                              "APS ACK OK, retransmit of %hd confirmed",
                              (FMT__H, param));
                    done_with_this_ack(i, *fc_p, RET_OK);
                }
                else {
                    ZB_ASSERT(
                        ZG->aps.retrans.hash[i].state ==
                        ZB_APS_RETRANS_ENT_SENT_MAC_NOT_CONFIRMED_ALRM_RUNNING);
                    TRACE_MSG(TRACE_APS2,
                              "packet %hd len %hd confirm state %hd, set st %hd, still wait for ack",
                              (FMT__H_H_H_H, param, ZB_BUF_LEN(
                                   nsdu),
                               ZG->aps.retrans.hash[i].state,
...
void zb_nlde_data_indication_continue(zb_uint8_t param) ZB_CALLBACK
{
    ...
    TRACE_MSG(TRACE_APS1, "+zb_nlde_data_indication_continue %hd",
              (FMT__H, param));
...
#endif
    {
        zb_buf_assign_param(packet, (zb_uint8_t *)&aps_hdr,
...
#endif
        {
            ...
            if (!(ZB_APS_FC_GET_FRAME_TYPE(aps_hdr.fc) == ZB_APS_FRAME_DATA
                  && ZB_APS_FC_GET_DELIVERY_MODE(aps_hdr.fc) ==
...
void zb_aps_hdr_parse(zb_buf_t *packet, zb_aps_hdr_t *aps_hdr,
                      zb_bool_t cut_nwk_hdr) ZB_SDCC_REENTRANT
{
    zb_nwk_hdr_t *nwk_hdr = (zb_nwk_hdr_t *)ZB_BUF_BEGIN(packet);
...
void zb_aps_pass_group_msg_up(zb_uint8_t param) ZB_CALLBACK
{
    ...
    {
        zb_uint16_t group_addr = ZB_GET_BUF_PARAM(
            ZG->aps.group.active_pass_up_buf, zb_aps_hdr_t)->group_addr;
    ...
    if (g_i == ZG->aps.group.n_groups
        ...
        if (!ZB_RING_BUFFER_IS_EMPTY(&ZG->aps.group.pass_up_q)) {
            ZG->aps.group.active_pass_up_buf = ZB_BUF_FROM_REF(*ZB_RING_BUFFER_GET(
                                                                   &ZG->aps.
                                                                   group.
                                                                   pass_up_q));
        ...
        ZB_GET_BUF_PARAM(buf, zb_aps_hdr_t)->dst_endpoint =
            ZG->aps.group.groups[g_i].endpoints[ZG->aps.group.active_pass_up_buf
                                                ->u.hdr.handle];
        ZG->aps.group.active_pass_up_buf->u.hdr.handle++;
...
#ifdef ZB_SECURITY
void zb_aps_send_command(zb_uint8_t param, zb_uint16_t dest_addr,
                         zb_uint8_t command, zb_bool_t secure)
{
    ...
    TRACE_MSG(TRACE_SECUR3,
              ">>zb_aps_send_command param %hd cmd %hd secur %hd to %d need_ack %hd",
              (FMT__H_H_H_D_H, param, command, secure, dest_addr, need_ack));
    ...
    if (need_ack) {
        ...
        for (i = 0; i < ZB_N_APS_RETRANS_ENTRIES; ++i) {
            if (ZG->aps.retrans.hash[i].state == ZB_APS_RETRANS_ENT_FREE) {
                ...
                ZG->aps.retrans.hash[i].aps_retries =
                    ZB_N_APS_MAX_FRAME_ENTRIES;
                ZG->aps.retrans.hash[i].state =
                    ZB_APS_RETRANS_ENT_SENT_MAC_NOT_CONFIRMED_ALRM_RUNNING;
                TRACE_MSG(TRACE_APS2,
                          "Store buf %hd len %hd in retrans hash %d",
                          (FMT__H_H, param, ZB_BUF_LEN(buf), i));

                DUMP_TRAF("sending aps cmd", ZB_BUF_BEGIN(buf),
                          ZB_BUF_LEN(buf));
        ...
        if (i == ZB_N_APS_RETRANS_ENTRIES) {
            ...
            ZB_SCHEDULE_ALARM(zb_aps_ack_timer_cb, i,
    ...
    TRACE_MSG(TRACE_SECUR3, "send APS cmd %hd secur %hd to %d",
              (FMT__H_H_D, command, secure, dest_addr));
...
static void fill_nldereq(zb_uint8_t param, zb_uint16_t addr,
                         zb_uint8_t secure) ZB_SDCC_REENTRANT
{
    zb_nlde_data_req_t *nldereq = ZB_GET_BUF_TAIL(ZB_BUF_FROM_REF(
                                                      param),
                                                  sizeof(zb_nlde_data_req_t));
                                ...
                                zb_aps_hdr_t *aps_hdr) ZB_SDCC_REENTRANT
{
    ...
    TRACE_MSG(TRACE_APS2, "+aps_ack_send_handle %hd",
              (FMT__H, ZB_REF_FROM_BUF(packet)));
    ...
    *apshdr = fc;
    ...
    if (ZB_APS_FC_GET_ACK_FORMAT(fc) == 0) {
        ...
        *apshdr = aps_hdr->src_endpoint;
        ...
        TRACE_MSG(TRACE_APS2, "src_endpoint %d",
                  (FMT__D, aps_hdr->src_endpoint));
                                ...
                                zb_uint8_t *ref) ZB_SDCC_REENTRANT
{
    ...
    do {
        if (ZG->aps.retrans.hash[i].state == ZB_APS_RETRANS_ENT_FREE) {
            ...
            *ref = i;
            TRACE_MSG(TRACE_APS2,
                      "save_ack_data  clusterid %d aps_counter %d src_endpoint %d src_endpoint %d, i %hd",
                      (FMT__D_D_D_D_H, ZG->aps.retrans.hash[i].clusterid,
                       ZG->aps.retrans.hash[i].aps_counter,
                       ZG->aps.retrans.hash[i].src_endpoint,
                       ZG->aps.retrans.hash[i].dst_endpoint, i));
            break;
        }
        i++;
...
static void aps_ack_frame_handle(zb_aps_hdr_t *aps_hdr) ZB_SDCC_REENTRANT
{
    ...
    for (i = 0; i < ZB_N_APS_RETRANS_ENTRIES; ++i) {
        TRACE_MSG(TRACE_APS2, "aps_ack_check_handle addr %d, src_addr %d",
                  (FMT__D_D, ZG->aps.retrans.hash[i].addr, aps_hdr->src_addr));
        TRACE_MSG(TRACE_APS2,
                  "retrans.hash i %d, clusterid %d aps_counter %d src_endpoint %d dst_endpoint %d",
                  (FMT__D_D_D_D_D, i, ZG->aps.retrans.hash[i].clusterid,
                   ZG->aps.retrans.hash[i].aps_counter,
                   ZG->aps.retrans.hash[i].src_endpoint,
                   ZG->aps.retrans.hash[i].dst_endpoint));
        ...
        if (ZG->aps.retrans.hash[i].addr == aps_hdr->src_addr
            ...
            && (is_command
                || (ZG->aps.retrans.hash[i].clusterid == aps_hdr->clusterid
                    && ZG->aps.retrans.hash[i].src_endpoint ==
                    aps_hdr->dst_endpoint
                    && ZG->aps.retrans.hash[i].dst_endpoint ==
                    aps_hdr->src_endpoint))) {
            break;
        }
    }
    ...
    if (i < ZB_N_APS_RETRANS_ENTRIES) {
        ...
        if (ZG->aps.retrans.hash[i].state ==
            ZB_APS_RETRANS_ENT_SENT_MAC_NOT_CONFIRMED_ALRM_RUNNING) {
            TRACE_MSG(TRACE_APS2,
                      "ent %hd: got ACK while waiting for nwk confirm",
                      (FMT__H, i));
            ZG->aps.retrans.hash[i].state =
                ZB_APS_RETRANS_ENT_SENT_MAC_NOT_CONFIRMED_APS_ACKED_ALRM_RUNNING;
        }
        else {
            TRACE_MSG(TRACE_APS2, "packet %hd acked ok",
                      (FMT__H, ZG->aps.retrans.hash[i].buf));
                               ...
                               zb_uint8_t status) ZB_SDCC_REENTRANT
{
    ...
    if (ZB_APS_FC_GET_ACK_FORMAT(fc)) {
        TRACE_MSG(TRACE_APS2, "finally done with this commmand i %hd",
                  (FMT__H, i));
        zb_free_buf(ZB_BUF_FROM_REF(ZG->aps.retrans.hash[i].buf));
    }
    else {
        TRACE_MSG(TRACE_APS2, "finally done with this data pkt i %hd",
                  (FMT__H, i));
        ZB_SCHEDULE_CALLBACK(zb_apsde_data_confirm,
                             ZG->aps.retrans.hash[i].buf);
    }
...
void zb_aps_ack_timer_cb(zb_uint8_t param) ZB_CALLBACK
{
    TRACE_MSG(TRACE_APS2, "+zb_aps_ack_timer_cb %hd state %hd",
              (FMT__H_H, param, ZG->aps.retrans.hash[param].state));
    ...
    /* Note: 'param' here is index, not packet buffer!  */
    {
        ...
        if (ZG->aps.retrans.hash[param].aps_retries == 0
            && ZG->aps.retrans.hash[param].state ==
            ZB_APS_RETRANS_ENT_SENT_MAC_CONFIRMED_ALRM_RUNNING) {
            TRACE_MSG(TRACE_APS2,
                      "out of retransmissions - APS transmission failed",
                      (FMT__0));
            ...
            ZB_SCHEDULE_ALARM(zb_aps_ack_timer_cb, param,
                              ZB_N_APS_ACK_WAIT_DURATION);

            if (ZG->aps.retrans.hash[param].state ==
                ZB_APS_RETRANS_ENT_SENT_MAC_CONFIRMED_ALRM_RUNNING) {
                fill_nldereq(ZG->aps.retrans.hash[param].buf,
                             ZG->aps.retrans.hash[param].addr,
                             /* use NWK security only if not use APS security */
                             !(ZG->aps.retrans.hash[param].nwk_insecure ||
                               ZB_APS_FC_GET_SECURITY(fc)));

                TRACE_MSG(TRACE_APS2,
                          "ACK FAILED - retransmit [%hd] st %hd buf %hd to %d, retries %hd, insecure %hd",
                          (FMT__H_H_H_D_H,
                ...
                DUMP_TRAF("retransmitting aps",
                          ZB_BUF_BEGIN(ZB_BUF_FROM_REF(
                                           ZG->aps.retrans.hash[param].buf)),
                          ZB_BUF_LEN(ZB_BUF_FROM_REF(ZG->aps.retrans.hash[param]
                                                     .buf)));

                ZB_SCHEDULE_CALLBACK(zb_nlde_data_request,
                                     ZG->aps.retrans.hash[param].buf);
            }
            else {
                TRACE_MSG(TRACE_APS2,
                          "slit %hd state %hd - skip this retransmit",
                          (FMT__H_H, param, ZG->aps.retrans.hash[param].state));
            }
        }
    }
...
void zb_apsme_get_request(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (req_param.aib_attr == ZB_APS_AIB_DESIGNATED_COORD) {
        ZB_BUF_INITIAL_ALLOC(buf,
                             sizeof(zb_apsme_get_confirm_t) + sizeof(zb_uint8_t),
                             conf);
        conf->aib_length = sizeof(zb_uint8_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_apsme_get_confirm_t)) = ZB_AIB().aps_designated_coordinator;
    }
        ...
        conf->aib_length = sizeof(zb_uint8_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_apsme_get_confirm_t)) = ZB_AIB().aps_insecure_join;
    }
    else if (req_param.aib_attr == ZB_APS_AIB_CHANNEL_MASK) {
        ZB_BUF_INITIAL_ALLOC(buf,
                             sizeof(zb_apsme_get_confirm_t) + sizeof(zb_uint32_t),
                             conf);
        conf->aib_length = sizeof(zb_uint32_t);
        *(((zb_uint8_t *)conf) +
          sizeof(zb_apsme_get_confirm_t)) = ZB_AIB().aps_channel_mask;
    }
    else if (req_param.aib_attr == ZB_APS_AIB_USE_EXT_PANID) {
        ZB_BUF_INITIAL_ALLOC(buf,
                             sizeof(zb_apsme_get_confirm_t) +
                             sizeof(zb_ext_pan_id_t), conf);
        conf->aib_length = sizeof(zb_ext_pan_id_t);
        ZB_64BIT_ADDR_COPY((((zb_uint8_t *)conf) +
                            sizeof(zb_apsme_get_confirm_t)),
...
void zb_apsme_get_confirm(zb_uint8_t param) ZB_CALLBACK
{
    ...
    if (conf->aib_attr == ZB_APS_AIB_DESIGNATED_COORD) {
        TRACE_MSG(TRACE_APS2, "attr %hd get param %hd",
                  (FMT__H_H, conf->aib_attr,
                   *(((zb_uint8_t *)conf) + sizeof(zb_apsme_get_confirm_t))));
    }
    else if (conf->aib_attr == ZB_APS_AIB_USE_EXT_PANID) {
        TRACE_MSG(TRACE_APS2, "attr %hd get param "TRACE_FORMAT_64 "",
                  (FMT__H_A, conf->aib_attr,
                   TRACE_ARG_64((((
                                      zb_uint8_t *)conf) +
                                 sizeof(zb_apsme_get_confirm_t)))));
    }
    TRACE_MSG(TRACE_APS2, "<<zb_apsme_get_confirm status %hd",
              (FMT__H, conf->status));
    zb_free_buf(buf);
}
...
void zb_apsme_set_confirm(zb_uint8_t param) ZB_CALLBACK
{
    ...
    {
        zb_apsme_set_confirm_t *conf = ( zb_apsme_set_confirm_t *)ZB_BUF_BEGIN(
            buf);
        TRACE_MSG(TRACE_APS2, "<<zb_apsme_set_confirm status %hd",
                  (FMT__H, conf->status));
    }
#endif
    zb_free_buf(buf);
