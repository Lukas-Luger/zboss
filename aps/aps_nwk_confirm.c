...
void zb_nlme_route_discovery_confirm(zb_uint8_t param) ZB_CALLBACK
{
    ...
    TRACE_MSG(TRACE_NWK1, ">>zb_nlme_route_discovery_confirm %p",
              (FMT__P, buf));

    if (buf) {
        zb_nlme_route_discovery_confirm_t *confirm =
            (zb_nlme_route_discovery_confirm_t *)ZB_BUF_BEGIN(buf);
        ZVUNUSED(confirm);
    }
...
void zb_nlme_sync_loss_indication(zb_uint8_t param) ZB_CALLBACK
{
    ...
    TRACE_MSG(TRACE_NWK1, ">>zb_nlme_sync_loss_indication %hd",
              (FMT__H, param));
