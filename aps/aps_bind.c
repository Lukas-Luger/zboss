...
zb_uint8_t aps_find_src_ref(zb_address_ieee_ref_t src_addr_ref,
                            zb_uint8_t src_end, zb_uint16_t cluster_id)
{
    zb_uint8_t l = 0;
...
void zb_apsme_bind_request(zb_uint8_t param) ZB_CALLBACK
{
    ...
    zb_apsme_binding_req_t *apsreq = ZB_GET_BUF_PARAM(aps,
    ...
    aps->u.hdr.status = zb_address_by_ieee(apsreq->src_addr, ZB_TRUE, ZB_FALSE,
                                           &src_addr_ref);
    if (aps->u.hdr.status == RET_OK) {
        s = aps_find_src_ref(src_addr_ref, apsreq->src_endpoint,
                             apsreq->clusterid);
        ...
        if (s < ZB_APS_SRC_BINDING_TABLE_SIZE
            ...
            if ((int)apsreq->addr_mode == ZB_APS_ADDR_MODE_64_ENDP_PRESENT) {
                aps->u.hdr.status = zb_address_by_ieee(
                    apsreq->dst_addr.addr_long, ZB_TRUE, ZB_FALSE,
                    &dst_addr_ref);
            }
            else if ((int)apsreq->addr_mode ==
                     ZB_APS_ADDR_MODE_16_GROUP_ENDP_NOT_PRESENT) {
                /* do nothing */
            }
            else {
                TRACE_MSG(TRACE_ERROR, "invalid DstAddrMode %hd",
                          (FMT__H, apsreq->addr_mode));
            ...
            if (aps->u.hdr.status == RET_OK) {
                ...
                if ((int)apsreq->addr_mode ==
                    ZB_APS_ADDR_MODE_64_ENDP_PRESENT) {
                    ZG->aps.binding.dst_table[d].dst_addr_mode =
                        ZB_APS_BIND_DST_ADDR_LONG;
                    ZG->aps.binding.dst_table[d].u.long_addr.dst_addr =
                        dst_addr_ref;
                    ZG->aps.binding.dst_table[d].u.long_addr.dst_end =
                        apsreq->dst_endpoint;
                }
                else {
                    ZG->aps.binding.dst_table[d].dst_addr_mode =
                        ZB_APS_BIND_DST_ADDR_GROUP;
                    ZG->aps.binding.dst_table[d].u.group_addr =
    ...
    TRACE_MSG(TRACE_APS1, "-zb_apsme_bind_request status %d",
              (FMT__D, aps->u.hdr.status));
}
...
void zb_apsme_unbind_request(zb_uint8_t param) ZB_CALLBACK
{
    ...
    aps->u.hdr.status = zb_address_by_ieee(apsreq->src_addr, ZB_FALSE, ZB_FALSE,
                                           &src_addr_ref);
    if (aps->u.hdr.status == RET_OK) {
        s = aps_find_src_ref(src_addr_ref, apsreq->src_endpoint,
                             apsreq->clusterid);
            ...
            if (aps->u.hdr.status == RET_OK) {
                ...
                do {
                    if (ZG->aps.binding.dst_table[d].src_table_index == s
                        && (((int)apsreq->addr_mode ==
                             ZB_APS_ADDR_MODE_64_ENDP_PRESENT
                             && ZG->aps.binding.dst_table[d].dst_addr_mode ==
                             ZB_APS_BIND_DST_ADDR_LONG
                             && ZG->aps.binding.dst_table[d].u.long_addr.
                             dst_addr == dst_addr_ref
                             && ZG->aps.binding.dst_table[d].u.long_addr.dst_end
                             == apsreq->dst_endpoint)
                            || ((int)apsreq->addr_mode ==
                                ZB_APS_ADDR_MODE_16_GROUP_ENDP_NOT_PRESENT
                        ...
                        ZB_MEMMOVE(&ZG->aps.binding.dst_table[d],
                                   ...
                                   (ZG->aps.binding.dst_n_elements - d));
                    }
                    else {
                        if (ZG->aps.binding.dst_table[d].src_table_index == s) {
                            found = 1;
                ...
                if (!found) {
                    ...
                    ZB_MEMMOVE(&ZG->aps.binding.src_table[s],
                               ...
                               sizeof(zb_aps_bind_src_table_t) *
                               (ZG->aps.binding.src_n_elements - s));
    ...
    TRACE_MSG(TRACE_APS1, "-zb_apsme_unbind_request status %d",
              (FMT__D, aps->u.hdr.status));
}
...
void zb_apsme_add_group_request(zb_uint8_t param) ZB_CALLBACK
{
    ...
    ZB_MEMCPY(&req,
              ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(
                                   param), zb_apsme_add_group_req_t),
              sizeof(req));
    TRACE_MSG(TRACE_APS3,
              "zb_apsme_add_group_request group_addr %d endpoint %hd",
        ...
        for (i = 0; i < ZG->aps.group.n_groups; ++i) {
            if (ZG->aps.group.groups[i].group_addr == req.group_address) {
                ...
                if (j < ZB_APS_ENDPOINTS_IN_GROUP_TABLE) {
                    ...
                    TRACE_MSG(TRACE_ERROR, "No more space for endpoints",
                              (FMT__0));
...
void zb_apsme_add_group_request(ZPAR void *v_grp)
{
    ...
    if ((apsgrp->group_addr == 0) || (apsgrp->endpoint == 0)) {
        ...
        grp->u.hdr.status = zb_address_by_short( apsgrp->group_addr, ZB_FALSE,
                                                 &addr_ref);
    ...
    TRACE_MSG(TRACE_APS1, "<<apsme_add_group_req status %d",
              (FMT__D, grp->u.hdr.status));
}
...
void zb_apsme_remove_group_request(ZPAR void *v_grp)
{
        ...
        if (grp->u.hdr.status == RET_OK) {
            ...
            do {
                if ((ZG->aps.group.group_addr[l] == addr_ref)
                    ...
                    do {
                        ZG->aps.group.group_addr[l] =
                            ZG->aps.group.group_addr[l + 1];
                        ZG->aps.group.endpoint[l] =
                            ZG->aps.group.endpoint[l + 1];
                        l++;
    ...
    TRACE_MSG(TRACE_APS1, "<<apsme_remove_group_req status %d",
              (FMT__D, grp->u.hdr.status));
}
...
void zb_apsme_remove_all_group_request(ZPAR void *v_grp)
{
    ...
    TRACE_MSG(TRACE_APS1, "<<apsme_remove_all_group_req status %d",
              (FMT__D, grp->u.hdr.status));
}
...
void zb_apsme_add_group_confirm(ZPAR void *v_grp)
{
    ...
    TRACE_MSG(TRACE_APS2, "+add_group_confirm %p status %d",
              (FMT__P_D, grp, grp->u.hdr.status));
    zb_free_buf( grp);
}
...
void zb_apsme_remove_group_confirm(ZPAR void *v_grp)
{
    ...
    TRACE_MSG(TRACE_APS2, "+remove_group_confirm %p status %d",
              (FMT__P_D, grp, grp->u.hdr.status));
    zb_free_buf( grp);
}
...
void zb_apsme_remove_all_group_confirm(ZPAR void *v_grp)
{
    ...
    TRACE_MSG(TRACE_APS2, "+remove_all_group_confirm %p status %d",
              (FMT__P_D, grp, grp->u.hdr.status));
    zb_free_buf( grp);
}
