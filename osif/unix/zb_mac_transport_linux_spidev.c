...
static void zb_spidev_dump_tx_tx(zb_uint8_t *buf1, zb_ushort_t len1,
                                 zb_uint8_t *buf2, zb_ushort_t len2);
static void zb_spidev_dump_rx(zb_uint8_t *buf, zb_ushort_t len);

#define DUMP_TX(buf, len) zb_spidev_dump_tx((buf), (len))
#define DUMP_TX_TX(buf1, len1, buf2, len2) zb_spidev_dump_tx_tx((buf1), (len1), \
                                                                (buf2), (len2))
...
void zb_mac_transport_init(zb_char_t *rpipe_path, zb_char_t *wpipe_path)
{
    ...
    if (!ZIG->ioctx.dump_file) {
        ...
        if (!ZIG->ioctx.dump_file) {
            TRACE_MSG(TRACE_ERROR, "Can't open dump file %s %d",
                      (FMT__0, namefile, errno));
        }
    }
}
                                 ...
                                 zb_uint8_t *buf2, zb_ushort_t len2)
{
    ...
    zb_packet_cnt++;
    TRACE_MSG(TRACE_MAC3, "tx #%d len %hd",
              (FMT__D_H, zb_packet_cnt, len1 + len2));
...
zb_ret_t zb_spidev_long_write_fifo(zb_ushort_t long_addr, zb_uint8_t *buf,
                                   zb_ushort_t len)
{
...
zb_ret_t zb_spidev_long_read_fifo(zb_ushort_t long_addr, zb_uint8_t *buf,
                                  zb_ushort_t len)
{
    struct spi_ioc_transfer ioc[24];
    zb_uint8_t txb[24][8];
...
zb_ret_t zb_spidev_wait_for_intr(zb_uint8_t can_wait)
{
    ...
    TRACE_MSG(TRACE_MAC1, ">>zb_spidev_wait_for_intr can_wait %hd",
              (FMT__H, can_wait));
    ...
    if (!can_wait) {
        ...
        if (tmo != (zb_time_t) ~0) {
            ...
            TRACE_MSG(TRACE_MAC3, "default timeout %d",
                      (FMT__D, ZB_LINUX_SPIDEV_INTR_TIMEOUT));
    ...
    /* deal with time */
    {
        ...
        delta_us = end_t.tv_sec * 1000000ll + end_t.tv_usec - start_t.tv_sec *
                   1000000ll - start_t.tv_usec + ZIG->ioctx.time_delta_rest_us;
        delta_b = ZB_MILLISECONDS_TO_BEACON_INTERVAL(delta_us / 1000);
        TRACE_MSG(TRACE_MAC3, "delta %llu us / %d ticks",
                  (FMT__D_D, delta_us, delta_b));
        if (ZB_TIMER_CTX().started) {
            /* imitate 8051: move timer if it started only */
            ZB_TIMER_CTX().timer = ZB_TIME_ADD(ZB_TIMER_CTX().timer, delta_b);
            ZIG->ioctx.time_delta_rest_us = delta_us -
        ...
        TRACE_MSG(TRACE_MAC3, "timer %d, delta_rest_us %llu",
                  (FMT__D_D,
                   ZB_TIMER_CTX().timer, ZIG->ioctx.time_delta_rest_us));
