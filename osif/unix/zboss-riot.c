...
static char _zigbee_thread_stack [THREAD_STACKSIZE_DEFAULT + 1024]; //512
...
//  gnrc_netapi_set(_zb_iface_id, NETOPT_PROMISCUOUSMODE, 0, &set, sizeof(netopt_enable_t));
    ...
    //this could also work instead of ZG->nwk.nib.security_level = 0
    // zb_secur_setup_preconfigured_key(g_key, 0);
    // zb_read_security_key();
    //how to calculate this? - starts at zero, increases with every new key so devices are up to date
    ZG->nwk.nib.active_key_seq_number = 0;
    // 32Bit counter that should not be zero and should actually persist factory resets unless APSME-SWITCH-KEY happens
    //nvram functions to write this counter are commented out and untested
    ZG->nwk.nib.outgoing_frame_counter = 0x10805;
    //ZG->nwk.nib.prev_outgoing_frame_counter =0xffff;
