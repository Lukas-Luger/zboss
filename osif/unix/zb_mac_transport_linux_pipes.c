...
static void dump_traf2(char *comment, zb_uint8_t *buf, zb_ushort_t len,
                       zb_ushort_t total)
{
    zb_ushort_t i;
    char s[512];
    ...
    *p = 0;
    ...
    TRACE_MSG(TRACE_MAC3, "%s: len %d total %d",
              (FMT__D_D, comment, len, total + len));
    for (i = 0; i < len; ++i) {
        sprintf(p, "%02X ", buf[i]);
        p += 3;
        if (i % 16 == 15) {
...
void zb_mac_transport_init(zb_char_t *rpipe_path, zb_char_t *wpipe_path)
{
    ...
    ZIG->ioctx.rpipe_path =
        rpipe_path ? rpipe_path : ZB_MAC_TRANSPORT_DEFAULT_RPIPE_PATH;
    ZIG->ioctx.wpipe_path =
        wpipe_path ? wpipe_path : ZB_MAC_TRANSPORT_DEFAULT_WPIPE_PATH;
...
void zb_mac_traffic_dump(zb_buf_t *buf, zb_bool_t is_w)
{
    ...
    TRACE_MSG(TRACE_MAC1,
              ">>zb_mac_traffic_dump #%d buf %p / %hd in_b %hd (size %d) is_w %d",
              (FMT__D_P_H_H_D_D, cnt, buf, ZB_REF_FROM_BUF(buf),
               buf->u.hdr.is_in_buf, ZB_BUF_LEN(buf), is_w));
    ...
    if (!ZIG->ioctx.dump_file) {
        ...
        if (!ZIG->ioctx.dump_file) {
            TRACE_MSG(TRACE_ERROR, "Can't open dump file %s",
                      (FMT__0, namefile));
        }
    }
    ...
    if (ZIG->ioctx.dump_file) {
        ...
        hdr.len -=
            (!is_w &&
             buf->u.hdr.len >
             ZB_MAC_EXTRA_DATA_SIZE ? ZB_MAC_EXTRA_DATA_SIZE : 0);
#endif
        hdr.type = ZB_MAC_TRANSPORT_TYPE_DUMP | (is_w ? 0x80 : 0);
        {
            ...
            hdr.time =
                ZB_MILLISECONDS_TO_BEACON_INTERVAL((zb_int_t)((t.tv_sec -
                                                               s_start_sec) *
                                                              1000 + t.tv_usec /
                                                              1000));
            TRACE_MSG(TRACE_MAC3, "t: start %u; t %u %u, time %d",
                      (FMT__D_D_D_D, s_start_sec, t.tv_sec, t.tv_usec,
                       (int)hdr.time));
        }
        if (fwrite(&hdr, sizeof(hdr), 1, ZIG->ioctx.dump_file) != 1
            || fwrite(ZB_BUF_BEGIN(buf), hdr.len - sizeof(hdr), 1,
                      ZIG->ioctx.dump_file) != 1) {
...
void zb_mac_transport_put_data(zb_buf_t *buf)
{
    ...
    if (ZIG->ioctx.send_data_buf) {
        TRACE_MSG(TRACE_ERROR, "output buffer is already in progress",
                  (FMT__0));
        ZB_ASSERT(0);
    }
    else {
...
void zb_mac_transport_start_recv(zb_buf_t *buf, zb_short_t bytes_to_recv)
{
    ...
    if (ZIG->ioctx.recv_data_buf) {
        TRACE_MSG(TRACE_ERROR, "incoming buffer is already in progress",
                  (FMT__0));
...
zb_ret_t read_from_pipe()
{
    ...
    if (!ZIG->ioctx.recv_data_buf) {
        TRACE_MSG(TRACE_MAC1,
                  "no buffer available, call mac to get incoming buffer",
                  (FMT__0));
    ...
    if (!ZB_BUF_LEN(ZIG->ioctx.recv_data_buf)) {
        ...
        if (bts_read
            ...
            *ptr = len;
            ...
            TRACE_MSG(TRACE_MAC1,
                      "error reading packet length bts_read %d len %d ret %d errno %d",
                      (FMT__D_D_D_D,
                       bts_read,
                       len, ret, errno));
        }
    ...
    if (ZB_BUF_LEN(ZIG->ioctx.recv_data_buf)) {
        ...
        do {
            ret = read(ZIG->ioctx.rpipe,
                       ZB_BUF_BEGIN(
                           ZIG->ioctx.recv_data_buf) + ZIG->ioctx.in_buf_read,
                       ZB_BUF_LEN(
                           ZIG->ioctx.recv_data_buf) - ZIG->ioctx.in_buf_read);
            if (ret > 0) {
                DUMP_TRAF("read", ZB_BUF_BEGIN(
                              ZIG->ioctx.recv_data_buf) + ZIG->ioctx.in_buf_read, ret,
                          bts_read);
    ...
    if (ZIG->ioctx.in_buf_read
        ...
        TRACE_MSG(TRACE_MAC1, "receive full buffer %hd len %d",
                  (FMT__H_D, ZB_REF_FROM_BUF(ZIG->ioctx.recv_data_buf),
                   ZIG->ioctx.in_buf_read));

#ifdef ZB_NS_BUILD
        {
            zb_uint8_t *p;
            /* If got data from ns-3, imitate UZ trailer (9b) */
            ZB_BUF_ALLOC_RIGHT(ZIG->ioctx.recv_data_buf, ZB_MAC_EXTRA_DATA_SIZE,
                               p);
...
static zb_ret_t write_to_pipe()
{
    ...
    if (ZIG->ioctx.send_data_buf) {
        ...
        if (ZIG->ioctx.wpipe == -1
            ...
            TRACE_MSG(TRACE_MAC1, "open wpipe %s. wait if nessesary",
                      (FMT__0, ZIG->ioctx.wpipe_path));
            while ((ZIG->ioctx.wpipe =
                        open(ZIG->ioctx.wpipe_path,
        ...
        while (ZIG->ioctx.out_buf_written <
               ZB_BUF_LEN(ZIG->ioctx.send_data_buf)) {
            ...
            if (ret >= 0) {
                if (FD_ISSET(ZIG->ioctx.wpipe, &writeSet)) {
                    do {
                        ret = write(ZIG->ioctx.wpipe,
                                    &ZIG->ioctx.send_data_buf->buf[ ZIG->ioctx.
                                                                    send_data_buf
                                                                    ->u.hdr.
                                                                    data_offset
                                                                    +
                                                                    ZIG->ioctx.
                                                                    out_buf_written ],
                                    ZIG->ioctx.send_data_buf->u.hdr.len -
                                    ZIG->ioctx.out_buf_written);
                        if (ret > 0) {
                            DUMP_TRAF("written",
                                      &ZIG->ioctx.send_data_buf->buf[ ZIG->ioctx
                                                                      .
                            ...
                            TRACE_MSG(TRACE_MAC3, "ret %d errno %d",
                                      (FMT__D_D, ret, errno));
                        }
                    } while ((ZIG->ioctx.out_buf_written <
                              ZB_BUF_LEN(ZIG->ioctx.send_data_buf))
                ...
                TRACE_MSG(TRACE_ERROR, "select error %s",
                          (FMT__0, strerror(errno)));
                sleep(1);
                continue;
            }
        }
...
void zb_mac_wait_for_ext_event()
{
    ...
    if (ZIG->ioctx.rpipe == -1) {
        ...
        TRACE_MSG(TRACE_MAC1, "open rpipe %d. ret %d",
                  (FMT__D_D, ZIG->ioctx.rpipe, errno));
    }
    ...
    if ((done_smsng = write_to_pipe()) == 0
        ...
        TRACE_MSG(TRACE_MAC3, "timer started = %d",
                  (FMT__D, ZB_TIMER_CTX().started));
        if (ZB_TIMER_CTX().started) {
            if (ZB_TIME_GE(ZB_TIMER_CTX().timer_stop,
                           ZB_TIMER_CTX().timer) &&
                ZB_TIMER_CTX().timer_stop != ZB_TIMER_CTX().timer) {
                tmo = ZB_TIME_SUBTRACT(ZB_TIMER_CTX().timer_stop,
                                       ZB_TIMER_CTX().timer);
                TRACE_MSG(TRACE_MAC3, "timer stop %d timer %d  tmo %d",
                          (FMT__D_D_D, ZB_TIMER_CTX().timer_stop,
                           ZB_TIMER_CTX().timer,
                           tmo));
            }
        }
        ...
        if (tmo != (zb_time_t) ~0) {
            ...
            TRACE_MSG(TRACE_MAC3, "default timeout %d",
                      (FMT__D, ZIG->ioctx.timeout));
            tv.tv_sec = ZIG->ioctx.timeout;
            tv.tv_usec = 0;
        }

        TRACE_MSG(TRACE_MAC3, "select() timeout %d.%d",
                  (FMT__D_D, tv.tv_sec, tv.tv_usec));
        ...
        /* deal with time */
        {
            ...
            TRACE_MSG(TRACE_MAC3, "delta %d (%d)",
                      (FMT__D_D, delta,
                       ZB_MILLISECONDS_TO_BEACON_INTERVAL(delta)));
            if (ZB_TIMER_CTX().started) {
                /* imitate 8051: move timer if it started only */
                ZB_TIMER_CTX().timer = ZB_TIME_ADD(
                    ZB_TIMER_CTX().timer,
                    ZB_MILLISECONDS_TO_BEACON_INTERVAL(
                        delta + ZIG->ioctx.time_delta_rest_ms +
                        ZIG->ioctx.time_delta_rest_us / 1000));
                if (ZB_MILLISECONDS_TO_BEACON_INTERVAL(delta +
                                                       ZIG->ioctx.
                                                       time_delta_rest_ms) ==
                    0) {
                    ZIG->ioctx.time_delta_rest_ms = delta +
                                                    ZIG->ioctx.
                                                    time_delta_rest_ms;
                    ZIG->ioctx.time_delta_rest_us +=
                        abs((zb_int_t)(end_t.tv_usec - start_t.tv_usec));
                }
                else {
            ...
            TRACE_MSG(TRACE_MAC3, "timer %d, delta_rest %d",
                      (FMT__D_D, ZB_TIMER_CTX().timer,
                       ZIG->ioctx.time_delta_rest_ms));
        ...
        if (ret >= 0) {
            if (FD_ISSET(ZIG->ioctx.rpipe, &read_set)) {
                ...
                if (ret == 0) {
                    ...
                    TRACE_MSG(TRACE_MAC1,
                              "seems rpipe is closed from the other side, reopen it",
                              (FMT__0));
                    ZIG->ioctx.rpipe = open(ZIG->ioctx.rpipe_path,
                                            O_RDONLY | O_NONBLOCK);
                    TRACE_MSG(TRACE_MAC1, "open rpipe %d. %s",
                              (FMT__D, ZIG->ioctx.rpipe, strerror(errno)));
                }
            }
            else if (ret == 0) {
            ...
            TRACE_MSG(TRACE_MAC1, "select error %d. %s",
                      (FMT__D, ret, strerror(errno)));
        }
    }
    if (done_smsng) {
