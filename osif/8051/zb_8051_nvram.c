                               ...
                               zb_ieee_addr_t mac_extended_address)
{
    ...
    zb_nvram_save = zb_test_nvram(ZB_CONFIG_PAGE + ZB_CONFIG_SIZE, tmp_buf,
                                  sizeof(tmp_buf));
    zb_erase_nvram(0);
    buf |=
        (aps_designated_coord | (aps_use_insecure_join << 1) |
         (aps_use_extended_pan_id << 2));
    written_bytes += zb_write_nvram(ZB_CONFIG_PAGE, &buf, 1);
    written_bytes +=
        zb_write_nvram(ZB_CONFIG_PAGE + written_bytes,
                       (zb_uint8_t *)mac_extended_address,
 ...
 */
    if (zb_nvram_save != RET_OK) {
        zb_write_nvram(ZB_CONFIG_PAGE + ZB_CONFIG_SIZE, tmp_buf,
                       sizeof(tmp_buf));
    }
...
zb_ret_t zb_config_from_nvram()
{
    ...
    //
    read_bytes +=
        zb_read_nvram(ZB_CONFIG_PAGE + read_bytes, buf, sizeof(zb_ieee_addr_t));
                                ...
                                zb_uint16_t nwk_short_addr)
{
    ...
    zb_nvram_save = zb_read_nvram(
        ZB_CONFIG_PAGE + ZB_SCRATCHPAD_PAGE_SIZE - sizeof(tmp_buf_keys),
        tmp_buf_keys, sizeof(tmp_buf_keys));

    written_bytes += ZB_CONFIG_SIZE; /* skip config section */
    written_bytes +=
        zb_write_nvram(ZB_CONFIG_PAGE + written_bytes,
                       (zb_uint8_t *)&profile_in_use, sizeof(zb_uint8_t));
    written_bytes +=
        zb_write_nvram(ZB_CONFIG_PAGE + written_bytes,
                       (zb_uint8_t *)long_parent_addr, sizeof(zb_ieee_addr_t));
    written_bytes +=
        zb_write_nvram(ZB_CONFIG_PAGE + written_bytes,
                       (zb_uint8_t *)&aps_channel_mask, sizeof(zb_uint32_t));
    written_bytes +=
        zb_write_nvram(ZB_CONFIG_PAGE + written_bytes,
                       (zb_uint8_t *)&short_parent_addr, sizeof(zb_uint16_t));
    written_bytes +=
        zb_write_nvram(ZB_CONFIG_PAGE + written_bytes, (zb_uint8_t *)&depth,
                       sizeof(zb_uint8_t));
    written_bytes +=
        zb_write_nvram(ZB_CONFIG_PAGE + written_bytes, (zb_uint8_t *)&pan_id,
                       sizeof(zb_uint16_t));
    written_bytes +=
        zb_write_nvram(ZB_CONFIG_PAGE + written_bytes, (zb_uint8_t *)ext_pan_id,
                       sizeof(zb_ext_pan_id_t));
    written_bytes +=
        zb_write_nvram(ZB_CONFIG_PAGE + written_bytes,
                       (zb_uint8_t *)&nwk_short_addr, sizeof(zb_uint16_t));
    /* restore security & config section */
    zb_write_nvram(ZB_CONFIG_PAGE, tmp_buf_config, ZB_CONFIG_SIZE);
    if (zb_nvram_save) {
        zb_write_nvram(ZB_CONFIG_PAGE + ZB_SCRATCHPAD_PAGE_SIZE -
                       sizeof(tmp_buf_keys), tmp_buf_keys,
                       sizeof(tmp_buf_keys));
    }
    return RET_OK;
}
...
zb_ret_t zb_read_formdesc_data()
{
    ...
    /* Stack profile */
    read_bytes +=
        zb_read_nvram(ZB_CONFIG_PAGE + read_bytes, buf, sizeof(zb_uint8_t));
    /* Parent's long addr */
    read_bytes +=
        zb_read_nvram(ZB_CONFIG_PAGE + read_bytes, buf, sizeof(zb_ieee_addr_t));
    ...
    if ((buf[0] & buf[1] & buf[2] & buf[3] & buf[4] & buf[5] & buf[6] &
        ...
        /* channel mask */
        read_bytes +=
            zb_read_nvram(ZB_CONFIG_PAGE + read_bytes, buf,
                          sizeof(zb_uint32_t));
        ...
        zb_address_update(parent_addr, s_parent_addr, ZB_FALSE,
...
/* define to pass bv-31, it contradicts specification, so this hack only for test*/
#ifdef TP_PRO_BV_31
        {
            ...
            for (channel_number = ZB_MAC_START_CHANNEL_NUMBER;
                 channel_number <= ZB_MAC_MAX_CHANNEL_NUMBER;
                 channel_number++) {
                /* check channel mask */
...
zb_ret_t zb_write_security_key()
{
    ...
    int i;
    zb_uint8_t tmp_buf[ZB_SCRATCHPAD_PAGE_SIZE - ZB_CCM_KEY_SIZE *
    ...
    for (i = 0; i < ZB_SECUR_N_SECUR_MATERIAL; i++) {
        zb_write_nvram(
            ZB_CONFIG_PAGE + ZB_SCRATCHPAD_PAGE_SIZE - ZB_CCM_KEY_SIZE *
            ((ZB_SECUR_N_SECUR_MATERIAL)-i),
...
zb_ret_t zb_write_up_counter()
{
    ...
    if (buf[ZB_SCRATCHPAD_PAGE_SIZE - 3] != 0xFF) {
        zb_erase_nvram(1);
        zb_write_nvram(ZB_VOLATILE_PAGE,
                       (zb_uint8_t *)&ZG->nwk.nib.outgoing_frame_counter,
                       sizeof(ZG->nwk.nib.outgoing_frame_counter));
    }
    else {
        for (i = ZB_SCRATCHPAD_PAGE_SIZE; i >= 0; i--) {
            if (((buf[i] != 0xFF) && (i < ZB_SCRATCHPAD_PAGE_SIZE)) ||
                (i == 0)) {
                zb_write_nvram(ZB_VOLATILE_PAGE + i + 1 * (i & 0x01),
                               (zb_uint8_t *)&ZG->nwk.nib.outgoing_frame_counter,
                               sizeof(ZG->nwk.nib.outgoing_frame_counter));
                break;
            }
