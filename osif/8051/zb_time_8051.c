...
#endif
ZB_INTERRUPT timer0_inter_handler(void) INTERRUPT_DECLARATION(
    TIMER0_INTER_NUMBER, REGISTER_BANK_2){
    ZB_TIMER_CTX().timer++;

    /* Stop timer if it expired or not running. */
    if (!ZB_TIMER_CTX().started ||
        ZB_TIME_GE((zb_time_t)ZB_TIMER_CTX().timer,
                   (zb_time_t)ZB_TIMER_CTX().timer_stop)) {
        ZB_STOP_8051_TIMER();
