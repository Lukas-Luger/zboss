...
#endif
ZB_INTERRUPT serial_inter_handler(void) INTERRUPT_DECLARATION(
    SERIAL_INTER_NUMBER, REGISTER_BANK_1){
...
#endif
ZB_INTERRUPT serial_rx_inter_handler(void) INTERRUPT_DECLARATION(
    SERIAL_RX_INTER_NUMBER, REGISTER_BANK_1){
...
static ZB_SDCC_XDATA zb_uint8_t s_putchar_s[PUTCHAR_BUF_SIZE] =
{ 0xde, 0xad, 0, ZB_MAC_TRANSPORT_TYPE_TRACE };
    ...
    if (s_putchar_len > INITIAL_OFFSET) {
        {
            zb_mac_transport_hdr_t *hdr =
                (zb_mac_transport_hdr_t *)(s_putchar_s + SIGNATURE_SIZE);
...
void zb_putchar(char c)
{
    ...
    if (s_putchar_len == PUTCHAR_BUF_SIZE
        ...
        zb_8051_serial_put_bytes(s_putchar_s, s_putchar_len);
#else
        zb_8051_serial_put_bytes(s_putchar_s + INITIAL_OFFSET,
                                 s_putchar_len - INITIAL_OFFSET);
#endif
        s_putchar_len = INITIAL_OFFSET;
    }
...
void zb_mac_traffic_dump(zb_buf_t *buf, zb_bool_t is_w) ZB_SDCC_REENTRANT
{
    ...
    TRACE_MSG(TRACE_MAC1,
              ">>zb_mac_traffic_dump is_w %hd #%u buf %p [%hd] %hx %hx %hx %hx",
