...
#endif
ZB_INTERRUPT spi_inter_handler(void) INTERRUPT_DECLARATION(SPI_INTER_NUMBER,
                                                           REGISTER_BANK_3){
    ...
    if (SPIF == 1) {
        ...
        if (SPI_CTX().io_request == ZB_NO_SPI_IO_REQUEST) {
            ...
            if (ZB_IOCTX().recv_data_buf != NULL) {
                ...
                [ZB_IOCTX().recv_data_buf->u.hdr.len +
                 ZB_IOCTX().recv_data_buf->u.hdr.data_offset] = SPI0DAT;
                ...
                if (ZB_IOCTX().bytes_to_recv == 0) {
                    ...
                    ZB_IOCTX().bytes_to_recv = ZB_MAC_PACKET_LENGTH_SIZE +
                                               ZB_MAC_EXTRA_DATA_SIZE +
                                               ZB_IOCTX().recv_data_buf->buf[
                        ZB_IOCTX().recv_data_buf->u.hdr.len +
                        ZB_IOCTX()
                        .recv_data_buf->u.hdr.data_offset];
                }
                ZB_IOCTX().recv_data_buf->u.hdr.len++;

                if (ZB_IOCTX().recv_data_buf->u.hdr.len >=
                    ZB_IOCTX().bytes_to_recv) {
        ...
        if (send_buf != NULL) {
            ...
            SPI0DAT =
                send_buf->buf[send_buf->u.hdr.data_offset + SPI_CTX().sent_num];
            need_tx = 0;
            SPI_CTX().sent_num++;
