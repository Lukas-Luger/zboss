...
#if defined ZB8051 && !defined ZB_NS_BUILD && (defined ZB_UZ2400 || \
                                               defined ZB_UZ2410)
...
#endif
ZB_INTERRUPT ubec_2400_handler(void) INTERRUPT_DECLARATION_NOBANK(
    UBEC_2400_INTER_NUMBER){
#ifdef ZB_USE_RX_QUEUE
    ZB_DISABLE_ALL_INTER();
    ZB_GET_ISRSTS();

#ifdef ZB_RESERVED_REGS_DUMP
    MAC_CTX().regs_queue.ring_buf[MAC_CTX().regs_queue.write_i]->isrsts =
        ZG->transceiver.int_status;
#endif
...
#ifdef ZB_RESERVED_REGS_DUMP
        MAC_CTX().regs_queue.ring_buf[MAC_CTX().regs_queue.write_i]->txsr =
            ZG->transceiver.tx_status;
#endif
        ZB_GET_TXSR();
#ifdef ZB_SECURITY
