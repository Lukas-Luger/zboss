...
void zb_mac_transport_start_recv(zb_buf_t *buf, zb_short_t bytes_to_recv)
{
    ...
    TRACE_MSG(TRACE_MAC3, "set recv_data_buf %p",
              (FMT__P, ZG->ioctx.recv_data_buf));
...
zb_ret_t zb_8051_serial_try_recv()
{
    ...
    if (!ZG->ioctx.recv_data_buf) {
        ...
        if (!ZB_RING_BUFFER_IS_EMPTY(&SER_CTX().rx_buf)) {
            MAC_SET_STATE_FOR_LAYER(ZB_MAC_IO_LAYER_RX,
                                    ZB_MAC_STATE_IO_RECV_PENDING);
            TRACE_MSG(TRACE_MAC3, "set mac rx state %i",
                      (FMT__D, ZB_MAC_STATE_IO_RECV_PENDING));
        }
    ...
    if (!ZG->ioctx.bytes_to_recv) {
        if (zb_8051_serial_get_bytes(&ZG->ioctx.bytes_to_recv, 1) != 0) {
            zb_uint8_t *p;
            ZB_BUF_INITIAL_ALLOC(ZG->ioctx.recv_data_buf,
                                 ZG->ioctx.bytes_to_recv, p);
            *p = ZG->ioctx.bytes_to_recv;
            TRACE_MSG( TRACE_MAC3, "will recv %d",
                       (FMT__D, (int)ZG->ioctx.bytes_to_recv));
            ZB_BUF_LEN(ZG->ioctx.recv_data_buf) = 1;
        }
        ret = RET_BLOCKED;
    }
    else {
        zb_ushort_t n = ZG->ioctx.bytes_to_recv - ZB_BUF_LEN(
            ZG->ioctx.recv_data_buf);

        if (n) {
            /* this is nonblocked read: get all we can */
            n = zb_8051_serial_get_bytes(
                ZB_BUF_BEGIN(ZG->ioctx.recv_data_buf) +
                ZB_BUF_LEN(ZG->ioctx.recv_data_buf),
                n);
            ...
            if (ZG->ioctx.bytes_to_recv ==
                ZB_BUF_LEN(ZG->ioctx.recv_data_buf)) {
                TRACE_MSG( TRACE_MAC3, "received buf %p %d bytes: %x %x %x %x",
                           (FMT__P_D_D_D_D_D,
                            ZG
                            ->ioctx.recv_data_buf,
                            (int)ZB_BUF_LEN(ZG->ioctx.recv_data_buf),
                            (
...
void zb_mac_transport_put_data(zb_buf_t *buf)
{
    ...
    TRACE_MSG( TRACE_MAC3, "put buf %p %d bytes",
               (FMT__P_D, buf, (int)ZB_BUF_LEN(buf)));
    /* synchronus write! */
